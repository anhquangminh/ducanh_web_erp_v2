...
{
    public class DM_ThongTinVatTuRepository : IDM_ThongTinVatTuRepository
    {
        ...
        public async Task<List<CT_DM_ThongTinVatTu>> GetHistoryIsValidEdit(string id)
        {
            using var context = _context.CreateDbContext();
            var data = await (from p1 in context.CT_DM_ThongTinVatTu_Logs
                              
                              where p1.IdChung == id && p1.IsValid == true
                              orderby p1.CreateAt
                              select new CT_DM_ThongTinVatTu
                              {
                                  Id = p1.Id,
                                  LoaiTruCuuHoa = p1.LoaiTruCuuHoa,
                                  DonVi = p1.DonVi,
                                  VatTu = p1.VatTu,
        ...
        public async Task<CT_DM_ThongTinVatTu> GetDetails(string id)
        {
            using var context = _context.CreateDbContext();
            var data = await (from p1 in context.CT_DM_ThongTinVatTus
        ...
        public async Task<List<CT_DM_ThongTinVatTu>> GetHistory(string id)
        {
            try
            {
                using var context = _context.CreateDbContext();
        ...
        public async Task<List<CT_DM_ThongTinVatTu>> GetAll(string groupId)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var entity = await context.CT_DM_ThongTinVatTus.Where(p => p.IsActive != 100).ToListAsync();
                return entity;
            }
            catch (Exception ex)
            {
        ...
        public async Task<CT_DM_ThongTinVatTu> GetById(string id)
        {
            using var context = _context.CreateDbContext();
            var entity = await context.CT_DM_ThongTinVatTus.Where(x => x.Id.Equals(id) && x.IsActive != 100).FirstOrDefaultAsync();
            if (entity == null)
            {
        ...
        public async Task Insert(CT_DM_ThongTinVatTu entity, string userId)
        {
            try
            {
                using var context = _context.CreateDbContext();
                ...
                context.CT_DM_ThongTinVatTus.Add(entity);
                var addLog = new CT_DM_ThongTinVatTu_Log()
                {
                    Id = entity.Id,
                    LoaiTruCuuHoa = entity.LoaiTruCuuHoa,
                    DonVi = entity.DonVi,
                    VatTu = entity.VatTu,
                ...
                context.CT_DM_ThongTinVatTu_Logs.Add(addLog);
        ...
        public async Task Update(CT_DM_ThongTinVatTu data, string userId)
        {
            using var context = _context.CreateDbContext();
            ...
            context.CT_DM_ThongTinVatTus.Update(data);
            if (data.IsActive == 3)
            {
                var updateLog = await (from p in context.CT_DM_ThongTinVatTu_Logs
                                       where p.IdChung == entity.Id && p.IsValid == true
                                       select p).ToListAsync();
                updateLog.ForEach(p => p.IsValid = false);
                context.CT_DM_ThongTinVatTu_Logs.UpdateRange(updateLog);
            }
            ...
            else if (entity.IsActive != 3)
            {
                ...
                if (updateLog != null)
                {
                    ...
                    context.CT_DM_ThongTinVatTu_Logs.Update(updateLog);
                }
            }
            var addLog = new CT_DM_ThongTinVatTu_Log()
            {
                Id = Guid.NewGuid().ToString(),
                LoaiTruCuuHoa = data.LoaiTruCuuHoa,
                DonVi = data.DonVi,
                VatTu = data.VatTu,
        ...
        public async Task UpdateMulti(CT_DM_ThongTinVatTu[] CT_DM_ThongTinVatTus)
        {
            using var context = _context.CreateDbContext();
            string[] ids = CT_DM_ThongTinVatTus.Select(x => x.Id).ToArray();
            var listEntities = await context.CT_DM_ThongTinVatTus.Where(x => ids.Contains(x.Id)).ToListAsync();
            foreach (var entity in listEntities)
            {
                context.CT_DM_ThongTinVatTus.Update(entity);
            }
            await context.SaveChangesAsync();
        }
        public async Task Delete(CT_DM_ThongTinVatTu data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            if (entity == null)
            ...
            else
            {
                ...
                {
                    var logdata = (from p in context.CT_DM_ThongTinVatTu_Logs
                                   where p.IdChung == entity.Id && p.IsValid == true
                                   orderby p.CreateAt ascending
                                   select p).Take(1).FirstOrDefault();
                    if (logdata != null)
                    {
                        data.LoaiTruCuuHoa = logdata.LoaiTruCuuHoa;
                        data.DonVi = logdata.DonVi;
                        data.VatTu = logdata.VatTu;
                        data.Ordinarily = logdata.Ordinarily;
                        data.ApprovalUserId = "";
                        data.DateApproval = null;

                        logdata.IsValid = true;
                        context.CT_DM_ThongTinVatTu_Logs.Update(logdata);
                    }

                    var logupdate = (from p in context.CT_DM_ThongTinVatTu_Logs
                                     where p.IdChung == entity.Id && p.IsValid == true
                                     select p).ToList();
                    if (logupdate != null)
                    {
                        logupdate.ForEach(p => p.IsValid = false);
                        context.CT_DM_ThongTinVatTu_Logs.UpdateRange(logupdate);
                    }
                    var addLog = new CT_DM_ThongTinVatTu_Log()
                    {
                        Id = Guid.NewGuid().ToString(),
        ...
        public async Task Approval(CT_DM_ThongTinVatTu data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            if (entity == null)
        ...
        public async Task NoApproval(CT_DM_ThongTinVatTu data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            ...
            else
            {
                ...
                else if (entity.IsActive == 1)
                {
                    ...
                    if (logdata != null)
                    {
                        entity.LoaiTruCuuHoa = logdata.LoaiTruCuuHoa;
                        entity.DonVi = logdata.DonVi;
                        entity.VatTu = logdata.VatTu;
        ...
        public async Task DeleteById(string id, string userId)
        {
            ...
            context.Set<CT_DM_ThongTinVatTu>().Remove(entity);
            await context.SaveChangesAsync();
        }
        public async Task<bool> CheckStatus(string ids, string name)
        {
            using var context = _context.CreateDbContext();
            var model = await context.CT_DM_ThongTinVatTus.Where(p => p.Id == ids && p.IsActive != 100).FirstOrDefaultAsync();
            if (model == null)
            {
                throw new Exception($"Không tìm thấy dữ liệu đã chọn");
            }
        ...
        public async Task<bool> CheckSave(CT_DM_ThongTinVatTu input)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var model = await (from p in context.CT_DM_ThongTinVatTu_Logs
                                   where p.GroupId == input.GroupId && p.IdChung != input.Id && p.IsValid == true && p.IsActive != 100
                                   && p.LoaiTruCuuHoa == input.LoaiTruCuuHoa && p.DonVi == input.DonVi && p.VatTu == input.VatTu
                                   select p).CountAsync();
                if (model > 0)
                {
        ...
        public async Task<bool> CheckEdit(CT_DM_ThongTinVatTu input)
        {
            try
            {
                using var context = _context.CreateDbContext();
        ...
        public async Task<bool> CheckDelete(CT_DM_ThongTinVatTu input)
        {
            try
            {
                using var context = _context.CreateDbContext();
