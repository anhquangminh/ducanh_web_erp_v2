...
{
    public class DM_LoaiCauKienRepository : IDM_LoaiCauKienRepository
    {
        private readonly IDbContextFactory<ApplicationDbContext> _context;
        public DM_LoaiCauKienRepository(IDbContextFactory<ApplicationDbContext> context)
        {
            _context = context;
        }
        public async Task<List<DM_ChungModel>> GetHistoryIsValidEdit(string id)
        {
            ...
            var data = await (from p1 in context.CT_DM_LoaiCauKien_Logs
        ...
        {
            using var context = _context.CreateDbContext();
            var data = await (from p1 in context.CT_DM_LoaiCauKiens
        ...
        public async Task<List<CT_DM_LoaiCauKien>> GetAll(string groupId)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var entity = await context.CT_DM_LoaiCauKiens.Where(p => p.IsActive != 100).OrderByDescending(p => p.CreateAt).ToListAsync();
                return entity;
            }
            catch (Exception ex)
            {
        ...
        public async Task<CT_DM_LoaiCauKien> GetById(string id)
        {
            using var context = _context.CreateDbContext();
            var entity = await context.CT_DM_LoaiCauKiens.Where(x => x.Id.Equals(id) && x.IsActive != 100).FirstOrDefaultAsync();
            if (entity == null)
            {
                throw new Exception($"Không tìm thấy danh mục đã chọn.");
            }
            return entity;
        }
        public async Task Insert(CT_DM_LoaiCauKien entity, string userId)
        {
            try
            {
                using var context = _context.CreateDbContext();
                if (entity == null)
                {
                    throw new Exception("Không có danh mục nào được thêm!");
                }
                context.CT_DM_LoaiCauKiens.Add(entity);
                var addLog = new CT_DM_LoaiCauKien_Log()
                {
                    Id = entity.Id,
                ...
                context.CT_DM_LoaiCauKien_Logs.Add(addLog);
        ...
        public async Task Update(CT_DM_LoaiCauKien data, string userId)
        {
            using var context = _context.CreateDbContext();
            ...
            context.CT_DM_LoaiCauKiens.Update(data);
            if (data.IsActive == 3)
            {
                var updateLog = await (from p in context.CT_DM_LoaiCauKien_Logs
                                       where p.IdChung == entity.Id && p.IsValid == true
                                       select p).ToListAsync();
                updateLog.ForEach(p => p.IsValid = false);
                context.CT_DM_LoaiCauKien_Logs.UpdateRange(updateLog);
            }
            ...
            else if (entity.IsActive != 3)
            {
                ...
                if (updateLog != null)
                {
                    ...
                    context.CT_DM_LoaiCauKien_Logs.Update(updateLog);
                }
            }
            var addLog = new CT_DM_LoaiCauKien_Log()
            {
        ...
        public async Task UpdateMulti(CT_DM_LoaiCauKien[] CT_DM_LoaiCauKiens)
        {
            using var context = _context.CreateDbContext();
            string[] ids = CT_DM_LoaiCauKiens.Select(x => x.Id).ToArray();
            var listEntities = await context.CT_DM_LoaiCauKiens.Where(x => ids.Contains(x.Id)).ToListAsync();
            foreach (var entity in listEntities)
            {
                context.CT_DM_LoaiCauKiens.Update(entity);
            }
            await context.SaveChangesAsync();
        }
        public async Task Delete(CT_DM_LoaiCauKien data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            if (entity == null)
            ...
            else
            {
                ...
                {
                    var logdata = (from p in context.CT_DM_LoaiCauKien_Logs
                                   where p.IdChung == entity.Id && p.IsValid == true
                    ...
                    if (logdata != null)
                    {
                       data.TenDanhMuc =logdata.TenDanhMuc;
                        ...
                        context.CT_DM_LoaiCauKien_Logs.Update(logdata);
                    }

                    var logupdate = (from p in context.CT_DM_LoaiCauKien_Logs
                                     where p.IdChung == entity.Id && p.IsValid == true
                                     select p).ToList();
                    if (logupdate != null)
                    {
                        logupdate.ForEach(p => p.IsValid = false);
                        context.CT_DM_LoaiCauKien_Logs.UpdateRange(logupdate);
                    }
                    var addLog = new CT_DM_LoaiCauKien_Log()
                    {
                        Id = Guid.NewGuid().ToString(),
        ...
        public async Task Approval(CT_DM_LoaiCauKien data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            if (entity == null)
        ...
        public async Task NoApproval(CT_DM_LoaiCauKien data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
        ...
        public async Task DeleteById(string id, string userId)
        {
            ...
            context.Set<CT_DM_LoaiCauKien>().Remove(entity);
            await context.SaveChangesAsync();
        }
        public async Task<bool> CheckStatus(string ids, string name)
        {
            using var context = _context.CreateDbContext();
            var model = await context.CT_DM_LoaiCauKiens.Where(p => p.Id == ids && p.IsActive != 100).FirstOrDefaultAsync();
            if (model == null)
            {
                throw new Exception($"Không tìm thấy danh mục đã chọn");
            }
            ...
            //{
            //throw new Exception($"Tên danh mục đã bị thay đổi.");
            //}
            return true;
        }
        public async Task<bool> CheckSave(CT_DM_LoaiCauKien input)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var model = await (from p in context.CT_DM_LoaiCauKien_Logs
        ...
        public async Task<bool> CheckEdit(CT_DM_LoaiCauKien input)
        {
            try
            {
                using var context = _context.CreateDbContext();
        ...
        public async Task<bool> CheckDelete(CT_DM_LoaiCauKien input)
        {
            try
            {
                using var context = _context.CreateDbContext();
