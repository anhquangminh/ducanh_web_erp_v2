...
namespace DucAnh2025.Services.HeThong
{
    public class CompanyTypeRepository : ICompanyTypeRepository
    {
        ...
        public CompanyTypeModel GetToEdit(string id)
        {
        ...
        public async Task<List<CompanyTypeModel>> GetHistoryIsValidEdit(string id)
        {
            using var context = _context.CreateDbContext();
            var data = await (from p1 in context.CompanyType_Logs
        ...
        public async Task<CompanyTypeModel> GetDetails(string id)
        {
            using var context = _context.CreateDbContext();
            var data = await (from p1 in context.CompanyTypes
        ...
        public async Task<List<CompanyTypeModel>> GetHistory(string id)
        {
            using var context = _context.CreateDbContext();
            var data = await (from p1 in context.CompanyType_Logs
                              join createBy in context.ApplicationUsers on p1.CreateBy equals createBy.Id
        ...
        public async Task Insert(CompanyType entity, string userId)
        {
            try
            {
                ...
                var addLog = new CompanyType_Log()
                {
                    Id = entity.Id,
                    TenLoaiChiNhanh = entity.TenLoaiChiNhanh,
                ...
                context.CompanyType_Logs.Add(addLog);
        ...
        public async Task Update(CompanyType data, string userId)
        {
            ...
            if (data.IsActive == 3)
            {
                var updateLog = await (from p in context.CompanyType_Logs
                                       where p.IdChung == entity.Id && p.IsValid == true
                                       select p).ToListAsync();
                updateLog.ForEach(p => p.IsValid = false);
                context.CompanyType_Logs.UpdateRange(updateLog);
            }
            ...
            else if (entity.IsActive != 3)
            {
                ...
                if (updateLog != null)
                {
                    ...
                    context.CompanyType_Logs.Update(updateLog);
                }
            }
            var addLog = new CompanyType_Log()
            {
                Id = Guid.NewGuid().ToString(),
                TenLoaiChiNhanh = data.TenLoaiChiNhanh,
        ...
        public async Task Delete(CompanyType data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            if (entity == null)
            ...
            else
            {
                ...
                {
                    var logdata = (from p in context.CompanyType_Logs
                                   where p.IdChung == entity.Id && p.IsValid == true
                                   orderby p.CreateAt ascending
                                   select p).Take(1).FirstOrDefault();
                    if (logdata != null)
                    {
                        data.TenLoaiChiNhanh = logdata.TenLoaiChiNhanh;
                        data.Ordinarily = logdata.Ordinarily;
                        data.ApprovalUserId = "";
                        data.DateApproval = null;

                        logdata.IsValid = true;
                        context.CompanyType_Logs.Update(logdata);
                    }

                    var logupdate = (from p in context.CompanyType_Logs
                                     where p.IdChung == entity.Id && p.IsValid == true
                                     select p).ToList();
                    if (logupdate != null)
                    {
                        logupdate.ForEach(p => p.IsValid = false);
                        context.CompanyType_Logs.UpdateRange(logupdate);
                    }
                    var addLog = new CompanyType_Log()
                    {
                        Id = Guid.NewGuid().ToString(),
        ...
        public async Task Approval(CompanyType data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            if (entity == null)
        ...
        public async Task NoApproval(CompanyType data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            ...
            else
            {
                ...
                else if (entity.IsActive == 1)
                {
                    ...
                    if (logdata != null)
                    {
                        entity.TenLoaiChiNhanh = logdata.TenLoaiChiNhanh;
                ...
                else if (entity.IsActive == 2)
                {
                    ...
                    if (logdata != null)
                    {
                        entity.TenLoaiChiNhanh = data.TenLoaiChiNhanh;
        ...
        {
            using var context = _context.CreateDbContext();
            var model = await context.CompanyTypes.Where(p => p.Id == ids && p.IsActive != 100).FirstOrDefaultAsync();
            if (model == null)
            {
                throw new Exception($"Không tìm thấy loại chi nhánh đã chọn");
            }
            ...
            //if (model != null && model.TenCompanyType.ToUpper() != name.ToUpper() && name != "")
            //{
            //throw new Exception($"Tên loại chi nhánh đã bị thay đổi.");
            //}
            return true;
        }
        public async Task<bool> CheckSave(CompanyType input)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var model = await (from p in context.CompanyType_Logs
                                   where p.GroupId == input.GroupId && p.IdChung != input.Id && p.IsValid == true && p.IsActive != 100
                                   && p.TenLoaiChiNhanh == input.TenLoaiChiNhanh
                                   select p).CountAsync();
                if (model > 0)
                {
        ...
        public async Task<bool> CheckEdit(CompanyType input)
        {
            try
            {
                using var context = _context.CreateDbContext();
        ...
        public async Task<bool> CheckDelete(CompanyType input)
        {
            try
            {
                using var context = _context.CreateDbContext();
