...
{
    public class Kho_NhapkhoNhienLieuRepository : IKho_NhapkhoNhienLieuRepository
    {
        private readonly IDbContextFactory<ApplicationDbContext> _context;
        public Kho_NhapkhoNhienLieuRepository(IDbContextFactory<ApplicationDbContext> context)
        {
            _context = context;
        }
        public async Task<List<Kho_NhapkhoNhienLieuModel>> GetAllByVM(
    Kho_NhapkhoNhienLieuModel dataModel,
    string groupId)
        {
            try
            {
                ...
                // ================= BASE QUERY =================
                var baseQuery = context.Kho_NhapkhoNhienLieus
                    .AsNoTracking()
                ...
                if (!string.IsNullOrEmpty(dataModel?.SoHopDong))
                    baseQuery = baseQuery.Where(m => m.SoHopDong == dataModel.SoHopDong);
                ...
                // ================= DICTIONARY =================
                var ChiNhanhsDict = await context.ChiNhanhs
                    .AsNoTracking()
                    .ToDictionaryAsync(x => x.Id);

                var Kho_HDMuaNhienLieusDict = await (
                    from hd in context.Kho_HDMuaNhienLieus.AsNoTracking()
                    join lcc in context.Kho_DM_LoaiNhaCungCaps.AsNoTracking()
                        on hd.Id_LoaiNhaCungCap equals lcc.Id
                    join ncc in context.Kho_DM_NhaCungCaps.AsNoTracking()
                        on hd.Id_NhaCungcap equals ncc.Id
                    select new
                    {
                        hd.Id,
                        hd.SoHopDong,
                        ncc.MaSoThue,
                        ncc.DiaChi,
                        TenLoaiNhaCungCap = lcc.TenLoaiNhaCungCap,
                        TenNhaCungCap = ncc.TenNhaCungcap
                    }
                ).ToDictionaryAsync(x => x.Id);

                var Kho_DM_NhomNhienLieusDict = await context.Kho_DM_NhomNhienLieus
                    .AsNoTracking()
                    .ToDictionaryAsync(x => x.Id);

                var Kho_DM_LoaiNhienLieusDict = await context.Kho_DM_LoaiNhienLieus
                    .AsNoTracking()
                    .ToDictionaryAsync(x => x.Id);

                var Kho_DM_NhanHieusDict = await context.Kho_DM_NhanHieus
                    .AsNoTracking()
                    .ToDictionaryAsync(x => x.Id);

                var Kho_DM_DonVisDict = await context.Kho_DM_DonVis
                    .AsNoTracking()
                    .ToDictionaryAsync(x => x.Id);

                // ================= MAP DATA =================
                var data = baseData.Select(p1 =>
                {
                    Kho_HDMuaNhienLieusDict.TryGetValue(p1.SoHopDong, out var hd);
                    ChiNhanhsDict.TryGetValue(p1.Id_TramTron, out var cn);
                    Kho_DM_NhomNhienLieusDict.TryGetValue(p1.Id_NhomNhienLieu, out var nhom);
                    Kho_DM_LoaiNhienLieusDict.TryGetValue(p1.Id_LoaiNhienLieu, out var loai);
                    Kho_DM_NhanHieusDict.TryGetValue(p1.Id_NhanHieu, out var nh);
                    Kho_DM_DonVisDict.TryGetValue(p1.Id_DonVi, out var dv);

                    return new Kho_NhapkhoNhienLieuModel
                    {
                        Id = p1.Id,
                        Id_TramTron = cn?.TenChiNhanh ?? "",
                        ...
                        Id_LoaiNhaCungCap = hd?.TenLoaiNhaCungCap ?? "",
                        Id_NhaCungcap = hd?.TenNhaCungCap ?? "",
                        MaSoThue = hd?.MaSoThue ?? "",
                        DiaChi = hd?.DiaChi ?? "",

                        Id_NhomNhienLieu = nhom?.TenNhienLieu ?? "",
                        Id_LoaiNhienLieu = loai?.LoaiNhienLieu ?? "",
                        Id_NhanHieu = nh?.TenNhanHieu ?? "",
                        Id_DonVi = dv?.TenDonVi ?? "",

                        SLNhapKhoCoThue = p1.SLNhapKhoCoThue,
                        SLNhapKhoKhongThue = p1.SLNhapKhoKhongThue,
                        DonViSauQuyDoi = p1.DonViSauQuyDoi,
                        SoLuongThucCoThue = p1.SoLuongThucCoThue,
                        SoLuongThucKhongThue = p1.SoLuongThucKhongThue,
                        ...
                        TongThanhTien = p1.TongThanhTien,

                        BienSoXe = p1.BienSoXe,
                        DonGiaCuocCoThue = p1.DonGiaCuocCoThue,
                        DonGiaCuocKhongThue = p1.DonGiaCuocKhongThue,
                        ThanhTienCuocCoThue = p1.ThanhTienCuocCoThue,
                        ThanhTienCuocKhongThue = p1.ThanhTienCuocKhongThue,

                        TongTienHangCoThue = p1.TongTienHangCoThue,
                        TongTienHangKhongThue = p1.TongTienHangKhongThue,
                        TongTienHang = p1.TongTienHang,
        ...
        public async Task<List<Kho_NhapkhoNhienLieuModel>> GetHistoryIsValidEdit(string id)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var query = (from p1 in context.Kho_NhapkhoNhienLieu_Logs
                             join ChiNhanhs1 in context.ChiNhanhs on p1.Id_TramTron equals ChiNhanhs1.Id
                             join Kho_HDMuaNhienLieus1 in context.Kho_HDMuaNhienLieus on p1.SoHopDong equals Kho_HDMuaNhienLieus1.Id
                             ...
                             orderby p1.CreateAt
                             select new Kho_NhapkhoNhienLieuModel
                             {
                                 Id = p1.Id,
                                 ...
                                 SoPhieu = p1.SoPhieu,
                                 BienSo = p1.BienSo,
                                 SoHopDong = Kho_HDMuaNhienLieus1.SoHopDong,
        ...
        public async Task<Kho_NhapkhoNhienLieuModel> GetDetails(string id)
        {
            using var context = _context.CreateDbContext();
            var data = await (from p1 in context.Kho_NhapkhoNhienLieus
                              ...
                              join Kho_HDMuaNhienLieus1 in context.Kho_HDMuaNhienLieus on p1.SoHopDong equals Kho_HDMuaNhienLieus1.Id into Kho_HDMuaNhienLieus11
                              from Kho_HDMuaNhienLieus1 in Kho_HDMuaNhienLieus11.DefaultIfEmpty()
        ...
        public async Task<List<Kho_NhapkhoNhienLieuModel>> GetHistory(string id)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var data = await (from p1 in context.Kho_NhapkhoNhienLieu_Logs
                                  join ChiNhanhs1 in context.ChiNhanhs on p1.Id_TramTron equals ChiNhanhs1.Id into ChiNhanhs11
                                  from ChiNhanhs1 in ChiNhanhs11.DefaultIfEmpty()
                                  join Kho_HDMuaNhienLieus1 in context.Kho_HDMuaNhienLieus on p1.SoHopDong equals Kho_HDMuaNhienLieus1.Id into Kho_HDMuaNhienLieus11
                                  from Kho_HDMuaNhienLieus1 in Kho_HDMuaNhienLieus11.DefaultIfEmpty()
        ...
        public async Task<List<Kho_NhapkhoNhienLieu>> GetAll(string groupId)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var entity = await context.Kho_NhapkhoNhienLieus.Where(p => p.IsActive != 100).OrderByDescending(p => p.CreateAt).ToListAsync();
                return entity;
            }
            catch (Exception ex)
            {
        ...
        public async Task<Kho_NhapkhoNhienLieu> GetById(string id)
        {
            using var context = _context.CreateDbContext();
            var entity = await context.Kho_NhapkhoNhienLieus.Where(x => x.Id.Equals(id) && x.IsActive != 100).FirstOrDefaultAsync();
            if (entity == null)
            {
        ...
        public async Task Insert(Kho_NhapkhoNhienLieu entity, string userId)
        {
            try
            {
                using var context = _context.CreateDbContext();
                ...
                context.Kho_NhapkhoNhienLieus.Add(entity);
                var addLog = new Kho_NhapkhoNhienLieu_Log()
                {
                    ...
                    SLNhapKhoCoThue = entity.SLNhapKhoCoThue,
                    SLNhapKhoKhongThue = entity.SLNhapKhoKhongThue,
                    DonViSauQuyDoi = entity.DonViSauQuyDoi,
                    SoLuongThucCoThue = entity.SoLuongThucCoThue,
                    SoLuongThucKhongThue = entity.SoLuongThucKhongThue,
                    ...
                    TongThanhTien = entity.TongThanhTien,
                    BienSoXe = entity.BienSoXe,
                    DonGiaCuocCoThue = entity.DonGiaCuocCoThue,
                    DonGiaCuocKhongThue = entity.DonGiaCuocKhongThue,
                    ThanhTienCuocCoThue = entity.ThanhTienCuocCoThue,
                    ThanhTienCuocKhongThue = entity.ThanhTienCuocKhongThue,
                    TongTienHangCoThue = entity.TongTienHangCoThue,
                    TongTienHangKhongThue = entity.TongTienHangKhongThue,
                    TongTienHang = entity.TongTienHang,
                ...
                context.Kho_NhapkhoNhienLieu_Logs.Add(addLog);
        ...
        public async Task Update(Kho_NhapkhoNhienLieu data, string userId)
        {
            using var context = _context.CreateDbContext();
            ...
            context.Kho_NhapkhoNhienLieus.Update(data);
            if (data.IsActive == 3)
            {
                var updateLog = await (from p in context.Kho_NhapkhoNhienLieu_Logs
                                       where p.IdChung == entity.Id && p.IsValid == true
                                       select p).ToListAsync();
                updateLog.ForEach(p => p.IsValid = false);
                context.Kho_NhapkhoNhienLieu_Logs.UpdateRange(updateLog);
            }
            ...
            else if (entity.IsActive != 3)
            {
                ...
                if (updateLog != null)
                {
                    ...
                    context.Kho_NhapkhoNhienLieu_Logs.Update(updateLog);
                }
            }
            var addLog = new Kho_NhapkhoNhienLieu_Log
            {
                ...
                SLNhapKhoCoThue = data.SLNhapKhoCoThue,
                SLNhapKhoKhongThue = data.SLNhapKhoKhongThue,
                DonViSauQuyDoi = data.DonViSauQuyDoi,
                SoLuongThucCoThue = data.SoLuongThucCoThue,
                SoLuongThucKhongThue = data.SoLuongThucKhongThue,
                ...
                DonGiaCuocCoThue = data.DonGiaCuocCoThue,
                DonGiaCuocKhongThue = data.DonGiaCuocKhongThue,
                ThanhTienCuocCoThue = data.ThanhTienCuocCoThue,
                ThanhTienCuocKhongThue = data.ThanhTienCuocKhongThue,
                TongTienHangCoThue = data.TongTienHangCoThue,
                TongTienHangKhongThue = data.TongTienHangKhongThue,
        ...
        public async Task UpdateMulti(Kho_NhapkhoNhienLieu[] Kho_NhapkhoNhienLieus)
        {
            using var context = _context.CreateDbContext();
            string[] ids = Kho_NhapkhoNhienLieus.Select(x => x.Id).ToArray();
            var listEntities = await context.Kho_NhapkhoNhienLieus.Where(x => ids.Contains(x.Id)).ToListAsync();
            foreach (var entity in listEntities)
            {
                context.Kho_NhapkhoNhienLieus.Update(entity);
            }
            await context.SaveChangesAsync();
        }
        public async Task Delete(Kho_NhapkhoNhienLieu data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            if (entity == null)
            ...
            else
            {
                ...
                {
                    var logdata = (from p in context.Kho_NhapkhoNhienLieu_Logs
                                   where p.IdChung == entity.Id && p.IsValid == true
                    ...
                    if (logdata != null)
                    {
                        ...
                        data.SLNhapKhoCoThue = logdata.SLNhapKhoCoThue;
                        data.SLNhapKhoKhongThue = logdata.SLNhapKhoKhongThue;
                        data.DonViSauQuyDoi = logdata.DonViSauQuyDoi;
                        data.SoLuongThucCoThue = logdata.SoLuongThucCoThue;
                        data.SoLuongThucKhongThue = logdata.SoLuongThucKhongThue;
                        ...
                        data.TongThanhTien = logdata.TongThanhTien;
                        data.BienSoXe = logdata.BienSoXe;
                        data.DonGiaCuocCoThue = logdata.DonGiaCuocCoThue;
                        data.DonGiaCuocKhongThue = logdata.DonGiaCuocKhongThue;
                        data.ThanhTienCuocCoThue = logdata.ThanhTienCuocCoThue;
                        data.ThanhTienCuocKhongThue = logdata.ThanhTienCuocKhongThue;
                        data.TongTienHangCoThue = logdata.TongTienHangCoThue;
                        data.TongTienHangKhongThue = logdata.TongTienHangKhongThue;
                        data.TongTienHang = logdata.TongTienHang;
                        ...
                        context.Kho_NhapkhoNhienLieu_Logs.Update(logdata);
                    }
                    var logupdate = (from p in context.Kho_NhapkhoNhienLieu_Logs
                                     where p.IdChung == entity.Id && p.IsValid == true
                                     select p).ToList();
                    if (logupdate != null)
                    {
                        logupdate.ForEach(p => p.IsValid = false);
                        context.Kho_NhapkhoNhienLieu_Logs.UpdateRange(logupdate);
                    }
                    var addLog = new Kho_NhapkhoNhienLieu_Log()
                    {
                        Id_TramTron = data.Id_TramTron,
        ...
        public async Task Approval(Kho_NhapkhoNhienLieu data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            if (entity == null)
        ...
        public async Task NoApproval(Kho_NhapkhoNhienLieu data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            ...
            else
            {
                ...
                else if (entity.IsActive == 1)
                {
                    ...
                    if (logdata != null)
                    {
                        ...
                        entity.SLNhapKhoCoThue = logdata.SLNhapKhoCoThue;
                        entity.SLNhapKhoKhongThue = logdata.SLNhapKhoKhongThue;
                        entity.DonViSauQuyDoi = logdata.DonViSauQuyDoi;
                        entity.SoLuongThucCoThue = logdata.SoLuongThucCoThue;
                        entity.SoLuongThucKhongThue = logdata.SoLuongThucKhongThue;
                        ...
                        entity.TongThanhTien = logdata.TongThanhTien;
                        entity.BienSoXe = logdata.BienSoXe;
                        entity.DonGiaCuocCoThue = logdata.DonGiaCuocCoThue;
                        entity.DonGiaCuocKhongThue = logdata.DonGiaCuocKhongThue;
                        entity.ThanhTienCuocCoThue = logdata.ThanhTienCuocCoThue;
                        entity.ThanhTienCuocKhongThue = logdata.ThanhTienCuocKhongThue;
                        entity.TongTienHangCoThue = logdata.TongTienHangCoThue;
                        entity.TongTienHangKhongThue = logdata.TongTienHangKhongThue;
                        entity.TongTienHang = logdata.TongTienHang;
        ...
        public async Task DeleteById(string id, string userId)
        {
            ...
            context.Set<Kho_NhapkhoNhienLieu>().Remove(entity);
            await context.SaveChangesAsync();
        }
        public async Task<bool> CheckStatus(string ids, string name)
        {
            using var context = _context.CreateDbContext();
            var model = await context.Kho_NhapkhoNhienLieus.Where(p => p.Id == ids && p.IsActive != 100).FirstOrDefaultAsync();
            if (model == null)
            {
                throw new Exception($"Không tìm thấy dữ liệu đã chonj!");
            }
        ...
        public async Task<bool> CheckSave(Kho_NhapkhoNhienLieu input)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var model = await (from p in context.Kho_NhapkhoNhienLieu_Logs
                            ...
                            && p.SLNhapKhoCoThue == input.SLNhapKhoCoThue
                            && p.SLNhapKhoKhongThue == input.SLNhapKhoKhongThue
                            && p.DonViSauQuyDoi == input.DonViSauQuyDoi
                            && p.SoLuongThucCoThue == input.SoLuongThucCoThue
                            && p.SoLuongThucKhongThue == input.SoLuongThucKhongThue
                            ...
                            && p.TongThanhTien == input.TongThanhTien
                            && p.BienSoXe == input.BienSoXe
                            && p.DonGiaCuocCoThue == input.DonGiaCuocCoThue
                            && p.DonGiaCuocKhongThue == input.DonGiaCuocKhongThue
                            && p.ThanhTienCuocCoThue == input.ThanhTienCuocCoThue
                            && p.ThanhTienCuocKhongThue == input.ThanhTienCuocKhongThue
                            && p.TongTienHangCoThue == input.TongTienHangCoThue
                            && p.TongTienHangKhongThue == input.TongTienHangKhongThue
                            && p.TongTienHang == input.TongTienHang
        ...
        public async Task<bool> CheckEdit(Kho_NhapkhoNhienLieu input)
        {
            try
            {
                using var context = _context.CreateDbContext();
        ...
        public async Task<bool> CheckDelete(Kho_NhapkhoNhienLieu input)
        {
            try
            {
                using var context = _context.CreateDbContext();
