...
{
    public class Kho_DM_DanhMucBaoCaoRepository : IKho_DM_DanhMucBaoCaoRepository
    {
        private readonly IDbContextFactory<ApplicationDbContext> _context;
        public Kho_DM_DanhMucBaoCaoRepository(IDbContextFactory<ApplicationDbContext> context)
        {
            _context = context;
        }
        public async Task<List<Kho_DM_DanhMucBaoCaoModel>> GetAllByVM(Kho_DM_DanhMucBaoCaoModel dataModel, string groupId)
        {
            try
            {
                using var context = _context.CreateDbContext();

                var baseQuery = context.Kho_DM_DanhMucBaoCaos
                    .AsNoTracking()
                ...
                var data = baseData.Select(p1 => new Kho_DM_DanhMucBaoCaoModel
                {
                    Id = p1.Id,
        ...
        public async Task<List<Kho_DM_DanhMucBaoCaoModel>> GetHistoryIsValidEdit(string id)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var query = (from p1 in context.Kho_DM_DanhMucBaoCao_Logs
                             ...
                             orderby p1.CreateAt
                             select new Kho_DM_DanhMucBaoCaoModel
                             {
                                 Id = p1.Id,
                                 TenDanhMuc = p1.TenDanhMuc,
                                 GroupId = p1.GroupId,
        ...
        public async Task<Kho_DM_DanhMucBaoCaoModel> GetDetails(string id)
        {
            using var context = _context.CreateDbContext();
            var data = await (from p1 in context.Kho_DM_DanhMucBaoCaos
                              join createBy in context.ApplicationUsers on p1.CreateBy equals createBy.Id
                              join a in context.ApplicationUsers on p1.ApprovalUserId equals a.Id into a1
        ...
        public async Task<List<Kho_DM_DanhMucBaoCaoModel>> GetHistory(string id)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var data = await (from p1 in context.Kho_DM_DanhMucBaoCao_Logs
                                  join createBy in context.ApplicationUsers on p1.CreateBy equals createBy.Id
                                  join a in context.ApplicationUsers on p1.ApprovalUserId equals a.Id into a1
                                  from approvalUserId in a1.DefaultIfEmpty()
                                  join b in context.Departments on p1.DepartmentId equals b.Id into b1
        ...
        public async Task<List<Kho_DM_DanhMucBaoCao>> GetAll(string groupId)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var entity = await context.Kho_DM_DanhMucBaoCaos.Where(p => p.IsActive != 100).OrderByDescending(p => p.CreateAt).ToListAsync();
                return entity;
            }
            catch (Exception ex)
            {
        ...
        public async Task<Kho_DM_DanhMucBaoCao> GetById(string id)
        {
            using var context = _context.CreateDbContext();
            var entity = await context.Kho_DM_DanhMucBaoCaos.Where(x => x.Id.Equals(id) && x.IsActive != 100).FirstOrDefaultAsync();
            if (entity == null)
            {
        ...
        public async Task Insert(Kho_DM_DanhMucBaoCao entity, string userId)
        {
            try
            {
                using var context = _context.CreateDbContext();
                ...
                context.Kho_DM_DanhMucBaoCaos.Add(entity);
                var addLog = new Kho_DM_DanhMucBaoCao_Log()
                {
                ...
                context.Kho_DM_DanhMucBaoCao_Logs.Add(addLog);
        ...
        public async Task Update(Kho_DM_DanhMucBaoCao data, string userId)
        {
            using var context = _context.CreateDbContext();
            ...
            context.Kho_DM_DanhMucBaoCaos.Update(data);
            if (data.IsActive == 3)
            {
                var updateLog = await (from p in context.Kho_DM_DanhMucBaoCao_Logs
                                       where p.IdChung == entity.Id && p.IsValid == true
                                       select p).ToListAsync();
                updateLog.ForEach(p => p.IsValid = false);
                context.Kho_DM_DanhMucBaoCao_Logs.UpdateRange(updateLog);
            }
            ...
            else if (entity.IsActive != 3)
            {
                ...
                if (updateLog != null)
                {
                    ...
                    context.Kho_DM_DanhMucBaoCao_Logs.Update(updateLog);
                }
            }
            var addLog = new Kho_DM_DanhMucBaoCao_Log
            {
        ...
        public async Task UpdateMulti(Kho_DM_DanhMucBaoCao[] Kho_DM_DanhMucBaoCaos)
        {
            using var context = _context.CreateDbContext();
            string[] ids = Kho_DM_DanhMucBaoCaos.Select(x => x.Id).ToArray();
            var listEntities = await context.Kho_DM_DanhMucBaoCaos.Where(x => ids.Contains(x.Id)).ToListAsync();
            foreach (var entity in listEntities)
            {
                context.Kho_DM_DanhMucBaoCaos.Update(entity);
            }
            await context.SaveChangesAsync();
        }
        public async Task Delete(Kho_DM_DanhMucBaoCao data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            if (entity == null)
            ...
            else
            {
                ...
                {
                    var logdata = (from p in context.Kho_DM_DanhMucBaoCao_Logs
                                   where p.IdChung == entity.Id && p.IsValid == true
                    ...
                    if (logdata != null)
                    {
                        ...
                        context.Kho_DM_DanhMucBaoCao_Logs.Update(logdata);
                    }
                    var logupdate = (from p in context.Kho_DM_DanhMucBaoCao_Logs
                                     where p.IdChung == entity.Id && p.IsValid == true
                                     select p).ToList();
                    if (logupdate != null)
                    {
                        logupdate.ForEach(p => p.IsValid = false);
                        context.Kho_DM_DanhMucBaoCao_Logs.UpdateRange(logupdate);
                    }
                    var addLog = new Kho_DM_DanhMucBaoCao_Log()
                    {
                        TenDanhMuc = data.TenDanhMuc,
        ...
        public async Task Approval(Kho_DM_DanhMucBaoCao data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            if (entity == null)
        ...
        public async Task NoApproval(Kho_DM_DanhMucBaoCao data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
        ...
        public async Task DeleteById(string id, string userId)
        {
            ...
            context.Set<Kho_DM_DanhMucBaoCao>().Remove(entity);
            await context.SaveChangesAsync();
        }
        public async Task<bool> CheckStatus(string ids, string name)
        {
            using var context = _context.CreateDbContext();
            var model = await context.Kho_DM_DanhMucBaoCaos.Where(p => p.Id == ids && p.IsActive != 100).FirstOrDefaultAsync();
            if (model == null)
            {
                throw new Exception($"Không tìm thấy dữ liệu đã chonj!");
            }
        ...
        public async Task<bool> CheckSave(Kho_DM_DanhMucBaoCao input)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var model = await (from p in context.Kho_DM_DanhMucBaoCao_Logs
        ...
        public async Task<bool> CheckEdit(Kho_DM_DanhMucBaoCao input)
        {
            try
            {
                using var context = _context.CreateDbContext();
        ...
        public async Task<bool> CheckDelete(Kho_DM_DanhMucBaoCao input)
        {
            try
            {
                using var context = _context.CreateDbContext();
