...
{
    public class Kho_DM_TenBaoCaoRepository : IKho_DM_TenBaoCaoRepository
    {
        private readonly IDbContextFactory<ApplicationDbContext> _context;
        public Kho_DM_TenBaoCaoRepository(IDbContextFactory<ApplicationDbContext> context)
        {
            _context = context;
        }
        public async Task<List<Kho_DM_TenBaoCaoModel>> GetAllByVM(Kho_DM_TenBaoCaoModel dataModel, string groupId)
        {
            try
            {
                using var context = _context.CreateDbContext();

                var baseQuery = context.Kho_DM_TenBaoCaos
                    .AsNoTracking()
                ...
                var data = baseData.Select(p1 => new Kho_DM_TenBaoCaoModel
                {
                    Id = p1.Id,
                    Id_DanhMuc = p1.Id_DanhMuc,
                    GroupId = p1.GroupId,
        ...
        public async Task<List<Kho_DM_TenBaoCaoModel>> GetHistoryIsValidEdit(string id)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var query = (from p1 in context.Kho_DM_TenBaoCao_Logs
                             ...
                             orderby p1.CreateAt
                             select new Kho_DM_TenBaoCaoModel
                             {
                                 Id = p1.Id,
                                 Id_DanhMuc = p1.Id_DanhMuc,
                                 GroupId = p1.GroupId,
        ...
        public async Task<Kho_DM_TenBaoCaoModel> GetDetails(string id)
        {
            using var context = _context.CreateDbContext();
            var data = await (from p1 in context.Kho_DM_TenBaoCaos
                              join createBy in context.ApplicationUsers on p1.CreateBy equals createBy.Id
                              join a in context.ApplicationUsers on p1.ApprovalUserId equals a.Id into a1
        ...
        public async Task<List<Kho_DM_TenBaoCaoModel>> GetHistory(string id)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var data = await (from p1 in context.Kho_DM_TenBaoCao_Logs
                                  join createBy in context.ApplicationUsers on p1.CreateBy equals createBy.Id
                                  join a in context.ApplicationUsers on p1.ApprovalUserId equals a.Id into a1
                                  from approvalUserId in a1.DefaultIfEmpty()
                                  join b in context.Departments on p1.DepartmentId equals b.Id into b1
        ...
        public async Task<List<Kho_DM_TenBaoCao>> GetAll(string groupId)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var entity = await context.Kho_DM_TenBaoCaos.Where(p => p.IsActive != 100).OrderByDescending(p => p.CreateAt).ToListAsync();
                return entity;
            }
            catch (Exception ex)
            {
        ...
        public async Task<Kho_DM_TenBaoCao> GetById(string id)
        {
            using var context = _context.CreateDbContext();
            var entity = await context.Kho_DM_TenBaoCaos.Where(x => x.Id.Equals(id) && x.IsActive != 100).FirstOrDefaultAsync();
            if (entity == null)
            {
        ...
        public async Task Insert(Kho_DM_TenBaoCao entity, string userId)
        {
            try
            {
                using var context = _context.CreateDbContext();
                ...
                context.Kho_DM_TenBaoCaos.Add(entity);
                var addLog = new Kho_DM_TenBaoCao_Log()
                {
                    Id_DanhMuc = entity.Id_DanhMuc,
                ...
                context.Kho_DM_TenBaoCao_Logs.Add(addLog);
        ...
        public async Task Update(Kho_DM_TenBaoCao data, string userId)
        {
            using var context = _context.CreateDbContext();
            ...
            context.Kho_DM_TenBaoCaos.Update(data);
            if (data.IsActive == 3)
            {
                var updateLog = await (from p in context.Kho_DM_TenBaoCao_Logs
                                       where p.IdChung == entity.Id && p.IsValid == true
                                       select p).ToListAsync();
                updateLog.ForEach(p => p.IsValid = false);
                context.Kho_DM_TenBaoCao_Logs.UpdateRange(updateLog);
            }
            ...
            else if (entity.IsActive != 3)
            {
                ...
                if (updateLog != null)
                {
                    ...
                    context.Kho_DM_TenBaoCao_Logs.Update(updateLog);
                }
            }
            var addLog = new Kho_DM_TenBaoCao_Log
            {
        ...
        public async Task UpdateMulti(Kho_DM_TenBaoCao[] Kho_DM_TenBaoCaos)
        {
            using var context = _context.CreateDbContext();
            string[] ids = Kho_DM_TenBaoCaos.Select(x => x.Id).ToArray();
            var listEntities = await context.Kho_DM_TenBaoCaos.Where(x => ids.Contains(x.Id)).ToListAsync();
            foreach (var entity in listEntities)
            {
                context.Kho_DM_TenBaoCaos.Update(entity);
            }
            await context.SaveChangesAsync();
        }
        public async Task Delete(Kho_DM_TenBaoCao data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            if (entity == null)
            ...
            else
            {
                ...
                {
                    var logdata = (from p in context.Kho_DM_TenBaoCao_Logs
                                   where p.IdChung == entity.Id && p.IsValid == true
                                   orderby p.CreateAt ascending
                                   select p).Take(1).FirstOrDefault();
                    if (logdata != null)
                    {
                        data.Id_DanhMuc = logdata.Id_DanhMuc;
                        ...
                        context.Kho_DM_TenBaoCao_Logs.Update(logdata);
                    }
                    var logupdate = (from p in context.Kho_DM_TenBaoCao_Logs
                                     where p.IdChung == entity.Id && p.IsValid == true
                                     select p).ToList();
                    if (logupdate != null)
                    {
                        logupdate.ForEach(p => p.IsValid = false);
                        context.Kho_DM_TenBaoCao_Logs.UpdateRange(logupdate);
                    }
                    var addLog = new Kho_DM_TenBaoCao_Log()
                    {
                        Id_DanhMuc = data.Id_DanhMuc,
        ...
        public async Task Approval(Kho_DM_TenBaoCao data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            if (entity == null)
        ...
        public async Task NoApproval(Kho_DM_TenBaoCao data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            ...
            else
            {
                ...
                else if (entity.IsActive == 1)
                {
                    ...
                    if (logdata != null)
                    {
                        entity.Id_DanhMuc = logdata.Id_DanhMuc;
        ...
        public async Task DeleteById(string id, string userId)
        {
            ...
            context.Set<Kho_DM_TenBaoCao>().Remove(entity);
            await context.SaveChangesAsync();
        }
        public async Task<bool> CheckStatus(string ids, string name)
        {
            using var context = _context.CreateDbContext();
            var model = await context.Kho_DM_TenBaoCaos.Where(p => p.Id == ids && p.IsActive != 100).FirstOrDefaultAsync();
            if (model == null)
            {
                throw new Exception($"Không tìm thấy dữ liệu đã chonj!");
            }
        ...
        public async Task<bool> CheckSave(Kho_DM_TenBaoCao input)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var model = await (from p in context.Kho_DM_TenBaoCao_Logs
                                   where p.GroupId == input.GroupId && p.IdChung != input.Id && p.IsValid == true && p.IsActive != 100
                            && p.Id_DanhMuc == input.Id_DanhMuc
        ...
        public async Task<bool> CheckEdit(Kho_DM_TenBaoCao input)
        {
            try
            {
                using var context = _context.CreateDbContext();
        ...
        public async Task<bool> CheckDelete(Kho_DM_TenBaoCao input)
        {
            try
            {
                using var context = _context.CreateDbContext();
