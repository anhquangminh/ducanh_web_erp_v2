...
{
    public class NhanSu_ContractRepository : INhanSu_ContractRepository
    {
        private readonly IDbContextFactory<ApplicationDbContext> _context;
        public NhanSu_ContractRepository(IDbContextFactory<ApplicationDbContext> context)
        {
            _context = context;
        }
        public async Task<List<NhanSu_ContractModel>> GetAllByVM(NhanSu_ContractModel dataModel, string groupId)
        {
            try
            {
                using var context = _context.CreateDbContext();

                var baseQuery = context.NhanSu_Contracts
                    .AsNoTracking()
                ...
                if (!string.IsNullOrEmpty(dataModel?.ContractTypeId))
                {
                    baseQuery = baseQuery.Where(m => m.ContractTypeId == dataModel.ContractTypeId);
                }
                if (!string.IsNullOrEmpty(dataModel?.ContractStatusId))
                {
                    baseQuery = baseQuery.Where(m => m.ContractStatusId == dataModel.ContractStatusId);
                }
                ...
                var data = baseData.Select(p1 => new NhanSu_ContractModel
                {
                    Id = p1.Id,
                    ...
                    ContractTypeId = NhanSu_DM_ContractTypesDict.TryGetValue(p1.ContractTypeId, out var tmpNhanSu_DM_ContractTypes) ? tmpNhanSu_DM_ContractTypes.ContractTypeName : "",
                    ContractCode = p1.ContractCode,
                    ContractName = p1.ContractName,
                    SigningDate = p1.SigningDate,
                    EffectiveDate = p1.EffectiveDate,
                    ExpirationDate = p1.ExpirationDate,
                    ContractStatusId = NhanSu_DM_WorkStatusDict.TryGetValue(p1.ContractStatusId, out var tmpNhanSu_DM_WorkStatus) ? tmpNhanSu_DM_WorkStatus.WorkStatusName : "",
                    ContractSalary = p1.ContractSalary,
                    DurationMonths = p1.DurationMonths,
                    FilePath = p1.FilePath,
                    IsESignRequired = p1.IsESignRequired,
                    LastRenewalDate = p1.LastRenewalDate,
                    Notes = p1.Notes,
                    AlertDate = p1.AlertDate,
                    SignerId = NhanSu_EmployeeProfilesDict.TryGetValue(p1.SignerId, out var tmpNhanSu_EmployeeProfiles2) ? tmpNhanSu_EmployeeProfiles1.FullName : "",
                    SignerPosition = p1.SignerPosition,
                    GroupId = p1.GroupId,
        ...
        public async Task<List<NhanSu_ContractModel>> GetHistoryIsValidEdit(string id)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var query = (from p1 in context.NhanSu_Contract_Logs
                             join NhanSu_EmployeeProfiles1 in context.NhanSu_EmployeeProfiles on p1.EmployeeId equals NhanSu_EmployeeProfiles1.Id
                             join NhanSu_DM_ContractTypes1 in context.NhanSu_DM_ContractTypes on p1.ContractTypeId equals NhanSu_DM_ContractTypes1.Id
                             join NhanSu_DM_WorkStatus1 in context.NhanSu_DM_WorkStatus on p1.ContractStatusId equals NhanSu_DM_WorkStatus1.Id
                             ...
                             orderby p1.CreateAt
                             select new NhanSu_ContractModel
                             {
                                 Id = p1.Id,
                                 EmployeeId = p1.EmployeeId,
                                 MaNhanVien = NhanSu_EmployeeProfiles1.EmployeeId,
                                 FullName = NhanSu_EmployeeProfiles1.FullName,
                                 ContractTypeId = NhanSu_DM_ContractTypes1.ContractTypeName,
                                 ContractCode = p1.ContractCode,
                                 ContractName = p1.ContractName,
                                 SigningDate = p1.SigningDate,
                                 EffectiveDate = p1.EffectiveDate,
                                 ExpirationDate = p1.ExpirationDate,
                                 ContractStatusId = NhanSu_DM_WorkStatus1.WorkStatusName,
                                 ContractSalary = p1.ContractSalary,
                                 DurationMonths = p1.DurationMonths,
                                 FilePath = p1.FilePath,
                                 IsESignRequired = p1.IsESignRequired,
        ...
        public async Task<NhanSu_ContractModel> GetDetails(string id)
        {
            using var context = _context.CreateDbContext();
            var data = await (from p1 in context.NhanSu_Contracts
                              ...
                              join NhanSu_DM_ContractTypes1 in context.NhanSu_DM_ContractTypes on p1.ContractTypeId equals NhanSu_DM_ContractTypes1.Id into NhanSu_DM_ContractTypes11
                              from NhanSu_DM_ContractTypes1 in NhanSu_DM_ContractTypes11.DefaultIfEmpty()
                              join NhanSu_DM_WorkStatus1 in context.NhanSu_DM_WorkStatus on p1.ContractStatusId equals NhanSu_DM_WorkStatus1.Id into NhanSu_DM_WorkStatus11
        ...
        public async Task<List<NhanSu_ContractModel>> GetHistory(string id)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var data = await (from p1 in context.NhanSu_Contract_Logs
                                  join NhanSu_EmployeeProfiles1 in context.NhanSu_EmployeeProfiles on p1.EmployeeId equals NhanSu_EmployeeProfiles1.Id into NhanSu_EmployeeProfiles11
                                  from NhanSu_EmployeeProfiles1 in NhanSu_EmployeeProfiles11.DefaultIfEmpty()
                                  join NhanSu_DM_ContractTypes1 in context.NhanSu_DM_ContractTypes on p1.ContractTypeId equals NhanSu_DM_ContractTypes1.Id into NhanSu_DM_ContractTypes11
                                  from NhanSu_DM_ContractTypes1 in NhanSu_DM_ContractTypes11.DefaultIfEmpty()
                                  ...
                                  select new NhanSu_ContractModel
                                  {
                                      Id = p1.Id,
                                      EmployeeId = p1.EmployeeId,
                                      MaNhanVien = NhanSu_EmployeeProfiles1.FullName,
                                      FullName = NhanSu_EmployeeProfiles1.FullName,
                                      ContractTypeId = NhanSu_DM_ContractTypes1.ContractTypeName,
                                      ContractCode = p1.ContractCode,
                                      ContractName = p1.ContractName,
        ...
        public async Task<List<NhanSu_Contract>> GetAll(string groupId)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var entity = await context.NhanSu_Contracts.Where(p => p.IsActive != 100).OrderByDescending(p => p.CreateAt).ToListAsync();
                return entity;
            }
            catch (Exception ex)
            {
        ...
        public async Task<NhanSu_Contract> GetById(string id)
        {
            using var context = _context.CreateDbContext();
            var entity = await context.NhanSu_Contracts.Where(x => x.Id.Equals(id) && x.IsActive != 100).FirstOrDefaultAsync();
            if (entity == null)
            {
        ...
        public async Task Insert(NhanSu_Contract entity, string userId)
        {
            try
            {
                using var context = _context.CreateDbContext();
                ...
                context.NhanSu_Contract_Logs.Add(addLog);
        ...
        public async Task Update(NhanSu_Contract data, string userId)
        {
            using var context = _context.CreateDbContext();
            ...
            context.NhanSu_Contracts.Update(data);
            if (data.IsActive == 3)
            {
                var updateLog = await (from p in context.NhanSu_Contract_Logs
                                       where p.IdChung == entity.Id && p.IsValid == true
                                       select p).ToListAsync();
                updateLog.ForEach(p => p.IsValid = false);
                context.NhanSu_Contract_Logs.UpdateRange(updateLog);
            }
            ...
            else if (entity.IsActive != 3)
            {
                ...
                if (updateLog != null)
                {
                    ...
                    context.NhanSu_Contract_Logs.Update(updateLog);
                }
            }
            var addLog = new NhanSu_Contract_Log
            {
                ...
                IsESignRequired = data.IsESignRequired,
        ...
        public async Task UpdateMulti(NhanSu_Contract[] NhanSu_Contracts)
        {
            using var context = _context.CreateDbContext();
            string[] ids = NhanSu_Contracts.Select(x => x.Id).ToArray();
            var listEntities = await context.NhanSu_Contracts.Where(x => ids.Contains(x.Id)).ToListAsync();
            foreach (var entity in listEntities)
            {
                context.NhanSu_Contracts.Update(entity);
            }
            await context.SaveChangesAsync();
        }
        public async Task Delete(NhanSu_Contract data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            if (entity == null)
            ...
            else
            {
                ...
                {
                    var logdata = (from p in context.NhanSu_Contract_Logs
                                   where p.IdChung == entity.Id && p.IsValid == true
                    ...
                    if (logdata != null)
                    {
                        ...
                        data.ContractTypeId = logdata.ContractTypeId;
                        data.ContractCode = logdata.ContractCode;
                        data.ContractName = logdata.ContractName;
                        data.SigningDate = logdata.SigningDate;
                        data.EffectiveDate = logdata.EffectiveDate;
                        data.ExpirationDate = logdata.ExpirationDate;
                        data.ContractStatusId = logdata.ContractStatusId;
                        data.ContractSalary = logdata.ContractSalary;
                        data.DurationMonths = logdata.DurationMonths;
                        data.FilePath = logdata.FilePath;
                        data.IsESignRequired = logdata.IsESignRequired;
                        data.LastRenewalDate = logdata.LastRenewalDate;
                        data.Notes = logdata.Notes;
                        data.AlertDate = logdata.AlertDate;
                        data.SignerId = logdata.SignerId;
                        data.SignerPosition = logdata.SignerPosition;
                        ...
                        context.NhanSu_Contract_Logs.Update(logdata);
                    }
                    var logupdate = (from p in context.NhanSu_Contract_Logs
                                     where p.IdChung == entity.Id && p.IsValid == true
                                     select p).ToList();
                    if (logupdate != null)
                    {
                        logupdate.ForEach(p => p.IsValid = false);
                        context.NhanSu_Contract_Logs.UpdateRange(logupdate);
                    }
                    var addLog = new NhanSu_Contract_Log()
                    {
                        EmployeeId = data.EmployeeId,
        ...
        public async Task Approval(NhanSu_Contract data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            if (entity == null)
        ...
        public async Task NoApproval(NhanSu_Contract data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            ...
            else
            {
                ...
                else if (entity.IsActive == 1)
                {
                    ...
                    if (logdata != null)
                    {
                        ...
                        entity.ContractTypeId = logdata.ContractTypeId;
                        entity.ContractCode = logdata.ContractCode;
                        entity.ContractName = logdata.ContractName;
                        entity.SigningDate = logdata.SigningDate;
                        entity.EffectiveDate = logdata.EffectiveDate;
                        entity.ExpirationDate = logdata.ExpirationDate;
                        entity.ContractStatusId = logdata.ContractStatusId;
                        entity.ContractSalary = logdata.ContractSalary;
                        entity.DurationMonths = logdata.DurationMonths;
                        entity.FilePath = logdata.FilePath;
                        entity.IsESignRequired = logdata.IsESignRequired;
                        entity.LastRenewalDate = logdata.LastRenewalDate;
                        entity.Notes = logdata.Notes;
                        entity.AlertDate = logdata.AlertDate;
                        entity.SignerId = logdata.SignerId;
                        entity.SignerPosition = logdata.SignerPosition;
        ...
        public async Task DeleteById(string id, string userId)
        {
            ...
            context.Set<NhanSu_Contract>().Remove(entity);
            await context.SaveChangesAsync();
        }
        public async Task<bool> CheckStatus(string ids, string name)
        {
            using var context = _context.CreateDbContext();
            var model = await context.NhanSu_Contracts.Where(p => p.Id == ids && p.IsActive != 100).FirstOrDefaultAsync();
            if (model == null)
            {
                throw new Exception($"Không tìm thấy dữ liệu đã chonj!");
            }
        ...
        public async Task<bool> CheckSave(NhanSu_Contract input)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var model = await (from p in context.NhanSu_Contract_Logs
                                   where p.GroupId == input.GroupId && p.IdChung != input.Id && p.IsValid == true && p.IsActive != 100
                            && p.EmployeeId == input.EmployeeId
                            && p.ContractTypeId == input.ContractTypeId
                            && p.ContractCode == input.ContractCode
                            && p.ContractName == input.ContractName
                            && p.SigningDate == input.SigningDate
                            && p.EffectiveDate == input.EffectiveDate
                            && p.ExpirationDate == input.ExpirationDate
                            && p.ContractStatusId == input.ContractStatusId
                            && p.ContractSalary == input.ContractSalary
                            && p.DurationMonths == input.DurationMonths
                            && p.FilePath == input.FilePath
                            && p.IsESignRequired == input.IsESignRequired
                            && p.LastRenewalDate == input.LastRenewalDate
                            && p.Notes == input.Notes
                            && p.AlertDate == input.AlertDate
                            && p.SignerId == input.SignerId
                            && p.SignerPosition == input.SignerPosition
        ...
        public async Task<bool> CheckEdit(NhanSu_Contract input)
        {
            try
            {
                using var context = _context.CreateDbContext();
        ...
        public async Task<bool> CheckDelete(NhanSu_Contract input)
        {
            try
            {
                using var context = _context.CreateDbContext();
