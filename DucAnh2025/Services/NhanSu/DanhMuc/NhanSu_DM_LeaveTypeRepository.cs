...
{
    public class NhanSu_DM_LeaveTypeRepository : INhanSu_DM_LeaveTypeRepository
    {
        private readonly IDbContextFactory<ApplicationDbContext> _context;
        public NhanSu_DM_LeaveTypeRepository(IDbContextFactory<ApplicationDbContext> context)
        {
            _context = context;
        }
        public async Task<List<NhanSu_DM_LeaveTypeModel>> GetAllByVM(NhanSu_DM_LeaveTypeModel dataModel, string groupId)
        {
            try
            {
                using var context = _context.CreateDbContext();

                var baseQuery = context.NhanSu_DM_LeaveTypes
                    .AsNoTracking()
                ...
                var data = baseData.Select(p1 => new NhanSu_DM_LeaveTypeModel
                {
                    Id = p1.Id,
                    LeaveTypeName = p1.LeaveTypeName,
                    IsDeductible = p1.IsDeductible,
                    DefaultQuota = p1.DefaultQuota,
        ...
        public async Task<List<NhanSu_DM_LeaveTypeModel>> GetHistoryIsValidEdit(string id)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var query = (from p1 in context.NhanSu_DM_LeaveType_Logs
                             ...
                             orderby p1.CreateAt
                             select new NhanSu_DM_LeaveTypeModel
                             {
                                 Id = p1.Id,
                                 LeaveTypeName = p1.LeaveTypeName,
                                 IsDeductible = p1.IsDeductible,
        ...
        public async Task<NhanSu_DM_LeaveTypeModel> GetDetails(string id)
        {
            using var context = _context.CreateDbContext();
            var data = await (from p1 in context.NhanSu_DM_LeaveTypes
                              join createBy in context.ApplicationUsers on p1.CreateBy equals createBy.Id
                              join a in context.ApplicationUsers on p1.ApprovalUserId equals a.Id into a1
        ...
        public async Task<List<NhanSu_DM_LeaveTypeModel>> GetHistory(string id)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var data = await (from p1 in context.NhanSu_DM_LeaveType_Logs
                                  join createBy in context.ApplicationUsers on p1.CreateBy equals createBy.Id
                                  join a in context.ApplicationUsers on p1.ApprovalUserId equals a.Id into a1
                                  from approvalUserId in a1.DefaultIfEmpty()
                                  join b in context.Departments on p1.DepartmentId equals b.Id into b1
        ...
        public async Task<List<NhanSu_DM_LeaveType>> GetAll(string groupId)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var entity = await context.NhanSu_DM_LeaveTypes.Where(p => p.IsActive != 100).OrderByDescending(p => p.CreateAt).ToListAsync();
                return entity;
            }
            catch (Exception ex)
            {
        ...
        public async Task<NhanSu_DM_LeaveType> GetById(string id)
        {
            using var context = _context.CreateDbContext();
            var entity = await context.NhanSu_DM_LeaveTypes.Where(x => x.Id.Equals(id) && x.IsActive != 100).FirstOrDefaultAsync();
            if (entity == null)
            {
        ...
        public async Task Insert(NhanSu_DM_LeaveType entity, string userId)
        {
            try
            {
                using var context = _context.CreateDbContext();
                ...
                context.NhanSu_DM_LeaveTypes.Add(entity);
                var addLog = new NhanSu_DM_LeaveType_Log()
                {
                    LeaveTypeName = entity.LeaveTypeName,
                    IsDeductible = entity.IsDeductible,
                    DefaultQuota = entity.DefaultQuota,
                ...
                context.NhanSu_DM_LeaveType_Logs.Add(addLog);
        ...
        public async Task Update(NhanSu_DM_LeaveType data, string userId)
        {
            using var context = _context.CreateDbContext();
            ...
            context.NhanSu_DM_LeaveTypes.Update(data);
            if (data.IsActive == 3)
            {
                var updateLog = await (from p in context.NhanSu_DM_LeaveType_Logs
                                       where p.IdChung == entity.Id && p.IsValid == true
                                       select p).ToListAsync();
                updateLog.ForEach(p => p.IsValid = false);
                context.NhanSu_DM_LeaveType_Logs.UpdateRange(updateLog);
            }
            ...
            else if (entity.IsActive != 3)
            {
                ...
                if (updateLog != null)
                {
                    ...
                    context.NhanSu_DM_LeaveType_Logs.Update(updateLog);
                }
            }
            var addLog = new NhanSu_DM_LeaveType_Log
            {
        ...
        public async Task UpdateMulti(NhanSu_DM_LeaveType[] NhanSu_DM_LeaveTypes)
        {
            using var context = _context.CreateDbContext();
            string[] ids = NhanSu_DM_LeaveTypes.Select(x => x.Id).ToArray();
            var listEntities = await context.NhanSu_DM_LeaveTypes.Where(x => ids.Contains(x.Id)).ToListAsync();
            foreach (var entity in listEntities)
            {
                context.NhanSu_DM_LeaveTypes.Update(entity);
            }
            await context.SaveChangesAsync();
        }
        public async Task Delete(NhanSu_DM_LeaveType data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            if (entity == null)
            ...
            else
            {
                ...
                {
                    var logdata = (from p in context.NhanSu_DM_LeaveType_Logs
                                   where p.IdChung == entity.Id && p.IsValid == true
                                   orderby p.CreateAt ascending
                                   select p).Take(1).FirstOrDefault();
                    if (logdata != null)
                    {
                        data.LeaveTypeName = logdata.LeaveTypeName;
                        data.IsDeductible = logdata.IsDeductible;
                        data.DefaultQuota = logdata.DefaultQuota;
                        data.LegalBasis = logdata.LegalBasis;
                        ...
                        context.NhanSu_DM_LeaveType_Logs.Update(logdata);
                    }
                    var logupdate = (from p in context.NhanSu_DM_LeaveType_Logs
                                     where p.IdChung == entity.Id && p.IsValid == true
                                     select p).ToList();
                    if (logupdate != null)
                    {
                        logupdate.ForEach(p => p.IsValid = false);
                        context.NhanSu_DM_LeaveType_Logs.UpdateRange(logupdate);
                    }
                    var addLog = new NhanSu_DM_LeaveType_Log()
                    {
                        LeaveTypeName = data.LeaveTypeName,
        ...
        public async Task Approval(NhanSu_DM_LeaveType data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            if (entity == null)
        ...
        public async Task NoApproval(NhanSu_DM_LeaveType data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            ...
            else
            {
                ...
                else if (entity.IsActive == 1)
                {
                    ...
                    if (logdata != null)
                    {
                        entity.LeaveTypeName = logdata.LeaveTypeName;
                        entity.IsDeductible = logdata.IsDeductible;
                        entity.DefaultQuota = logdata.DefaultQuota;
                        entity.LegalBasis = logdata.LegalBasis;
        ...
        public async Task DeleteById(string id, string userId)
        {
            ...
            context.Set<NhanSu_DM_LeaveType>().Remove(entity);
            await context.SaveChangesAsync();
        }
        public async Task<bool> CheckStatus(string ids, string name)
        {
            using var context = _context.CreateDbContext();
            var model = await context.NhanSu_DM_LeaveTypes.Where(p => p.Id == ids && p.IsActive != 100).FirstOrDefaultAsync();
            if (model == null)
            {
                throw new Exception($"Không tìm thấy dữ liệu đã chonj!");
            }
        ...
        public async Task<bool> CheckSave(NhanSu_DM_LeaveType input)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var model = await (from p in context.NhanSu_DM_LeaveType_Logs
                                   where p.GroupId == input.GroupId && p.IdChung != input.Id && p.IsValid == true && p.IsActive != 100
                            && p.LeaveTypeName == input.LeaveTypeName
                            && p.IsDeductible == input.IsDeductible
                            && p.DefaultQuota == input.DefaultQuota
                            && p.LegalBasis == input.LegalBasis
        ...
        public async Task<bool> CheckEdit(NhanSu_DM_LeaveType input)
        {
            try
            {
                using var context = _context.CreateDbContext();
        ...
        public async Task<bool> CheckDelete(NhanSu_DM_LeaveType input)
        {
            try
            {
                using var context = _context.CreateDbContext();
