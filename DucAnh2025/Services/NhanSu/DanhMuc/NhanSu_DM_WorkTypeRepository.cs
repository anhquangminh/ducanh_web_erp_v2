...
{
    public class NhanSu_DM_WorkTypeRepository : INhanSu_DM_WorkTypeRepository
    {
        private readonly IDbContextFactory<ApplicationDbContext> _context;
        public NhanSu_DM_WorkTypeRepository(IDbContextFactory<ApplicationDbContext> context)
        {
            _context = context;
        }
        public async Task<List<NhanSu_DM_WorkTypeModel>> GetAllByVM(NhanSu_DM_WorkTypeModel dataModel, string groupId)
        {
            try
            {
                using var context = _context.CreateDbContext();

                var baseQuery = context.NhanSu_DM_WorkTypes
                    .AsNoTracking()
                ...
                var data = baseData.Select(p1 => new NhanSu_DM_WorkTypeModel
                {
                    Id = p1.Id,
                    WorkTypeName = p1.WorkTypeName,
                    WorkTypeGroup = p1.WorkTypeGroup,
                    SalaryFactor = p1.SalaryFactor,
        ...
        public async Task<List<NhanSu_DM_WorkTypeModel>> GetHistoryIsValidEdit(string id)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var query = (from p1 in context.NhanSu_DM_WorkType_Logs
                             ...
                             orderby p1.CreateAt
                             select new NhanSu_DM_WorkTypeModel
                             {
                                 Id = p1.Id,
                                 WorkTypeName = p1.WorkTypeName,
                                 WorkTypeGroup = p1.WorkTypeGroup,
        ...
        public async Task<NhanSu_DM_WorkTypeModel> GetDetails(string id)
        {
            using var context = _context.CreateDbContext();
            var data = await (from p1 in context.NhanSu_DM_WorkTypes
                              join createBy in context.ApplicationUsers on p1.CreateBy equals createBy.Id
                              join a in context.ApplicationUsers on p1.ApprovalUserId equals a.Id into a1
        ...
        public async Task<List<NhanSu_DM_WorkTypeModel>> GetHistory(string id)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var data = await (from p1 in context.NhanSu_DM_WorkType_Logs
                                  join createBy in context.ApplicationUsers on p1.CreateBy equals createBy.Id
                                  join a in context.ApplicationUsers on p1.ApprovalUserId equals a.Id into a1
                                  from approvalUserId in a1.DefaultIfEmpty()
                                  join b in context.Departments on p1.DepartmentId equals b.Id into b1
        ...
        public async Task<List<NhanSu_DM_WorkType>> GetAll(string groupId)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var entity = await context.NhanSu_DM_WorkTypes.Where(p => p.IsActive != 100).OrderByDescending(p => p.CreateAt).ToListAsync();
                return entity;
            }
            catch (Exception ex)
            {
        ...
        public async Task<NhanSu_DM_WorkType> GetById(string id)
        {
            using var context = _context.CreateDbContext();
            var entity = await context.NhanSu_DM_WorkTypes.Where(x => x.Id.Equals(id) && x.IsActive != 100).FirstOrDefaultAsync();
            if (entity == null)
            {
        ...
        public async Task Insert(NhanSu_DM_WorkType entity, string userId)
        {
            try
            {
                using var context = _context.CreateDbContext();
                ...
                context.NhanSu_DM_WorkTypes.Add(entity);
                var addLog = new NhanSu_DM_WorkType_Log()
                {
                    WorkTypeName = entity.WorkTypeName,
                    WorkTypeGroup = entity.WorkTypeGroup,
                    SalaryFactor = entity.SalaryFactor,
                ...
                context.NhanSu_DM_WorkType_Logs.Add(addLog);
        ...
        public async Task Update(NhanSu_DM_WorkType data, string userId)
        {
            using var context = _context.CreateDbContext();
            ...
            context.NhanSu_DM_WorkTypes.Update(data);
            if (data.IsActive == 3)
            {
                var updateLog = await (from p in context.NhanSu_DM_WorkType_Logs
                                       where p.IdChung == entity.Id && p.IsValid == true
                                       select p).ToListAsync();
                updateLog.ForEach(p => p.IsValid = false);
                context.NhanSu_DM_WorkType_Logs.UpdateRange(updateLog);
            }
            ...
            else if (entity.IsActive != 3)
            {
                ...
                if (updateLog != null)
                {
                    ...
                    context.NhanSu_DM_WorkType_Logs.Update(updateLog);
                }
            }
            var addLog = new NhanSu_DM_WorkType_Log
            {
                WorkTypeName = data.WorkTypeName,
                WorkTypeGroup = data.WorkTypeGroup,
        ...
        public async Task UpdateMulti(NhanSu_DM_WorkType[] NhanSu_DM_WorkTypes)
        {
            using var context = _context.CreateDbContext();
            string[] ids = NhanSu_DM_WorkTypes.Select(x => x.Id).ToArray();
            var listEntities = await context.NhanSu_DM_WorkTypes.Where(x => ids.Contains(x.Id)).ToListAsync();
            foreach (var entity in listEntities)
            {
                context.NhanSu_DM_WorkTypes.Update(entity);
            }
            await context.SaveChangesAsync();
        }
        public async Task Delete(NhanSu_DM_WorkType data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            if (entity == null)
            ...
            else
            {
                ...
                {
                    var logdata = (from p in context.NhanSu_DM_WorkType_Logs
                                   where p.IdChung == entity.Id && p.IsValid == true
                                   orderby p.CreateAt ascending
                                   select p).Take(1).FirstOrDefault();
                    if (logdata != null)
                    {
                        data.WorkTypeName = logdata.WorkTypeName;
                        data.WorkTypeGroup = logdata.WorkTypeGroup;
                        data.SalaryFactor = logdata.SalaryFactor;
                        ...
                        context.NhanSu_DM_WorkType_Logs.Update(logdata);
                    }
                    var logupdate = (from p in context.NhanSu_DM_WorkType_Logs
                                     where p.IdChung == entity.Id && p.IsValid == true
                                     select p).ToList();
                    if (logupdate != null)
                    {
                        logupdate.ForEach(p => p.IsValid = false);
                        context.NhanSu_DM_WorkType_Logs.UpdateRange(logupdate);
                    }
                    var addLog = new NhanSu_DM_WorkType_Log()
                    {
                        WorkTypeName = data.WorkTypeName,
        ...
        public async Task Approval(NhanSu_DM_WorkType data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            if (entity == null)
        ...
        public async Task NoApproval(NhanSu_DM_WorkType data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            ...
            else
            {
                ...
                else if (entity.IsActive == 1)
                {
                    ...
                    if (logdata != null)
                    {
                        entity.WorkTypeName = logdata.WorkTypeName;
                        entity.WorkTypeGroup = logdata.WorkTypeGroup;
                        entity.SalaryFactor = logdata.SalaryFactor;
        ...
        public async Task DeleteById(string id, string userId)
        {
            ...
            context.Set<NhanSu_DM_WorkType>().Remove(entity);
            await context.SaveChangesAsync();
        }
        public async Task<bool> CheckStatus(string ids, string name)
        {
            using var context = _context.CreateDbContext();
            var model = await context.NhanSu_DM_WorkTypes.Where(p => p.Id == ids && p.IsActive != 100).FirstOrDefaultAsync();
            if (model == null)
            {
                throw new Exception($"Không tìm thấy dữ liệu đã chonj!");
            }
        ...
        public async Task<bool> CheckSave(NhanSu_DM_WorkType input)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var model = await (from p in context.NhanSu_DM_WorkType_Logs
                                   where p.GroupId == input.GroupId && p.IdChung != input.Id && p.IsValid == true && p.IsActive != 100
                            && p.WorkTypeName == input.WorkTypeName
                            && p.WorkTypeGroup == input.WorkTypeGroup
                            && p.SalaryFactor == input.SalaryFactor
        ...
        public async Task<bool> CheckEdit(NhanSu_DM_WorkType input)
        {
            try
            {
                using var context = _context.CreateDbContext();
        ...
        public async Task<bool> CheckDelete(NhanSu_DM_WorkType input)
        {
            try
            {
                using var context = _context.CreateDbContext();
