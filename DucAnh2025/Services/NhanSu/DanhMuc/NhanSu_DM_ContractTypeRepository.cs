...
{
    public class NhanSu_DM_ContractTypeRepository : INhanSu_DM_ContractTypeRepository
    {
        private readonly IDbContextFactory<ApplicationDbContext> _context;
        public NhanSu_DM_ContractTypeRepository(IDbContextFactory<ApplicationDbContext> context)
        {
            _context = context;
        }
        public async Task<List<NhanSu_DM_ContractTypeModel>> GetAllByVM(NhanSu_DM_ContractTypeModel dataModel, string groupId)
        {
            try
            {
                using var context = _context.CreateDbContext();

                var baseQuery = context.NhanSu_DM_ContractTypes
                    .AsNoTracking()
                    .Where(p => p.GroupId == groupId && p.IsActive != 100);

                if (!string.IsNullOrEmpty(dataModel?.NextContractTypeId))
                {
                    baseQuery = baseQuery.Where(m => m.NextContractTypeId == dataModel.NextContractTypeId);
                }
                ...
                var data = baseData.Select(p1 => new NhanSu_DM_ContractTypeModel
                {
                    Id = p1.Id,
                    ContractTypeName = p1.ContractTypeName,
                    Description = p1.Description,
                    DurationMonths = p1.DurationMonths,
                    WarningDays = p1.WarningDays,
                    IsRenewable = p1.IsRenewable,
                    NextContractTypeId = NhanSu_DM_ContractTypesDict.TryGetValue(p1.NextContractTypeId, out var tmpNhanSu_DM_ContractTypes) ? tmpNhanSu_DM_ContractTypes.ContractTypeName : "",
                    ProbationSalaryRate = p1.ProbationSalaryRate,
                    GroupId = p1.GroupId,
        ...
        public async Task<List<NhanSu_DM_ContractTypeModel>> GetHistoryIsValidEdit(string id)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var query = (from p1 in context.NhanSu_DM_ContractType_Logs
                             join NhanSu_DM_ContractTypes1 in context.NhanSu_DM_ContractTypes on p1.NextContractTypeId equals NhanSu_DM_ContractTypes1.Id
                             ...
                             orderby p1.CreateAt
                             select new NhanSu_DM_ContractTypeModel
                             {
                                 Id = p1.Id,
                                 ContractTypeName = p1.ContractTypeName,
                                 Description = p1.Description,
                                 ...
                                 WarningDays = p1.WarningDays,
                                 IsRenewable = p1.IsRenewable,
                                 NextContractTypeId = NhanSu_DM_ContractTypes1.ContractTypeName,
                                 ProbationSalaryRate = p1.ProbationSalaryRate,
                                 GroupId = p1.GroupId,
                                 Ordinarily = p1.Ordinarily,
        ...
        public async Task<NhanSu_DM_ContractTypeModel> GetDetails(string id)
        {
            using var context = _context.CreateDbContext();
            var data = await (from p1 in context.NhanSu_DM_ContractTypes
                              join NhanSu_DM_ContractTypes1 in context.NhanSu_DM_ContractTypes on p1.NextContractTypeId equals NhanSu_DM_ContractTypes1.Id into NhanSu_DM_ContractTypes11
        ...
        public async Task<List<NhanSu_DM_ContractTypeModel>> GetHistory(string id)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var data = await (from p1 in context.NhanSu_DM_ContractType_Logs
                                  join NhanSu_DM_ContractTypes1 in context.NhanSu_DM_ContractTypes on p1.NextContractTypeId equals NhanSu_DM_ContractTypes1.Id into NhanSu_DM_ContractTypes11
                                  from NhanSu_DM_ContractTypes1 in NhanSu_DM_ContractTypes11.DefaultIfEmpty()
                                  join createBy in context.ApplicationUsers on p1.CreateBy equals createBy.Id
                                  join a in context.ApplicationUsers on p1.ApprovalUserId equals a.Id into a1
        ...
        public async Task<List<NhanSu_DM_ContractType>> GetAll(string groupId)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var entity = await context.NhanSu_DM_ContractTypes.Where(p => p.IsActive != 100).OrderByDescending(p => p.CreateAt).ToListAsync();
                return entity;
            }
            catch (Exception ex)
            {
        ...
        public async Task<NhanSu_DM_ContractType> GetById(string id)
        {
            using var context = _context.CreateDbContext();
            var entity = await context.NhanSu_DM_ContractTypes.Where(x => x.Id.Equals(id) && x.IsActive != 100).FirstOrDefaultAsync();
            if (entity == null)
            {
        ...
        public async Task Insert(NhanSu_DM_ContractType entity, string userId)
        {
            try
            {
                using var context = _context.CreateDbContext();
                ...
                context.NhanSu_DM_ContractTypes.Add(entity);
                var addLog = new NhanSu_DM_ContractType_Log()
                {
                    ContractTypeName = entity.ContractTypeName,
                    Description = entity.Description,
                    DurationMonths = entity.DurationMonths,
                    WarningDays = entity.WarningDays,
                    IsRenewable = entity.IsRenewable,
                    NextContractTypeId = entity.NextContractTypeId,
                    ProbationSalaryRate = entity.ProbationSalaryRate,
                ...
                context.NhanSu_DM_ContractType_Logs.Add(addLog);
        ...
        public async Task Update(NhanSu_DM_ContractType data, string userId)
        {
            using var context = _context.CreateDbContext();
            ...
            context.NhanSu_DM_ContractTypes.Update(data);
            if (data.IsActive == 3)
            {
                var updateLog = await (from p in context.NhanSu_DM_ContractType_Logs
                                       where p.IdChung == entity.Id && p.IsValid == true
                                       select p).ToListAsync();
                updateLog.ForEach(p => p.IsValid = false);
                context.NhanSu_DM_ContractType_Logs.UpdateRange(updateLog);
            }
            ...
            else if (entity.IsActive != 3)
            {
                ...
                if (updateLog != null)
                {
                    ...
                    context.NhanSu_DM_ContractType_Logs.Update(updateLog);
                }
            }
            var addLog = new NhanSu_DM_ContractType_Log
            {
                ...
                NextContractTypeId = data.NextContractTypeId,
        ...
        public async Task UpdateMulti(NhanSu_DM_ContractType[] NhanSu_DM_ContractTypes)
        {
            using var context = _context.CreateDbContext();
            string[] ids = NhanSu_DM_ContractTypes.Select(x => x.Id).ToArray();
            var listEntities = await context.NhanSu_DM_ContractTypes.Where(x => ids.Contains(x.Id)).ToListAsync();
            foreach (var entity in listEntities)
            {
                context.NhanSu_DM_ContractTypes.Update(entity);
            }
            await context.SaveChangesAsync();
        }
        public async Task Delete(NhanSu_DM_ContractType data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            if (entity == null)
            ...
            else
            {
                ...
                {
                    var logdata = (from p in context.NhanSu_DM_ContractType_Logs
                                   where p.IdChung == entity.Id && p.IsValid == true
                                   orderby p.CreateAt ascending
                                   select p).Take(1).FirstOrDefault();
                    if (logdata != null)
                    {
                        data.ContractTypeName = logdata.ContractTypeName;
                        data.Description = logdata.Description;
                        data.DurationMonths = logdata.DurationMonths;
                        data.WarningDays = logdata.WarningDays;
                        data.IsRenewable = logdata.IsRenewable;
                        data.NextContractTypeId = logdata.NextContractTypeId;
                        data.ProbationSalaryRate = logdata.ProbationSalaryRate;
                        ...
                        context.NhanSu_DM_ContractType_Logs.Update(logdata);
                    }
                    var logupdate = (from p in context.NhanSu_DM_ContractType_Logs
                                     where p.IdChung == entity.Id && p.IsValid == true
                                     select p).ToList();
                    if (logupdate != null)
                    {
                        logupdate.ForEach(p => p.IsValid = false);
                        context.NhanSu_DM_ContractType_Logs.UpdateRange(logupdate);
                    }
                    var addLog = new NhanSu_DM_ContractType_Log()
                    {
                        ContractTypeName = data.ContractTypeName,
        ...
        public async Task Approval(NhanSu_DM_ContractType data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            if (entity == null)
        ...
        public async Task NoApproval(NhanSu_DM_ContractType data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            ...
            else
            {
                ...
                else if (entity.IsActive == 1)
                {
                    ...
                    if (logdata != null)
                    {
                        entity.ContractTypeName = logdata.ContractTypeName;
                        entity.Description = logdata.Description;
                        entity.DurationMonths = logdata.DurationMonths;
                        entity.WarningDays = logdata.WarningDays;
                        entity.IsRenewable = logdata.IsRenewable;
                        entity.NextContractTypeId = logdata.NextContractTypeId;
                        entity.ProbationSalaryRate = logdata.ProbationSalaryRate;
        ...
        public async Task DeleteById(string id, string userId)
        {
            ...
            context.Set<NhanSu_DM_ContractType>().Remove(entity);
            await context.SaveChangesAsync();
        }
        public async Task<bool> CheckStatus(string ids, string name)
        {
            using var context = _context.CreateDbContext();
            var model = await context.NhanSu_DM_ContractTypes.Where(p => p.Id == ids && p.IsActive != 100).FirstOrDefaultAsync();
            if (model == null)
            {
                throw new Exception($"Không tìm thấy dữ liệu đã chonj!");
            }
        ...
        public async Task<bool> CheckSave(NhanSu_DM_ContractType input)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var model = await (from p in context.NhanSu_DM_ContractType_Logs
                                   where p.GroupId == input.GroupId && p.IdChung != input.Id && p.IsValid == true && p.IsActive != 100
                            && p.ContractTypeName == input.ContractTypeName
                            && p.Description == input.Description
                            && p.DurationMonths == input.DurationMonths
                            && p.WarningDays == input.WarningDays
                            && p.IsRenewable == input.IsRenewable
                            && p.NextContractTypeId == input.NextContractTypeId
                            && p.ProbationSalaryRate == input.ProbationSalaryRate
        ...
        public async Task<bool> CheckEdit(NhanSu_DM_ContractType input)
        {
            try
            {
                using var context = _context.CreateDbContext();
        ...
        public async Task<bool> CheckDelete(NhanSu_DM_ContractType input)
        {
            try
            {
                using var context = _context.CreateDbContext();
