...
{
    public class NhanSu_DM_ChangeTypeRepository : INhanSu_DM_ChangeTypeRepository
    {
        private readonly IDbContextFactory<ApplicationDbContext> _context;
        public NhanSu_DM_ChangeTypeRepository(IDbContextFactory<ApplicationDbContext> context)
        {
            _context = context;
        }
        public async Task<List<NhanSu_DM_ChangeTypeModel>> GetAllByVM(NhanSu_DM_ChangeTypeModel dataModel, string groupId)
        {
            try
            {
                using var context = _context.CreateDbContext();

                var baseQuery = context.NhanSu_DM_ChangeTypes
                    .AsNoTracking()
                ...
                var data = baseData.Select(p1 => new NhanSu_DM_ChangeTypeModel
                {
                    Id = p1.Id,
                    ChangeTypeName = p1.ChangeTypeName,
                    IsSalaryImpact = p1.IsSalaryImpact,
                    IsPositionImpact = p1.IsPositionImpact,
                    GroupId = p1.GroupId,
        ...
        public async Task<List<NhanSu_DM_ChangeTypeModel>> GetHistoryIsValidEdit(string id)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var query = (from p1 in context.NhanSu_DM_ChangeType_Logs
                             ...
                             orderby p1.CreateAt
                             select new NhanSu_DM_ChangeTypeModel
                             {
                                 Id = p1.Id,
                                 ChangeTypeName = p1.ChangeTypeName,
                                 IsSalaryImpact = p1.IsSalaryImpact,
        ...
        public async Task<NhanSu_DM_ChangeTypeModel> GetDetails(string id)
        {
            using var context = _context.CreateDbContext();
            var data = await (from p1 in context.NhanSu_DM_ChangeTypes
                              join createBy in context.ApplicationUsers on p1.CreateBy equals createBy.Id
                              join a in context.ApplicationUsers on p1.ApprovalUserId equals a.Id into a1
        ...
        public async Task<List<NhanSu_DM_ChangeTypeModel>> GetHistory(string id)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var data = await (from p1 in context.NhanSu_DM_ChangeType_Logs
                                  join createBy in context.ApplicationUsers on p1.CreateBy equals createBy.Id
                                  join a in context.ApplicationUsers on p1.ApprovalUserId equals a.Id into a1
                                  from approvalUserId in a1.DefaultIfEmpty()
                                  join b in context.Departments on p1.DepartmentId equals b.Id into b1
        ...
        public async Task<List<NhanSu_DM_ChangeType>> GetAll(string groupId)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var entity = await context.NhanSu_DM_ChangeTypes.Where(p => p.IsActive != 100).OrderByDescending(p => p.CreateAt).ToListAsync();
                return entity;
            }
            catch (Exception ex)
            {
        ...
        public async Task<NhanSu_DM_ChangeType> GetById(string id)
        {
            using var context = _context.CreateDbContext();
            var entity = await context.NhanSu_DM_ChangeTypes.Where(x => x.Id.Equals(id) && x.IsActive != 100).FirstOrDefaultAsync();
            if (entity == null)
            {
        ...
        public async Task Insert(NhanSu_DM_ChangeType entity, string userId)
        {
            try
            {
                using var context = _context.CreateDbContext();
                ...
                context.NhanSu_DM_ChangeTypes.Add(entity);
                var addLog = new NhanSu_DM_ChangeType_Log()
                {
                    ChangeTypeName = entity.ChangeTypeName,
                    IsSalaryImpact = entity.IsSalaryImpact,
                    IsPositionImpact = entity.IsPositionImpact,
                ...
                context.NhanSu_DM_ChangeType_Logs.Add(addLog);
        ...
        public async Task Update(NhanSu_DM_ChangeType data, string userId)
        {
            using var context = _context.CreateDbContext();
            ...
            context.NhanSu_DM_ChangeTypes.Update(data);
            if (data.IsActive == 3)
            {
                var updateLog = await (from p in context.NhanSu_DM_ChangeType_Logs
                                       where p.IdChung == entity.Id && p.IsValid == true
                                       select p).ToListAsync();
                updateLog.ForEach(p => p.IsValid = false);
                context.NhanSu_DM_ChangeType_Logs.UpdateRange(updateLog);
            }
            ...
            else if (entity.IsActive != 3)
            {
                ...
                if (updateLog != null)
                {
                    ...
                    context.NhanSu_DM_ChangeType_Logs.Update(updateLog);
                }
            }
            var addLog = new NhanSu_DM_ChangeType_Log
            {
                ChangeTypeName = data.ChangeTypeName,
                IsSalaryImpact = data.IsSalaryImpact,
                IsPositionImpact = data.IsPositionImpact,
        ...
        public async Task UpdateMulti(NhanSu_DM_ChangeType[] NhanSu_DM_ChangeTypes)
        {
            using var context = _context.CreateDbContext();
            string[] ids = NhanSu_DM_ChangeTypes.Select(x => x.Id).ToArray();
            var listEntities = await context.NhanSu_DM_ChangeTypes.Where(x => ids.Contains(x.Id)).ToListAsync();
            foreach (var entity in listEntities)
            {
                context.NhanSu_DM_ChangeTypes.Update(entity);
            }
            await context.SaveChangesAsync();
        }
        public async Task Delete(NhanSu_DM_ChangeType data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            if (entity == null)
            ...
            else
            {
                ...
                {
                    var logdata = (from p in context.NhanSu_DM_ChangeType_Logs
                                   where p.IdChung == entity.Id && p.IsValid == true
                                   orderby p.CreateAt ascending
                                   select p).Take(1).FirstOrDefault();
                    if (logdata != null)
                    {
                        data.ChangeTypeName = logdata.ChangeTypeName;
                        data.IsSalaryImpact = logdata.IsSalaryImpact;
                        data.IsPositionImpact = logdata.IsPositionImpact;
                        ...
                        context.NhanSu_DM_ChangeType_Logs.Update(logdata);
                    }
                    var logupdate = (from p in context.NhanSu_DM_ChangeType_Logs
                                     where p.IdChung == entity.Id && p.IsValid == true
                                     select p).ToList();
                    if (logupdate != null)
                    {
                        logupdate.ForEach(p => p.IsValid = false);
                        context.NhanSu_DM_ChangeType_Logs.UpdateRange(logupdate);
                    }
                    var addLog = new NhanSu_DM_ChangeType_Log()
                    {
                        ChangeTypeName = data.ChangeTypeName,
        ...
        public async Task Approval(NhanSu_DM_ChangeType data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            if (entity == null)
        ...
        public async Task NoApproval(NhanSu_DM_ChangeType data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            ...
            else
            {
                ...
                else if (entity.IsActive == 1)
                {
                    ...
                    if (logdata != null)
                    {
                        entity.ChangeTypeName = logdata.ChangeTypeName;
                        entity.IsSalaryImpact = logdata.IsSalaryImpact;
                        entity.IsPositionImpact = logdata.IsPositionImpact;
        ...
        public async Task DeleteById(string id, string userId)
        {
            ...
            context.Set<NhanSu_DM_ChangeType>().Remove(entity);
            await context.SaveChangesAsync();
        }
        public async Task<bool> CheckStatus(string ids, string name)
        {
            using var context = _context.CreateDbContext();
            var model = await context.NhanSu_DM_ChangeTypes.Where(p => p.Id == ids && p.IsActive != 100).FirstOrDefaultAsync();
            if (model == null)
            {
                throw new Exception($"Không tìm thấy dữ liệu đã chonj!");
            }
        ...
        public async Task<bool> CheckSave(NhanSu_DM_ChangeType input)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var model = await (from p in context.NhanSu_DM_ChangeType_Logs
                                   where p.GroupId == input.GroupId && p.IdChung != input.Id && p.IsValid == true && p.IsActive != 100
                            && p.ChangeTypeName == input.ChangeTypeName
                            && p.IsSalaryImpact == input.IsSalaryImpact
                            && p.IsPositionImpact == input.IsPositionImpact
        ...
        public async Task<bool> CheckEdit(NhanSu_DM_ChangeType input)
        {
            try
            {
                using var context = _context.CreateDbContext();
        ...
        public async Task<bool> CheckDelete(NhanSu_DM_ChangeType input)
        {
            try
            {
                using var context = _context.CreateDbContext();
