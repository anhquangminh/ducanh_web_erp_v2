...
{
    public class NhanSu_EmployeeLeaveQuotaRepository : INhanSu_EmployeeLeaveQuotaRepository
    {
        private readonly IDbContextFactory<ApplicationDbContext> _context;
        public NhanSu_EmployeeLeaveQuotaRepository(IDbContextFactory<ApplicationDbContext> context)
        {
            _context = context;
        }
        public async Task<List<NhanSu_EmployeeLeaveQuotaModel>> GetAllByVM(NhanSu_EmployeeLeaveQuotaModel dataModel, string groupId)
        {
            try
            {
                using var context = _context.CreateDbContext();

                var baseQuery = context.NhanSu_EmployeeLeaveQuotas
                    .AsNoTracking()
                ...
                var data = baseData.Select(p1 => new NhanSu_EmployeeLeaveQuotaModel
                {
                    Id = p1.Id,
                    ...
                    Year = p1.Year,
                    TotalAnnualLeave = p1.TotalAnnualLeave,
                    CarriedOverLeave = p1.CarriedOverLeave,
                    UsedLeave = p1.UsedLeave,
                    RemainingLeave = p1.RemainingLeave,
                    OtherLeaveBalance = p1.OtherLeaveBalance,
                    GroupId = p1.GroupId,
        ...
        public async Task<List<NhanSu_EmployeeLeaveQuotaModel>> GetHistoryIsValidEdit(string id)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var query = (from p1 in context.NhanSu_EmployeeLeaveQuota_Logs
                             ...
                             orderby p1.CreateAt
                             select new NhanSu_EmployeeLeaveQuotaModel
                             {
                                 Id = p1.Id,
                                 EmployeeId = p1.EmployeeId,
        ...
        public async Task<NhanSu_EmployeeLeaveQuotaModel> GetDetails(string id)
        {
            using var context = _context.CreateDbContext();
            var data = await (from p1 in context.NhanSu_EmployeeLeaveQuotas
        ...
        public async Task<List<NhanSu_EmployeeLeaveQuotaModel>> GetHistory(string id)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var data = await (from p1 in context.NhanSu_EmployeeLeaveQuota_Logs
                                  join NhanSu_EmployeeProfiles1 in context.NhanSu_EmployeeProfiles on p1.EmployeeId equals NhanSu_EmployeeProfiles1.Id into NhanSu_EmployeeProfiles11
                                  from NhanSu_EmployeeProfiles1 in NhanSu_EmployeeProfiles11.DefaultIfEmpty()
                                  join createBy in context.ApplicationUsers on p1.CreateBy equals createBy.Id
                                  join a in context.ApplicationUsers on p1.ApprovalUserId equals a.Id into a1
        ...
        public async Task<List<NhanSu_EmployeeLeaveQuota>> GetAll(string groupId)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var entity = await context.NhanSu_EmployeeLeaveQuotas.Where(p => p.IsActive != 100).OrderByDescending(p => p.CreateAt).ToListAsync();
                return entity;
            }
            catch (Exception ex)
            {
        ...
        public async Task<NhanSu_EmployeeLeaveQuota> GetById(string id)
        {
            using var context = _context.CreateDbContext();
            var entity = await context.NhanSu_EmployeeLeaveQuotas.Where(x => x.Id.Equals(id) && x.IsActive != 100).FirstOrDefaultAsync();
            if (entity == null)
            {
        ...
        public async Task Insert(NhanSu_EmployeeLeaveQuota entity, string userId)
        {
            try
            {
                using var context = _context.CreateDbContext();
                ...
                context.NhanSu_EmployeeLeaveQuotas.Add(entity);
                var addLog = new NhanSu_EmployeeLeaveQuota_Log()
                {
                    EmployeeId = entity.EmployeeId,
                    Year = entity.Year,
                    TotalAnnualLeave = entity.TotalAnnualLeave,
                    CarriedOverLeave = entity.CarriedOverLeave,
                    UsedLeave = entity.UsedLeave,
                    RemainingLeave = entity.RemainingLeave,
                    OtherLeaveBalance = entity.OtherLeaveBalance,
                ...
                context.NhanSu_EmployeeLeaveQuota_Logs.Add(addLog);
        ...
        public async Task Update(NhanSu_EmployeeLeaveQuota data, string userId)
        {
            using var context = _context.CreateDbContext();
            ...
            context.NhanSu_EmployeeLeaveQuotas.Update(data);
            if (data.IsActive == 3)
            {
                var updateLog = await (from p in context.NhanSu_EmployeeLeaveQuota_Logs
                                       where p.IdChung == entity.Id && p.IsValid == true
                                       select p).ToListAsync();
                updateLog.ForEach(p => p.IsValid = false);
                context.NhanSu_EmployeeLeaveQuota_Logs.UpdateRange(updateLog);
            }
            ...
            else if (entity.IsActive != 3)
            {
                ...
                if (updateLog != null)
                {
                    ...
                    context.NhanSu_EmployeeLeaveQuota_Logs.Update(updateLog);
                }
            }
            var addLog = new NhanSu_EmployeeLeaveQuota_Log
            {
                ...
                CarriedOverLeave = data.CarriedOverLeave,
                UsedLeave = data.UsedLeave,
                RemainingLeave = data.RemainingLeave,
                OtherLeaveBalance = data.OtherLeaveBalance,
        ...
        public async Task UpdateMulti(NhanSu_EmployeeLeaveQuota[] NhanSu_EmployeeLeaveQuotas)
        {
            using var context = _context.CreateDbContext();
            string[] ids = NhanSu_EmployeeLeaveQuotas.Select(x => x.Id).ToArray();
            var listEntities = await context.NhanSu_EmployeeLeaveQuotas.Where(x => ids.Contains(x.Id)).ToListAsync();
            foreach (var entity in listEntities)
            {
                context.NhanSu_EmployeeLeaveQuotas.Update(entity);
            }
            await context.SaveChangesAsync();
        }
        public async Task Delete(NhanSu_EmployeeLeaveQuota data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            if (entity == null)
            ...
            else
            {
                ...
                {
                    var logdata = (from p in context.NhanSu_EmployeeLeaveQuota_Logs
                                   where p.IdChung == entity.Id && p.IsValid == true
                    ...
                    if (logdata != null)
                    {
                        ...
                        data.Year = logdata.Year;
                        data.TotalAnnualLeave = logdata.TotalAnnualLeave;
                        data.CarriedOverLeave = logdata.CarriedOverLeave;
                        data.UsedLeave = logdata.UsedLeave;
                        data.RemainingLeave = logdata.RemainingLeave;
                        data.OtherLeaveBalance = logdata.OtherLeaveBalance;
                        ...
                        context.NhanSu_EmployeeLeaveQuota_Logs.Update(logdata);
                    }
                    var logupdate = (from p in context.NhanSu_EmployeeLeaveQuota_Logs
                                     where p.IdChung == entity.Id && p.IsValid == true
                                     select p).ToList();
                    if (logupdate != null)
                    {
                        logupdate.ForEach(p => p.IsValid = false);
                        context.NhanSu_EmployeeLeaveQuota_Logs.UpdateRange(logupdate);
                    }
                    var addLog = new NhanSu_EmployeeLeaveQuota_Log()
                    {
                        EmployeeId = data.EmployeeId,
        ...
        public async Task Approval(NhanSu_EmployeeLeaveQuota data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            if (entity == null)
        ...
        public async Task NoApproval(NhanSu_EmployeeLeaveQuota data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            ...
            else
            {
                ...
                else if (entity.IsActive == 1)
                {
                    ...
                    if (logdata != null)
                    {
                        ...
                        entity.Year = logdata.Year;
                        entity.TotalAnnualLeave = logdata.TotalAnnualLeave;
                        entity.CarriedOverLeave = logdata.CarriedOverLeave;
                        entity.UsedLeave = logdata.UsedLeave;
                        entity.RemainingLeave = logdata.RemainingLeave;
                        entity.OtherLeaveBalance = logdata.OtherLeaveBalance;
        ...
        public async Task DeleteById(string id, string userId)
        {
            ...
            context.Set<NhanSu_EmployeeLeaveQuota>().Remove(entity);
            await context.SaveChangesAsync();
        }
        public async Task<bool> CheckStatus(string ids, string name)
        {
            using var context = _context.CreateDbContext();
            var model = await context.NhanSu_EmployeeLeaveQuotas.Where(p => p.Id == ids && p.IsActive != 100).FirstOrDefaultAsync();
            if (model == null)
            {
                throw new Exception($"Không tìm thấy dữ liệu đã chonj!");
            }
        ...
        public async Task<bool> CheckSave(NhanSu_EmployeeLeaveQuota input)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var model = await (from p in context.NhanSu_EmployeeLeaveQuota_Logs
                                   where p.GroupId == input.GroupId && p.IdChung != input.Id && p.IsValid == true && p.IsActive != 100
                            && p.EmployeeId == input.EmployeeId
                            && p.Year == input.Year
                            && p.TotalAnnualLeave == input.TotalAnnualLeave
                            && p.CarriedOverLeave == input.CarriedOverLeave
                            && p.UsedLeave == input.UsedLeave
                            && p.RemainingLeave == input.RemainingLeave
                            && p.OtherLeaveBalance == input.OtherLeaveBalance
        ...
        public async Task<bool> CheckEdit(NhanSu_EmployeeLeaveQuota input)
        {
            try
            {
                using var context = _context.CreateDbContext();
        ...
        public async Task<bool> CheckDelete(NhanSu_EmployeeLeaveQuota input)
        {
            try
            {
                using var context = _context.CreateDbContext();
