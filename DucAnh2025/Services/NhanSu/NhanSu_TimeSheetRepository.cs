...
{
    public class NhanSu_TimeSheetRepository : INhanSu_TimeSheetRepository
    {
        private readonly IDbContextFactory<ApplicationDbContext> _context;
        public NhanSu_TimeSheetRepository(IDbContextFactory<ApplicationDbContext> context)
        {
            _context = context;
        }
        public async Task<List<NhanSu_TimeSheetModel>> GetAllByVM(NhanSu_TimeSheetModel dataModel, string groupId)
        {
            try
            {
                using var context = _context.CreateDbContext();

                var baseQuery = context.NhanSu_TimeSheets
                    .AsNoTracking()
                ...
                if (!string.IsNullOrEmpty(dataModel?.WorkTypeId))
                {
                    baseQuery = baseQuery.Where(m => m.WorkTypeId == dataModel.WorkTypeId);
                }
                ...
                var NhanSu_DM_WorkTypesDict = await context.NhanSu_DM_WorkTypes.AsNoTracking().ToDictionaryAsync(x => x.Id);

                var data = baseData.Select(p1 => new NhanSu_TimeSheetModel
                {
                    Id = p1.Id,
                    ...
                    TimeSheetDate = p1.TimeSheetDate,
                    CheckInTime = p1.CheckInTime.HasValue ? (DateTime)p1.CheckInTime : DateTime.MinValue,
                    CheckOutTime = p1.CheckOutTime.HasValue ? (DateTime)p1.CheckOutTime : DateTime.MinValue,
                    WorkTypeId = NhanSu_DM_WorkTypesDict.TryGetValue(p1.WorkTypeId, out var tmpNhanSu_DM_WorkTypes) ? tmpNhanSu_DM_WorkTypes.WorkTypeName : "",
                    TotalHours = p1.TotalHours,
                    TotalDays = p1.TotalDays,
                    Source = p1.Source,
                    GroupId = p1.GroupId,
        ...
        public async Task<List<NhanSu_TimeSheetModel>> GetHistoryIsValidEdit(string id)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var query = (from p1 in context.NhanSu_TimeSheet_Logs
                             join NhanSu_EmployeeProfiles1 in context.NhanSu_EmployeeProfiles on p1.EmployeeId equals NhanSu_EmployeeProfiles1.Id
                             join NhanSu_DM_WorkTypes1 in context.NhanSu_DM_WorkTypes on p1.WorkTypeId equals NhanSu_DM_WorkTypes1.Id
                             ...
                             orderby p1.CreateAt
                             select new NhanSu_TimeSheetModel
                             {
                                 Id = p1.Id,
                                 EmployeeId = p1.EmployeeId,
                                 ...
                                 TimeSheetDate = p1.TimeSheetDate,
                                 CheckInTime = p1.CheckInTime ?? DateTime.MinValue,
                                 CheckOutTime = p1.CheckOutTime ?? DateTime.MinValue,
                                 WorkTypeId = NhanSu_DM_WorkTypes1.WorkTypeName,
                                 TotalHours = p1.TotalHours,
                                 TotalDays = p1.TotalDays,
                                 Source = p1.Source,
                                 GroupId = p1.GroupId,
        ...
        public async Task<NhanSu_TimeSheetModel> GetDetails(string id)
        {
            using var context = _context.CreateDbContext();
            var data = await (from p1 in context.NhanSu_TimeSheets
                              ...
                              join NhanSu_DM_WorkTypes1 in context.NhanSu_DM_WorkTypes on p1.WorkTypeId equals NhanSu_DM_WorkTypes1.Id into NhanSu_DM_WorkTypes11
                              from NhanSu_DM_WorkTypes1 in NhanSu_DM_WorkTypes11.DefaultIfEmpty()
                              join createBy in context.ApplicationUsers on p1.CreateBy equals createBy.Id
                              join a in context.ApplicationUsers on p1.ApprovalUserId equals a.Id into a1
                              ...
                              select new NhanSu_TimeSheetModel
                              {
                                  ...
                                  FullName = NhanSu_EmployeeProfiles1.FullName,
                                  TimeSheetDate = p1.TimeSheetDate,
                                  CheckInTime = (DateTime)p1.CheckInTime,
                                  CheckOutTime = (DateTime)p1.CheckOutTime,
                                  WorkTypeId = NhanSu_DM_WorkTypes1.WorkTypeName,
                                  TotalHours = p1.TotalHours,
                                  TotalDays = p1.TotalDays,
                                  Source = p1.Source,
        ...
        public async Task<List<NhanSu_TimeSheetModel>> GetHistory(string id)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var data = await (from p1 in context.NhanSu_TimeSheet_Logs
                                  join NhanSu_EmployeeProfiles1 in context.NhanSu_EmployeeProfiles on p1.EmployeeId equals NhanSu_EmployeeProfiles1.Id into NhanSu_EmployeeProfiles11
                                  from NhanSu_EmployeeProfiles1 in NhanSu_EmployeeProfiles11.DefaultIfEmpty()
                                  join NhanSu_DM_WorkTypes1 in context.NhanSu_DM_WorkTypes on p1.WorkTypeId equals NhanSu_DM_WorkTypes1.Id into NhanSu_DM_WorkTypes11
                                  from NhanSu_DM_WorkTypes1 in NhanSu_DM_WorkTypes11.DefaultIfEmpty()
        ...
        public async Task<List<NhanSu_TimeSheet>> GetAll(string groupId)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var entity = await context.NhanSu_TimeSheets.Where(p => p.IsActive != 100).OrderByDescending(p => p.CreateAt).ToListAsync();
                return entity;
            }
            catch (Exception ex)
            {
        ...
        public async Task<NhanSu_TimeSheet> GetById(string id)
        {
            using var context = _context.CreateDbContext();
            var entity = await context.NhanSu_TimeSheets.Where(x => x.Id.Equals(id) && x.IsActive != 100).FirstOrDefaultAsync();
            if (entity == null)
            {
        ...
        public async Task Insert(NhanSu_TimeSheet entity, string userId)
        {
            try
            {
                using var context = _context.CreateDbContext();
                ...
                context.NhanSu_TimeSheets.Add(entity);
                var addLog = new NhanSu_TimeSheet_Log()
                {
                ...
                context.NhanSu_TimeSheet_Logs.Add(addLog);
        ...
        public async Task Update(NhanSu_TimeSheet data, string userId)
        {
            using var context = _context.CreateDbContext();
            ...
            context.NhanSu_TimeSheets.Update(data);
            if (data.IsActive == 3)
            {
                var updateLog = await (from p in context.NhanSu_TimeSheet_Logs
                                       where p.IdChung == entity.Id && p.IsValid == true
                                       select p).ToListAsync();
                updateLog.ForEach(p => p.IsValid = false);
                context.NhanSu_TimeSheet_Logs.UpdateRange(updateLog);
            }
            ...
            else if (entity.IsActive != 3)
            {
                ...
                if (updateLog != null)
                {
                    ...
                    context.NhanSu_TimeSheet_Logs.Update(updateLog);
                }
            }
            var addLog = new NhanSu_TimeSheet_Log
            {
        ...
        public async Task UpdateMulti(NhanSu_TimeSheet[] NhanSu_TimeSheets)
        {
            using var context = _context.CreateDbContext();
            string[] ids = NhanSu_TimeSheets.Select(x => x.Id).ToArray();
            var listEntities = await context.NhanSu_TimeSheets.Where(x => ids.Contains(x.Id)).ToListAsync();
            foreach (var entity in listEntities)
            {
                context.NhanSu_TimeSheets.Update(entity);
            }
            await context.SaveChangesAsync();
        }
        public async Task Delete(NhanSu_TimeSheet data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            if (entity == null)
            ...
            else
            {
                ...
                {
                    var logdata = (from p in context.NhanSu_TimeSheet_Logs
                                   where p.IdChung == entity.Id && p.IsValid == true
                    ...
                    if (logdata != null)
                    {
                        ...
                        data.TimeSheetDate = logdata.TimeSheetDate;
                        data.CheckInTime = logdata.CheckInTime;
                        data.CheckOutTime = logdata.CheckOutTime;
                        data.WorkTypeId = logdata.WorkTypeId;
                        data.TotalHours = logdata.TotalHours;
                        data.TotalDays = logdata.TotalDays;
                        ...
                        context.NhanSu_TimeSheet_Logs.Update(logdata);
                    }
                    var logupdate = (from p in context.NhanSu_TimeSheet_Logs
                                     where p.IdChung == entity.Id && p.IsValid == true
                                     select p).ToList();
                    if (logupdate != null)
                    {
                        logupdate.ForEach(p => p.IsValid = false);
                        context.NhanSu_TimeSheet_Logs.UpdateRange(logupdate);
                    }
                    var addLog = new NhanSu_TimeSheet_Log()
                    {
                        EmployeeId = data.EmployeeId,
        ...
        public async Task Approval(NhanSu_TimeSheet data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            if (entity == null)
        ...
        public async Task NoApproval(NhanSu_TimeSheet data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            ...
            else
            {
                ...
                else if (entity.IsActive == 1)
                {
                    ...
                    if (logdata != null)
                    {
                        ...
                        entity.TimeSheetDate = logdata.TimeSheetDate;
                        entity.CheckInTime = logdata.CheckInTime;
                        entity.CheckOutTime = logdata.CheckOutTime;
                        entity.WorkTypeId = logdata.WorkTypeId;
                        entity.TotalHours = logdata.TotalHours;
                        entity.TotalDays = logdata.TotalDays;
                        entity.Source = logdata.Source;
        ...
        public async Task DeleteById(string id, string userId)
        {
            ...
            context.Set<NhanSu_TimeSheet>().Remove(entity);
            await context.SaveChangesAsync();
        }
        public async Task<bool> CheckStatus(string ids, string name)
        {
            using var context = _context.CreateDbContext();
            var model = await context.NhanSu_TimeSheets.Where(p => p.Id == ids && p.IsActive != 100).FirstOrDefaultAsync();
            if (model == null)
            {
                throw new Exception($"Không tìm thấy dữ liệu đã chonj!");
            }
        ...
        public async Task<bool> CheckSave(NhanSu_TimeSheet input)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var model = await (from p in context.NhanSu_TimeSheet_Logs
                                   where p.GroupId == input.GroupId && p.IdChung != input.Id && p.IsValid == true && p.IsActive != 100
                            && p.EmployeeId == input.EmployeeId
                            && p.TimeSheetDate == input.TimeSheetDate
                            && p.CheckInTime == input.CheckInTime
                            && p.CheckOutTime == input.CheckOutTime
                            && p.WorkTypeId == input.WorkTypeId
                            && p.TotalHours == input.TotalHours
                            && p.TotalDays == input.TotalDays
        ...
        public async Task<bool> CheckEdit(NhanSu_TimeSheet input)
        {
            try
            {
                using var context = _context.CreateDbContext();
        ...
        public async Task<bool> CheckDelete(NhanSu_TimeSheet input)
        {
            try
            {
                using var context = _context.CreateDbContext();
