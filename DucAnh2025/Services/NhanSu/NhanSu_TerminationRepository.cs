...
{
    public class NhanSu_TerminationRepository : INhanSu_TerminationRepository
    {
        private readonly IDbContextFactory<ApplicationDbContext> _context;
        public NhanSu_TerminationRepository(IDbContextFactory<ApplicationDbContext> context)
        {
            _context = context;
        }
        public async Task<List<NhanSu_TerminationModel>> GetAllByVM(NhanSu_TerminationModel dataModel, string groupId)
        {
            try
            {
                using var context = _context.CreateDbContext();

                var baseQuery = context.NhanSu_Terminations
                    .AsNoTracking()
                ...
                if (!string.IsNullOrEmpty(dataModel?.ReasonId))
                {
                    baseQuery = baseQuery.Where(m => m.ReasonId == dataModel.ReasonId);
                }
                ...
                var NhanSu_DM_TerminationReasonsDict = await context.NhanSu_DM_TerminationReasons.AsNoTracking().ToDictionaryAsync(x => x.Id);

                var data = baseData.Select(p1 => new NhanSu_TerminationModel
                {
                    Id = p1.Id,
                    EmployeeId = NhanSu_EmployeeProfilesDict.TryGetValue(p1.EmployeeId, out var tmpNhanSu_EmployeeProfiles) ? tmpNhanSu_EmployeeProfiles.FullName : "",
                    ...
                    FullName = NhanSu_EmployeeProfilesDict.TryGetValue(p1.EmployeeId, out var tmpNhanSu_EmployeeProfiles2) ? tmpNhanSu_EmployeeProfiles2.FullName : "",
                    ApplicationDate = p1.ApplicationDate,
                    DesiredLeaveDate = p1.DesiredLeaveDate,
                    OfficialLeaveDate = p1.OfficialLeaveDate,
                    ReasonId = NhanSu_DM_TerminationReasonsDict.TryGetValue(p1.ReasonId, out var tmpNhanSu_DM_TerminationReasons) ? tmpNhanSu_DM_TerminationReasons.ReasonName : "",
                    DecisionCode = p1.DecisionCode,
                    FilePath = p1.FilePath,
                    ClearanceDate = p1.ClearanceDate,
                    HRNotes = p1.HRNotes,
                    GroupId = p1.GroupId,
        ...
        public async Task<List<NhanSu_TerminationModel>> GetHistoryIsValidEdit(string id)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var query = (from p1 in context.NhanSu_Termination_Logs
                             join NhanSu_EmployeeProfiles1 in context.NhanSu_EmployeeProfiles on p1.EmployeeId equals NhanSu_EmployeeProfiles1.Id
                             join NhanSu_DM_TerminationReasons1 in context.NhanSu_DM_TerminationReasons on p1.ReasonId equals NhanSu_DM_TerminationReasons1.Id
                             ...
                             orderby p1.CreateAt
                             select new NhanSu_TerminationModel
                             {
                                 Id = p1.Id,
                                 ...
                                 DesiredLeaveDate = p1.DesiredLeaveDate,
                                 OfficialLeaveDate = p1.OfficialLeaveDate,
                                 ReasonId = NhanSu_DM_TerminationReasons1.ReasonName,
                                 DecisionCode = p1.DecisionCode,
                                 FilePath = p1.FilePath,
                                 ClearanceDate = p1.ClearanceDate,
                                 HRNotes = p1.HRNotes,
        ...
        public async Task<NhanSu_TerminationModel> GetDetails(string id)
        {
            using var context = _context.CreateDbContext();
            var data = await (from p1 in context.NhanSu_Terminations
                              ...
                              join NhanSu_DM_TerminationReasons1 in context.NhanSu_DM_TerminationReasons on p1.ReasonId equals NhanSu_DM_TerminationReasons1.Id into NhanSu_DM_TerminationReasons11
                              from NhanSu_DM_TerminationReasons1 in NhanSu_DM_TerminationReasons11.DefaultIfEmpty()
                              join createBy in context.ApplicationUsers on p1.CreateBy equals createBy.Id
                              join a in context.ApplicationUsers on p1.ApprovalUserId equals a.Id into a1
        ...
        public async Task<List<NhanSu_TerminationModel>> GetHistory(string id)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var data = await (from p1 in context.NhanSu_Termination_Logs
                                  join NhanSu_EmployeeProfiles1 in context.NhanSu_EmployeeProfiles on p1.EmployeeId equals NhanSu_EmployeeProfiles1.Id into NhanSu_EmployeeProfiles11
                                  from NhanSu_EmployeeProfiles1 in NhanSu_EmployeeProfiles11.DefaultIfEmpty()
                                  join NhanSu_DM_TerminationReasons1 in context.NhanSu_DM_TerminationReasons on p1.ReasonId equals NhanSu_DM_TerminationReasons1.Id into NhanSu_DM_TerminationReasons11
                                  from NhanSu_DM_TerminationReasons1 in NhanSu_DM_TerminationReasons11.DefaultIfEmpty()
        ...
        public async Task<List<NhanSu_Termination>> GetAll(string groupId)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var entity = await context.NhanSu_Terminations.Where(p => p.IsActive != 100).OrderByDescending(p => p.CreateAt).ToListAsync();
                return entity;
            }
            catch (Exception ex)
            {
        ...
        public async Task<NhanSu_Termination> GetById(string id)
        {
            using var context = _context.CreateDbContext();
            var entity = await context.NhanSu_Terminations.Where(x => x.Id.Equals(id) && x.IsActive != 100).FirstOrDefaultAsync();
            if (entity == null)
            {
        ...
        public async Task Insert(NhanSu_Termination entity, string userId)
        {
            try
            {
                using var context = _context.CreateDbContext();
                ...
                context.NhanSu_Terminations.Add(entity);
                var addLog = new NhanSu_Termination_Log()
                {
                    EmployeeId = entity.EmployeeId,
                    ApplicationDate = entity.ApplicationDate,
                    DesiredLeaveDate = entity.DesiredLeaveDate,
                    OfficialLeaveDate = entity.OfficialLeaveDate,
                    ReasonId = entity.ReasonId,
                    DecisionCode = entity.DecisionCode,
                    FilePath = entity.FilePath,
                    ClearanceDate = entity.ClearanceDate,
                ...
                context.NhanSu_Termination_Logs.Add(addLog);
        ...
        public async Task Update(NhanSu_Termination data, string userId)
        {
            using var context = _context.CreateDbContext();
            ...
            context.NhanSu_Terminations.Update(data);
            if (data.IsActive == 3)
            {
                var updateLog = await (from p in context.NhanSu_Termination_Logs
                                       where p.IdChung == entity.Id && p.IsValid == true
                                       select p).ToListAsync();
                updateLog.ForEach(p => p.IsValid = false);
                context.NhanSu_Termination_Logs.UpdateRange(updateLog);
            }
            ...
            else if (entity.IsActive != 3)
            {
                ...
                if (updateLog != null)
                {
                    ...
                    context.NhanSu_Termination_Logs.Update(updateLog);
                }
            }
            var addLog = new NhanSu_Termination_Log
            {
                ...
                DesiredLeaveDate = data.DesiredLeaveDate,
                OfficialLeaveDate = data.OfficialLeaveDate,
        ...
        public async Task UpdateMulti(NhanSu_Termination[] NhanSu_Terminations)
        {
            using var context = _context.CreateDbContext();
            string[] ids = NhanSu_Terminations.Select(x => x.Id).ToArray();
            var listEntities = await context.NhanSu_Terminations.Where(x => ids.Contains(x.Id)).ToListAsync();
            foreach (var entity in listEntities)
            {
                context.NhanSu_Terminations.Update(entity);
            }
            await context.SaveChangesAsync();
        }
        public async Task Delete(NhanSu_Termination data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            if (entity == null)
            ...
            else
            {
                ...
                {
                    var logdata = (from p in context.NhanSu_Termination_Logs
                                   where p.IdChung == entity.Id && p.IsValid == true
                    ...
                    if (logdata != null)
                    {
                        ...
                        data.ApplicationDate = logdata.ApplicationDate;
                        data.DesiredLeaveDate = logdata.DesiredLeaveDate;
                        data.OfficialLeaveDate = logdata.OfficialLeaveDate;
                        data.ReasonId = logdata.ReasonId;
                        data.DecisionCode = logdata.DecisionCode;
                        data.FilePath = logdata.FilePath;
                        data.ClearanceDate = logdata.ClearanceDate;
                        data.HRNotes = logdata.HRNotes;
                        ...
                        context.NhanSu_Termination_Logs.Update(logdata);
                    }
                    var logupdate = (from p in context.NhanSu_Termination_Logs
                                     where p.IdChung == entity.Id && p.IsValid == true
                                     select p).ToList();
                    if (logupdate != null)
                    {
                        logupdate.ForEach(p => p.IsValid = false);
                        context.NhanSu_Termination_Logs.UpdateRange(logupdate);
                    }
                    var addLog = new NhanSu_Termination_Log()
                    {
                        EmployeeId = data.EmployeeId,
        ...
        public async Task Approval(NhanSu_Termination data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            if (entity == null)
        ...
        public async Task NoApproval(NhanSu_Termination data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            ...
            else
            {
                ...
                else if (entity.IsActive == 1)
                {
                    ...
                    if (logdata != null)
                    {
                        ...
                        entity.ApplicationDate = logdata.ApplicationDate;
                        entity.DesiredLeaveDate = logdata.DesiredLeaveDate;
                        entity.OfficialLeaveDate = logdata.OfficialLeaveDate;
                        entity.ReasonId = logdata.ReasonId;
                        entity.DecisionCode = logdata.DecisionCode;
                        entity.FilePath = logdata.FilePath;
                        entity.ClearanceDate = logdata.ClearanceDate;
                        entity.HRNotes = logdata.HRNotes;
        ...
        public async Task DeleteById(string id, string userId)
        {
            ...
            context.Set<NhanSu_Termination>().Remove(entity);
            await context.SaveChangesAsync();
        }
        public async Task<bool> CheckStatus(string ids, string name)
        {
            using var context = _context.CreateDbContext();
            var model = await context.NhanSu_Terminations.Where(p => p.Id == ids && p.IsActive != 100).FirstOrDefaultAsync();
            if (model == null)
            {
                throw new Exception($"Không tìm thấy dữ liệu đã chonj!");
            }
        ...
        public async Task<bool> CheckSave(NhanSu_Termination input)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var model = await (from p in context.NhanSu_Termination_Logs
                                   where p.GroupId == input.GroupId && p.IdChung != input.Id && p.IsValid == true && p.IsActive != 100
                            && p.EmployeeId == input.EmployeeId
                            && p.ApplicationDate == input.ApplicationDate
                            && p.DesiredLeaveDate == input.DesiredLeaveDate
                            && p.OfficialLeaveDate == input.OfficialLeaveDate
                            && p.ReasonId == input.ReasonId
                            && p.DecisionCode == input.DecisionCode
                            && p.FilePath == input.FilePath
                            && p.ClearanceDate == input.ClearanceDate
                            && p.HRNotes == input.HRNotes
        ...
        public async Task<bool> CheckEdit(NhanSu_Termination input)
        {
            try
            {
                using var context = _context.CreateDbContext();
        ...
        public async Task<bool> CheckDelete(NhanSu_Termination input)
        {
            try
            {
                using var context = _context.CreateDbContext();
