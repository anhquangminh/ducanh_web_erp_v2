...
{
    public class DM_ChucVuRepository : IDM_ChucVuRepository
    {
        private readonly IDbContextFactory<ApplicationDbContext> _context;
        public DM_ChucVuRepository(IDbContextFactory<ApplicationDbContext> context)
        {
            _context = context;
        }
        public DM_ChucVuModel GetToEdit(string id)
        {
            using var context = _context.CreateDbContext();
            var query = (from p1 in context.DM_ChucVus
                         where p1.Id == id && p1.IsActive != 100
                         select new DM_ChucVuModel
                         {
                             Id = p1.Id,
                             ChucVu = p1.ChucVu,
                             GroupId = p1.GroupId,
        ...
        public async Task<List<DM_ChucVuModel>> GetAllByVM(DM_ChucVuModel dataModel, string groupId)
        {
            using var context = _context.CreateDbContext();
            var query = from p1 in context.DM_ChucVus
        ...
        public async Task<List<DM_ChucVuModel>> GetHistoryIsValidEdit(string id)
        {
            using var context = _context.CreateDbContext();
            var data = await (from p1 in context.DM_ChucVu_Logs
        ...
        public async Task<DM_ChucVuModel> GetDetails(string id)
        {
            using var context = _context.CreateDbContext();
            var data = await (from p1 in context.DM_ChucVus
        ...
        public async Task<List<DM_ChucVuModel>> GetHistory(string id)
        {
            using var context = _context.CreateDbContext();
            var data = await (from p1 in context.DM_ChucVu_Logs
                              join createBy in context.ApplicationUsers on p1.CreateBy equals createBy.Id
        ...
        public async Task<List<DM_ChucVu>> GetAll(string groupId)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var entity = await context.DM_ChucVus.Where(p => p.IsActive != 100 && p.GroupId == groupId).ToListAsync();
                return entity;
            }
            catch (Exception ex)
            {
        ...
        public async Task<DM_ChucVu> GetById(string id)
        {
            using var context = _context.CreateDbContext();
            var entity = await context.DM_ChucVus.Where(x => x.Id.Equals(id) && x.IsActive != 100).FirstOrDefaultAsync();
            if (entity == null)
            {
                throw new Exception($"Không tìm thấy chức vụ đã chọn.");
            }
            return entity;
        }
        public async Task Insert(DM_ChucVu entity, string userId)
        {
            try
            {
                using var context = _context.CreateDbContext();
                if (entity == null)
                {
                    throw new Exception("Không có chức vụ nào được thêm!");
                }
                context.DM_ChucVus.Add(entity);
                var addLog = new DM_ChucVu_Log()
                {
                    Id = entity.Id,
                ...
                context.DM_ChucVu_Logs.Add(addLog);
        ...
        public async Task Update(DM_ChucVu data, string userId)
        {
            using var context = _context.CreateDbContext();
            ...
            context.DM_ChucVus.Update(data);
            if (data.IsActive == 3)
            {
                var updateLog = await (from p in context.DM_ChucVu_Logs
                                       where p.IdChung == entity.Id && p.IsValid == true
                                       select p).ToListAsync();
                updateLog.ForEach(p => p.IsValid = false);
                context.DM_ChucVu_Logs.UpdateRange(updateLog);
            }
            ...
            else if (entity.IsActive != 3)
            {
                ...
                if (updateLog != null)
                {
                    ...
                    context.DM_ChucVu_Logs.Update(updateLog);
                }
            }
            var addLog = new DM_ChucVu_Log()
            {
        ...
        public async Task UpdateMulti(DM_ChucVu[] DM_ChucVus)
        {
            using var context = _context.CreateDbContext();
            string[] ids = DM_ChucVus.Select(x => x.Id).ToArray();
            var listEntities = await context.DM_ChucVus.Where(x => ids.Contains(x.Id)).ToListAsync();
            foreach (var entity in listEntities)
            {
                context.DM_ChucVus.Update(entity);
            }
            await context.SaveChangesAsync();
        }
        public async Task Delete(DM_ChucVu data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            if (entity == null)
            ...
            else
            {
                ...
                {
                    var logdata = (from p in context.DM_ChucVu_Logs
                                   where p.IdChung == entity.Id && p.IsValid == true
                                   orderby p.CreateAt ascending
                                   select p).Take(1).FirstOrDefault();
                    if (logdata != null)
                    {
                        data.ChucVu = logdata.ChucVu;
                        data.Ordinarily = logdata.Ordinarily;
                        data.ApprovalUserId = "";
                        data.DateApproval = null;
                        logdata.IsValid = true;
                        context.DM_ChucVu_Logs.Update(logdata);
                    }
                    var logupdate = (from p in context.DM_ChucVu_Logs
                                     where p.IdChung == entity.Id && p.IsValid == true
                                     select p).ToList();
                    if (logupdate != null)
                    {
                        logupdate.ForEach(p => p.IsValid = false);
                        context.DM_ChucVu_Logs.UpdateRange(logupdate);
                    }
                    var addLog = new DM_ChucVu_Log()
                    {
                        Id = Guid.NewGuid().ToString(),
        ...
        public async Task Approval(DM_ChucVu data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
            if (entity == null)
        ...
        public async Task NoApproval(DM_ChucVu data, string userId)
        {
            using var context = _context.CreateDbContext();
            var entity = await GetById(data.Id);
        ...
        public async Task DeleteById(string id, string userId)
        {
            ...
            context.Set<DM_ChucVu>().Remove(entity);
            await context.SaveChangesAsync();
        }
        public async Task<bool> CheckStatus(string ids, string name)
        {
            using var context = _context.CreateDbContext();
            var model = await context.DM_ChucVus.Where(p => p.Id == ids && p.IsActive != 100).FirstOrDefaultAsync();
            if (model == null)
            {
                throw new Exception($"Không tìm thấy chức vụ đã chọn");
            }
            ...
            {
                throw new Exception($"Chức vụ đang chờ duyệt thêm mới.");
            }
            if (model != null && model.IsActive == 1)
            {
                throw new Exception($"Chức vụ đang chờ duyệt sửa.");
            }
            if (model != null && model.IsActive == 2)
            {
                throw new Exception($"Chức vụ đang chờ duyệt xóa.");
            }
            //if (model != null && model.TenDM_ChucVu.ToUpper() != name.ToUpper() && name != "")
            //{
            //throw new Exception($"Tên chức vụ đã bị thay đổi.");
            //}
            return true;
        }
        public async Task<bool> CheckSave(DM_ChucVu input)
        {
            try
            {
                using var context = _context.CreateDbContext();
                var model = await (from p in context.DM_ChucVu_Logs
                                   where p.GroupId == input.GroupId && p.IdChung != input.Id && p.IsValid == true && p.IsActive != 100
                                   && p.ChucVu == input.ChucVu
                                   select p).CountAsync();
                if (model > 0)
                {
        ...
        public async Task<bool> CheckEdit(DM_ChucVu input)
        {
            try
            {
                using var context = _context.CreateDbContext();
        ...
        public async Task<bool> CheckDelete(DM_ChucVu input)
        {
            try
            {
                using var context = _context.CreateDbContext();
