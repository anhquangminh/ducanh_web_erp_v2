...
namespace DucAnh2025.Controllers.API.NhanSu
{
    ...
    [ApiController]
    public class NhanSuAppointmentsHistoryController : ControllerBase
    {
        private readonly IApprovalTaskRepository _approvalTaskService;
        private readonly INhanSu_AppointmentsHistoryRepository _nhansuappointmentshistoryService;
        ...
        public NhanSuAppointmentsHistoryController(IApprovalTaskRepository approvalTaskRepository,
            INhanSu_AppointmentsHistoryRepository nhansuappointmentshistoryRepository,
            ...
            IPermissionRepository permissionService)
        {
            ...
            _nhansuappointmentshistoryService = nhansuappointmentshistoryRepository;
        ...
        public string tableName = "NhanSu_AppointmentsHistorys";

        [HttpPost("GetByVM")]
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetByVM(string groupId, [FromBody] NhanSu_AppointmentsHistoryModel? input = null)
        {
            try
            {
                var model = input ?? new NhanSu_AppointmentsHistoryModel();
                var list = await _nhansuappointmentshistoryService.GetAllByVM(model, groupId);
        ...
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetAll(string groupId)
        {
            try
            {
                var list = await _nhansuappointmentshistoryService.GetAll(groupId);
                return Ok(new ApiResponse<IEnumerable<object>>(true, "Thành công", list));
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetById(string id)
        {
            try
            {
                var result = await _nhansuappointmentshistoryService.GetById(id);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetHistory(string id)
        {
            try
            {
                var result = await _nhansuappointmentshistoryService.GetHistory(id);
                return Ok(new ApiResponse<object>(true, "Lưu thành công .", result));
            }
            catch (Exception ex)
            {
        ...
        [HttpPost("Create")]
        public async Task<ActionResult<ApiResponse<object>>> Create([FromBody] NhanSu_AppointmentsHistory input, [FromQuery] bool isEdit)
        {
            try
            {
                ...
                if (editCheck)
                {
                    ...
                    if (!await _nhansuappointmentshistoryService.CheckExclusive(new[] { input.Id }, now))
                    ...
                    await _nhansuappointmentshistoryService.Update(input, "");
                    ...
                    {
                        await _approvalTaskService.Insert(CreateApprovalTask("Lịch sử Bổ nhiệm- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }
                    else
                    {
                        await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Lịch sử Bổ nhiệm- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }

                    await InsertEmailHistories(content, "Lịch sử Bổ nhiệm- sửa !", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
                ...
                else
                {
                    ...
                    input.DateApproval = null;
                    input.ApprovalDept = null;
                    await _nhansuappointmentshistoryService.Insert(input, "");
                    string content = await BuildEmailContentAdd(input.Id);
                    await _approvalTaskService.Insert(CreateApprovalTask("Lịch sử Bổ nhiệmThêm mơí !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    await InsertEmailHistories(content, "Lịch sử Bổ nhiệm - Thêm", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            try
            {
                ...
                var query = await _nhansuappointmentshistoryService.GetById(id);
                if (query == null)
                ...
                if (!await _nhansuappointmentshistoryService.CheckDelete(query))
                ...
                if (!await _nhansuappointmentshistoryService.CheckExclusive(new[] { id }, baseTime))
                ...
                await _nhansuappointmentshistoryService.Update(query, "");

                var content = await BuildEmailContentDelete(id);
                var dataApproval = await _approvalTaskService.GetByOriginalId(id);
                if (dataApproval == null)
                {
                    await _approvalTaskService.Insert(CreateApprovalTask("Lịch sử Bổ nhiệm - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }
                else
                {
                    await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Lịch sử Bổ nhiệm - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }

                await InsertEmailHistories(content, "Lịch sử Bổ nhiệm - xóa", query.GroupId, groupId, firstApproval.Id, query.Id, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> Duyet(string id)
        {
            try
            {
                var user = _applicationUserService.GetCurrentUser();
                var userId = user.Id;
                var entity = await _nhansuappointmentshistoryService.GetById(id);
                if (entity == null)
                ...
                if (entity.IsActive != 3)
                {
                    ...
                    await _nhansuappointmentshistoryService.Approval(entity, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> HuyDuyet(string id)
        {
            try
            {
                ...
                if (entity.IsActive != 3)
                {
                    await _nhansuappointmentshistoryService.NoApproval(entity, userId);
                    var dataNoApproval = await _nhansuappointmentshistoryService.GetById(entity.Id);
        ...
        {
            var isValidModel = await _nhansuappointmentshistoryService.GetHistoryIsValidEdit(id);
            var sb = new StringBuilder();

            sb.AppendLine("Thông tin sửa NhanSuAppointmentsHistory</h3>")
             .AppendLine("<table class=\"table table-hover table-bordered\">")
             .AppendLine("  <thead class=\"bg-info\">")
             .AppendLine("    <tr>")
             .AppendLine("     <th>Mã nhân viên</th>")
             .AppendLine("     <th>Loại Biến động</th>")
             .AppendLine("     <th>Mã Quyết định</th>")
             .AppendLine("     <th>Ngày Quyết định</th>")
             .AppendLine("     <th>Ngày Hiệu lực</th>")
             .AppendLine("     <th>Phòng ban (Cũ)</th>")
             .AppendLine("     <th>Chức danh (Cũ)</th>")
             .AppendLine("     <th>Phòng ban (Mới)</th>")
             .AppendLine("     <th>Chức danh (Mới)</th>")
             .AppendLine("     <th>Mức lương (Mới)</th>")
             .AppendLine("     <th>Nội dung thay đổi</th>")
             .AppendLine("     <th>Đường dẫn File QĐ</th>")
             .AppendLine("     <th>Người ký Quyết định</th>")
            ...
            if (isValidModel.Any())
            {
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{oldData?.ChangeTypeId}</td>")
                  .AppendLine($"      <td>{oldData?.DecisionCode}</td>")
                  .AppendLine($"      <td>{oldData?.DecisionDate}</td>")
                  .AppendLine($"      <td>{oldData?.EffectiveDate}</td>")
                  .AppendLine($"      <td>{oldData?.OldDepartmentId}</td>")
                  .AppendLine($"      <td>{oldData?.OldChucVuId}</td>")
                  .AppendLine($"      <td>{oldData?.NewDepartmentId}</td>")
                  .AppendLine($"      <td>{oldData?.NewChucVuId}</td>")
                  .AppendLine($"      <td>{oldData?.NewBasicSalary}</td>")
                  .AppendLine($"      <td>{oldData?.Content}</td>")
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{newData?.ChangeTypeId}</td>")
                  .AppendLine($"      <td>{newData?.DecisionCode}</td>")
                  .AppendLine($"      <td>{newData?.DecisionDate}</td>")
                  .AppendLine($"      <td>{newData?.EffectiveDate}</td>")
                  .AppendLine($"      <td>{newData?.OldDepartmentId}</td>")
                  .AppendLine($"      <td>{newData?.OldChucVuId}</td>")
                  .AppendLine($"      <td>{newData?.NewDepartmentId}</td>")
                  .AppendLine($"      <td>{newData?.NewChucVuId}</td>")
                  .AppendLine($"      <td>{newData?.NewBasicSalary}</td>")
                  .AppendLine($"      <td>{newData?.Content}</td>")
        ...
        {
            var detail = await _nhansuappointmentshistoryService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Thêm - NhanSuAppointmentsHistory</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Mã nhân viên</th>")
            ...
            sb.AppendLine($"    <tr>")
              ...
              .AppendLine($"     <td>{detail.ChangeTypeId}</td>")
              .AppendLine($"     <td>{detail.DecisionCode}</td>")
              .AppendLine($"     <td>{detail.DecisionDate}</td>")
              .AppendLine($"     <td>{detail.EffectiveDate}</td>")
              .AppendLine($"     <td>{detail.OldDepartmentId}</td>")
              .AppendLine($"     <td>{detail.OldChucVuId}</td>")
              .AppendLine($"     <td>{detail.NewDepartmentId}</td>")
              .AppendLine($"     <td>{detail.NewChucVuId}</td>")
              .AppendLine($"     <td>{detail.NewBasicSalary}</td>")
              .AppendLine($"     <td>{detail.Content}</td>")
        ...
        private async Task<string> BuildEmailContentDelete(string id)
        {
            var detail = await _nhansuappointmentshistoryService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Xóa - NhanSuAppointmentsHistory</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Mã nhân viên</th>")
        ...
        private ApprovalTask CreateApprovalTask(string title, string content, NhanSu_AppointmentsHistory input, string tableName, string parentMajorId, string majorId, string userId)
        {
        ...
        private ApprovalTask UpdateApprovalTask(string Id, string title, string content, NhanSu_AppointmentsHistory input, string tableName, string parentMajorId, string majorId, string userId)
        {
            return new ApprovalTask
            {
                Id = Id,
