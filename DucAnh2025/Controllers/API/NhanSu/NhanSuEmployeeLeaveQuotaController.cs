...
namespace DucAnh2025.Controllers.API.NhanSu
{
    ...
    [ApiController]
    public class NhanSuEmployeeLeaveQuotaController : ControllerBase
    {
        private readonly IApprovalTaskRepository _approvalTaskService;
        private readonly INhanSu_EmployeeLeaveQuotaRepository _nhansuemployeeleavequotaService;
        ...
        public NhanSuEmployeeLeaveQuotaController(IApprovalTaskRepository approvalTaskRepository,
            INhanSu_EmployeeLeaveQuotaRepository nhansuemployeeleavequotaRepository,
            ...
            IPermissionRepository permissionService)
        {
            ...
            _nhansuemployeeleavequotaService = nhansuemployeeleavequotaRepository;
        ...
        public string tableName = "NhanSu_EmployeeLeaveQuotas";

        [HttpPost("GetByVM")]
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetByVM(string groupId, [FromBody] NhanSu_EmployeeLeaveQuotaModel? input = null)
        {
            try
            {
                var model = input ?? new NhanSu_EmployeeLeaveQuotaModel();
                var list = await _nhansuemployeeleavequotaService.GetAllByVM(model, groupId);
        ...
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetAll(string groupId)
        {
            try
            {
                var list = await _nhansuemployeeleavequotaService.GetAll(groupId);
                return Ok(new ApiResponse<IEnumerable<object>>(true, "Thành công", list));
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetById(string id)
        {
            try
            {
                var result = await _nhansuemployeeleavequotaService.GetById(id);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetHistory(string id)
        {
            try
            {
                var result = await _nhansuemployeeleavequotaService.GetHistory(id);
                return Ok(new ApiResponse<object>(true, "Lưu thành công .", result));
            }
            catch (Exception ex)
            {
        ...
        [HttpPost("Create")]
        public async Task<ActionResult<ApiResponse<object>>> Create([FromBody] NhanSu_EmployeeLeaveQuota input, [FromQuery] bool isEdit)
        {
            try
            {
                ...
                if (editCheck)
                {
                    ...
                    if (!await _nhansuemployeeleavequotaService.CheckExclusive(new[] { input.Id }, now))
                    ...
                    await _nhansuemployeeleavequotaService.Update(input, "");
                    ...
                    {
                        await _approvalTaskService.Insert(CreateApprovalTask("Hạn mức Phép- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }
                    else
                    {
                        await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Hạn mức Phép- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }

                    await InsertEmailHistories(content, "Hạn mức Phép- sửa !", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
                ...
                else
                {
                    ...
                    input.DateApproval = null;
                    input.ApprovalDept = null;
                    await _nhansuemployeeleavequotaService.Insert(input, "");
                    string content = await BuildEmailContentAdd(input.Id);
                    await _approvalTaskService.Insert(CreateApprovalTask("Hạn mức PhépThêm mơí !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    await InsertEmailHistories(content, "Hạn mức Phép - Thêm", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            try
            {
                ...
                var query = await _nhansuemployeeleavequotaService.GetById(id);
                if (query == null)
                ...
                if (!await _nhansuemployeeleavequotaService.CheckDelete(query))
                ...
                if (!await _nhansuemployeeleavequotaService.CheckExclusive(new[] { id }, baseTime))
                ...
                await _nhansuemployeeleavequotaService.Update(query, "");

                var content = await BuildEmailContentDelete(id);
                var dataApproval = await _approvalTaskService.GetByOriginalId(id);
                if (dataApproval == null)
                {
                    await _approvalTaskService.Insert(CreateApprovalTask("Hạn mức Phép - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }
                else
                {
                    await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Hạn mức Phép - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }

                await InsertEmailHistories(content, "Hạn mức Phép - xóa", query.GroupId, groupId, firstApproval.Id, query.Id, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> Duyet(string id)
        {
            try
            {
                var user = _applicationUserService.GetCurrentUser();
                var userId = user.Id;
                var entity = await _nhansuemployeeleavequotaService.GetById(id);
                if (entity == null)
                ...
                if (entity.IsActive != 3)
                {
                    ...
                    await _nhansuemployeeleavequotaService.Approval(entity, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> HuyDuyet(string id)
        {
            try
            {
                ...
                if (entity.IsActive != 3)
                {
                    await _nhansuemployeeleavequotaService.NoApproval(entity, userId);
                    var dataNoApproval = await _nhansuemployeeleavequotaService.GetById(entity.Id);
        ...
        {
            var isValidModel = await _nhansuemployeeleavequotaService.GetHistoryIsValidEdit(id);
            var sb = new StringBuilder();

            sb.AppendLine("Thông tin sửa NhanSuEmployeeLeaveQuota</h3>")
             .AppendLine("<table class=\"table table-hover table-bordered\">")
             .AppendLine("  <thead class=\"bg-info\">")
             .AppendLine("    <tr>")
             .AppendLine("     <th>Mã nhân viên</th>")
             .AppendLine("     <th>Năm tính phép</th>")
             .AppendLine("     <th>Tổng phép năm được cấp</th>")
             .AppendLine("     <th>Phép tồn chuyển sang</th>")
             .AppendLine("     <th>Phép đã sử dụng</th>")
             .AppendLine("     <th>Phép còn lại</th>")
             .AppendLine("     <th>Phép bù/Khác</th>")
            ...
            if (isValidModel.Any())
            {
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{oldData?.Year}</td>")
                  .AppendLine($"      <td>{oldData?.TotalAnnualLeave}</td>")
                  .AppendLine($"      <td>{oldData?.CarriedOverLeave}</td>")
                  .AppendLine($"      <td>{oldData?.UsedLeave}</td>")
                  .AppendLine($"      <td>{oldData?.RemainingLeave}</td>")
                  .AppendLine($"      <td>{oldData?.OtherLeaveBalance}</td>")
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{newData?.Year}</td>")
                  .AppendLine($"      <td>{newData?.TotalAnnualLeave}</td>")
                  .AppendLine($"      <td>{newData?.CarriedOverLeave}</td>")
                  .AppendLine($"      <td>{newData?.UsedLeave}</td>")
                  .AppendLine($"      <td>{newData?.RemainingLeave}</td>")
                  .AppendLine($"      <td>{newData?.OtherLeaveBalance}</td>")
        ...
        {
            var detail = await _nhansuemployeeleavequotaService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Thêm - NhanSuEmployeeLeaveQuota</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Mã nhân viên</th>")
            ...
            sb.AppendLine($"    <tr>")
              ...
              .AppendLine($"     <td>{detail.Year}</td>")
              .AppendLine($"     <td>{detail.TotalAnnualLeave}</td>")
              .AppendLine($"     <td>{detail.CarriedOverLeave}</td>")
              .AppendLine($"     <td>{detail.UsedLeave}</td>")
              .AppendLine($"     <td>{detail.RemainingLeave}</td>")
              .AppendLine($"     <td>{detail.OtherLeaveBalance}</td>")
        ...
        private async Task<string> BuildEmailContentDelete(string id)
        {
            var detail = await _nhansuemployeeleavequotaService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Xóa - NhanSuEmployeeLeaveQuota</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Mã nhân viên</th>")
        ...
        private ApprovalTask CreateApprovalTask(string title, string content, NhanSu_EmployeeLeaveQuota input, string tableName, string parentMajorId, string majorId, string userId)
        {
        ...
        private ApprovalTask UpdateApprovalTask(string Id, string title, string content, NhanSu_EmployeeLeaveQuota input, string tableName, string parentMajorId, string majorId, string userId)
        {
            return new ApprovalTask
            {
                Id = Id,
