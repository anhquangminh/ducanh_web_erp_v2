...
namespace DucAnh2025.Controllers.API.NhanSu
{
    ...
    [ApiController]
    public class NhanSuRewardController : ControllerBase
    {
        private readonly IApprovalTaskRepository _approvalTaskService;
        private readonly INhanSu_RewardRepository _nhansurewardService;
        ...
        public NhanSuRewardController(IApprovalTaskRepository approvalTaskRepository,
            INhanSu_RewardRepository nhansurewardRepository,
            ...
            IPermissionRepository permissionService)
        {
            ...
            _nhansurewardService = nhansurewardRepository;
        ...
        public string tableName = "NhanSu_Rewards";

        [HttpPost("GetByVM")]
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetByVM(string groupId, [FromBody] NhanSu_RewardModel? input = null)
        {
            try
            {
                var model = input ?? new NhanSu_RewardModel();
                var list = await _nhansurewardService.GetAllByVM(model, groupId);
        ...
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetAll(string groupId)
        {
            try
            {
                var list = await _nhansurewardService.GetAll(groupId);
                return Ok(new ApiResponse<IEnumerable<object>>(true, "Thành công", list));
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetById(string id)
        {
            try
            {
                var result = await _nhansurewardService.GetById(id);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetHistory(string id)
        {
            try
            {
                var result = await _nhansurewardService.GetHistory(id);
                return Ok(new ApiResponse<object>(true, "Lưu thành công .", result));
            }
            catch (Exception ex)
            {
        ...
        [HttpPost("Create")]
        public async Task<ActionResult<ApiResponse<object>>> Create([FromBody] NhanSu_Reward input, [FromQuery] bool isEdit)
        {
            try
            {
                ...
                if (editCheck)
                {
                    ...
                    if (!await _nhansurewardService.CheckExclusive(new[] { input.Id }, now))
                    ...
                    await _nhansurewardService.Update(input, "");
                    ...
                    {
                        await _approvalTaskService.Insert(CreateApprovalTask("Khen thưởng- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }
                    else
                    {
                        await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Khen thưởng- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }

                    await InsertEmailHistories(content, "Khen thưởng- sửa !", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
                ...
                else
                {
                    ...
                    input.DateApproval = null;
                    input.ApprovalDept = null;
                    await _nhansurewardService.Insert(input, "");
                    string content = await BuildEmailContentAdd(input.Id);
                    await _approvalTaskService.Insert(CreateApprovalTask("Khen thưởngThêm mơí !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    await InsertEmailHistories(content, "Khen thưởng - Thêm", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            try
            {
                ...
                var query = await _nhansurewardService.GetById(id);
                if (query == null)
                ...
                if (!await _nhansurewardService.CheckDelete(query))
                ...
                if (!await _nhansurewardService.CheckExclusive(new[] { id }, baseTime))
                ...
                await _nhansurewardService.Update(query, "");

                var content = await BuildEmailContentDelete(id);
                var dataApproval = await _approvalTaskService.GetByOriginalId(id);
                if (dataApproval == null)
                {
                    await _approvalTaskService.Insert(CreateApprovalTask("Khen thưởng - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }
                else
                {
                    await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Khen thưởng - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }

                await InsertEmailHistories(content, "Khen thưởng - xóa", query.GroupId, groupId, firstApproval.Id, query.Id, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> Duyet(string id)
        {
            try
            {
                var user = _applicationUserService.GetCurrentUser();
                var userId = user.Id;
                var entity = await _nhansurewardService.GetById(id);
                if (entity == null)
                ...
                if (entity.IsActive != 3)
                {
                    ...
                    await _nhansurewardService.Approval(entity, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> HuyDuyet(string id)
        {
            try
            {
                ...
                if (entity.IsActive != 3)
                {
                    await _nhansurewardService.NoApproval(entity, userId);
                    var dataNoApproval = await _nhansurewardService.GetById(entity.Id);
        ...
        {
            var isValidModel = await _nhansurewardService.GetHistoryIsValidEdit(id);
            var sb = new StringBuilder();

            sb.AppendLine("Thông tin sửa NhanSuReward</h3>")
             .AppendLine("<table class=\"table table-hover table-bordered\">")
             .AppendLine("  <thead class=\"bg-info\">")
             .AppendLine("    <tr>")
             .AppendLine("     <th>Mã nhân viên</th>")
             .AppendLine("     <th>Loại Khen thưởng</th>")
             .AppendLine("     <th>Mã Quyết định Khen thưởng</th>")
             .AppendLine("     <th>Tên Quyết định/Sự kiện</th>")
             .AppendLine("     <th>Ngày Khen thưởng</th>")
             .AppendLine("     <th>Hình thức thưởng</th>")
             .AppendLine("     <th>Giá trị thưởng</th>")
             .AppendLine("     <th>Chi tiết thành tích</th>")
            ...
            if (isValidModel.Any())
            {
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{oldData?.RewardTypeId}</td>")
                  .AppendLine($"      <td>{oldData?.DecisionCode}</td>")
                  .AppendLine($"      <td>{oldData?.RewardName}</td>")
                  .AppendLine($"      <td>{oldData?.RewardDate}</td>")
                  .AppendLine($"      <td>{oldData?.RewardForm}</td>")
                  .AppendLine($"      <td>{oldData?.RewardValue}</td>")
                  .AppendLine($"      <td>{oldData?.AchievementDetail}</td>")
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{newData?.RewardTypeId}</td>")
                  .AppendLine($"      <td>{newData?.DecisionCode}</td>")
                  .AppendLine($"      <td>{newData?.RewardName}</td>")
                  .AppendLine($"      <td>{newData?.RewardDate}</td>")
                  .AppendLine($"      <td>{newData?.RewardForm}</td>")
                  .AppendLine($"      <td>{newData?.RewardValue}</td>")
                  .AppendLine($"      <td>{newData?.AchievementDetail}</td>")
        ...
        {
            var detail = await _nhansurewardService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Thêm - NhanSuReward</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Mã nhân viên</th>")
            ...
            sb.AppendLine($"    <tr>")
              ...
              .AppendLine($"     <td>{detail.RewardTypeId}</td>")
              .AppendLine($"     <td>{detail.DecisionCode}</td>")
              .AppendLine($"     <td>{detail.RewardName}</td>")
              .AppendLine($"     <td>{detail.RewardDate}</td>")
              .AppendLine($"     <td>{detail.RewardForm}</td>")
              .AppendLine($"     <td>{detail.RewardValue}</td>")
              .AppendLine($"     <td>{detail.AchievementDetail}</td>")
        ...
        private async Task<string> BuildEmailContentDelete(string id)
        {
            var detail = await _nhansurewardService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Xóa - NhanSuReward</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Mã nhân viên</th>")
        ...
        private ApprovalTask CreateApprovalTask(string title, string content, NhanSu_Reward input, string tableName, string parentMajorId, string majorId, string userId)
        {
        ...
        private ApprovalTask UpdateApprovalTask(string Id, string title, string content, NhanSu_Reward input, string tableName, string parentMajorId, string majorId, string userId)
        {
            return new ApprovalTask
            {
                Id = Id,
