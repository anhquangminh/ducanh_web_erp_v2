...
namespace DucAnh2025.Controllers.API.NhanSu
{
    ...
    [ApiController]
    public class NhanSuContractController : ControllerBase
    {
        private readonly IApprovalTaskRepository _approvalTaskService;
        private readonly INhanSu_ContractRepository _nhansucontractService;
        ...
        public NhanSuContractController(IApprovalTaskRepository approvalTaskRepository,
            INhanSu_ContractRepository nhansucontractRepository,
            ...
            IPermissionRepository permissionService)
        {
            ...
            _nhansucontractService = nhansucontractRepository;
        ...
        public string tableName = "NhanSu_Contracts";

        [HttpPost("GetByVM")]
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetByVM(string groupId, [FromBody] NhanSu_ContractModel? input = null)
        {
            try
            {
                var model = input ?? new NhanSu_ContractModel();
                var list = await _nhansucontractService.GetAllByVM(model, groupId);
        ...
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetAll(string groupId)
        {
            try
            {
                var list = await _nhansucontractService.GetAll(groupId);
                return Ok(new ApiResponse<IEnumerable<object>>(true, "Thành công", list));
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetById(string id)
        {
            try
            {
                var result = await _nhansucontractService.GetById(id);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetHistory(string id)
        {
            try
            {
                var result = await _nhansucontractService.GetHistory(id);
                return Ok(new ApiResponse<object>(true, "Lưu thành công .", result));
            }
            catch (Exception ex)
            {
        ...
        [HttpPost("Create")]
        public async Task<ActionResult<ApiResponse<object>>> Create([FromBody] NhanSu_Contract input, [FromQuery] bool isEdit)
        {
            try
            {
                ...
                if (editCheck)
                {
                    ...
                    if (!await _nhansucontractService.CheckExclusive(new[] { input.Id }, now))
                    ...
                    await _nhansucontractService.Update(input, "");
                    ...
                    {
                        await _approvalTaskService.Insert(CreateApprovalTask("Hợp đồng- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }
                    else
                    {
                        await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Hợp đồng- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }

                    await InsertEmailHistories(content, "Hợp đồng- sửa !", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
                ...
                else
                {
                    ...
                    input.DateApproval = null;
                    input.ApprovalDept = null;
                    await _nhansucontractService.Insert(input, "");
                    string content = await BuildEmailContentAdd(input.Id);
                    await _approvalTaskService.Insert(CreateApprovalTask("Hợp đồngThêm mơí !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    await InsertEmailHistories(content, "Hợp đồng - Thêm", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            try
            {
                ...
                var query = await _nhansucontractService.GetById(id);
                if (query == null)
                ...
                if (!await _nhansucontractService.CheckDelete(query))
                ...
                if (!await _nhansucontractService.CheckExclusive(new[] { id }, baseTime))
                ...
                await _nhansucontractService.Update(query, "");

                var content = await BuildEmailContentDelete(id);
                var dataApproval = await _approvalTaskService.GetByOriginalId(id);
                if (dataApproval == null)
                {
                    await _approvalTaskService.Insert(CreateApprovalTask("Hợp đồng - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }
                else
                {
                    await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Hợp đồng - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }

                await InsertEmailHistories(content, "Hợp đồng - xóa", query.GroupId, groupId, firstApproval.Id, query.Id, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> Duyet(string id)
        {
            try
            {
                var user = _applicationUserService.GetCurrentUser();
                var userId = user.Id;
                var entity = await _nhansucontractService.GetById(id);
                if (entity == null)
                ...
                if (entity.IsActive != 3)
                {
                    ...
                    await _nhansucontractService.Approval(entity, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> HuyDuyet(string id)
        {
            try
            {
                ...
                if (entity.IsActive != 3)
                {
                    await _nhansucontractService.NoApproval(entity, userId);
                    var dataNoApproval = await _nhansucontractService.GetById(entity.Id);
        ...
        {
            var isValidModel = await _nhansucontractService.GetHistoryIsValidEdit(id);
            var sb = new StringBuilder();

            sb.AppendLine("Thông tin sửa NhanSuContract</h3>")
             .AppendLine("<table class=\"table table-hover table-bordered\">")
             .AppendLine("  <thead class=\"bg-info\">")
             .AppendLine("    <tr>")
             .AppendLine("     <th>Mã nhân viên</th>")
             .AppendLine("     <th>Loại Hợp đồng</th>")
             .AppendLine("     <th>Mã Số Hợp đồng</th>")
             .AppendLine("     <th>Tên Hợp đồng</th>")
             .AppendLine("     <th>Ngày Ký kết</th>")
             .AppendLine("     <th>Ngày Hiệu lực</th>")
             .AppendLine("     <th>Ngày Hết hạn</th>")
             .AppendLine("     <th>Trạng thái Hợp đồng</th>")
             .AppendLine("     <th>Mức lương ghi trong HĐ</th>")
             .AppendLine("     <th>Thời hạn HĐ (tháng)</th>")
             .AppendLine("     <th>Đường dẫn File HĐ</th>")
             .AppendLine("     <th>Có cần Ký điện tử</th>")
             .AppendLine("     <th>Ngày gia hạn gần nhất</th>")
             .AppendLine("     <th>Ghi chú</th>")
             .AppendLine("     <th>Ngày cảnh báo hết hạn</th>")
             .AppendLine("     <th>Người ký (đại diện công ty)</th>")
             .AppendLine("     <th>Chức danh người ký</th>")
            ...
            if (isValidModel.Any())
            {
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{oldData?.ContractTypeId}</td>")
                  .AppendLine($"      <td>{oldData?.ContractCode}</td>")
                  .AppendLine($"      <td>{oldData?.ContractName}</td>")
                  .AppendLine($"      <td>{oldData?.SigningDate}</td>")
                  .AppendLine($"      <td>{oldData?.EffectiveDate}</td>")
                  .AppendLine($"      <td>{oldData?.ExpirationDate}</td>")
                  .AppendLine($"      <td>{oldData?.ContractStatusId}</td>")
                  .AppendLine($"      <td>{oldData?.ContractSalary}</td>")
                  .AppendLine($"      <td>{oldData?.DurationMonths}</td>")
                  .AppendLine($"      <td>{oldData?.FilePath}</td>")
                  .AppendLine($"      <td>{oldData?.IsESignRequired}</td>")
                  .AppendLine($"      <td>{oldData?.LastRenewalDate}</td>")
                  .AppendLine($"      <td>{oldData?.Notes}</td>")
                  .AppendLine($"      <td>{oldData?.AlertDate}</td>")
                  .AppendLine($"      <td>{oldData?.SignerId}</td>")
                  .AppendLine($"      <td>{oldData?.SignerPosition}</td>")
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{newData?.ContractTypeId}</td>")
                  .AppendLine($"      <td>{newData?.ContractCode}</td>")
                  .AppendLine($"      <td>{newData?.ContractName}</td>")
                  .AppendLine($"      <td>{newData?.SigningDate}</td>")
                  .AppendLine($"      <td>{newData?.EffectiveDate}</td>")
                  .AppendLine($"      <td>{newData?.ExpirationDate}</td>")
                  .AppendLine($"      <td>{newData?.ContractStatusId}</td>")
                  .AppendLine($"      <td>{newData?.ContractSalary}</td>")
                  .AppendLine($"      <td>{newData?.DurationMonths}</td>")
                  .AppendLine($"      <td>{newData?.FilePath}</td>")
                  .AppendLine($"      <td>{newData?.IsESignRequired}</td>")
                  .AppendLine($"      <td>{newData?.LastRenewalDate}</td>")
                  .AppendLine($"      <td>{newData?.Notes}</td>")
                  .AppendLine($"      <td>{newData?.AlertDate}</td>")
                  .AppendLine($"      <td>{newData?.SignerId}</td>")
                  .AppendLine($"      <td>{newData?.SignerPosition}</td>")
        ...
        {
            var detail = await _nhansucontractService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Thêm - NhanSuContract</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Mã nhân viên</th>")
            ...
            sb.AppendLine($"    <tr>")
              ...
              .AppendLine($"     <td>{detail.ContractTypeId}</td>")
              .AppendLine($"     <td>{detail.ContractCode}</td>")
              .AppendLine($"     <td>{detail.ContractName}</td>")
              .AppendLine($"     <td>{detail.SigningDate}</td>")
              .AppendLine($"     <td>{detail.EffectiveDate}</td>")
              .AppendLine($"     <td>{detail.ExpirationDate}</td>")
              .AppendLine($"     <td>{detail.ContractStatusId}</td>")
              .AppendLine($"     <td>{detail.ContractSalary}</td>")
              .AppendLine($"     <td>{detail.DurationMonths}</td>")
              .AppendLine($"     <td>{detail.FilePath}</td>")
              .AppendLine($"     <td>{detail.IsESignRequired}</td>")
              .AppendLine($"     <td>{detail.LastRenewalDate}</td>")
              .AppendLine($"     <td>{detail.Notes}</td>")
              .AppendLine($"     <td>{detail.AlertDate}</td>")
              .AppendLine($"     <td>{detail.SignerId}</td>")
              .AppendLine($"     <td>{detail.SignerPosition}</td>")
        ...
        private async Task<string> BuildEmailContentDelete(string id)
        {
            var detail = await _nhansucontractService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Xóa - NhanSuContract</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Mã nhân viên</th>")
        ...
        private ApprovalTask CreateApprovalTask(string title, string content, NhanSu_Contract input, string tableName, string parentMajorId, string majorId, string userId)
        {
        ...
        private ApprovalTask UpdateApprovalTask(string Id, string title, string content, NhanSu_Contract input, string tableName, string parentMajorId, string majorId, string userId)
        {
            return new ApprovalTask
            {
                Id = Id,
