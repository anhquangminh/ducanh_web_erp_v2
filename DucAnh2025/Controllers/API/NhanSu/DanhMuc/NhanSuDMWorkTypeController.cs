...
namespace DucAnh2025.Controllers.API.NhanSu.DanhMuc
{
    ...
    [ApiController]
    public class NhanSuDMWorkTypeController : ControllerBase
    {
        private readonly IApprovalTaskRepository _approvalTaskService;
        private readonly INhanSu_DM_WorkTypeRepository _nhansudmworktypeService;
        ...
        public NhanSuDMWorkTypeController(IApprovalTaskRepository approvalTaskRepository,
            INhanSu_DM_WorkTypeRepository nhansudmworktypeRepository,
            ...
            IPermissionRepository permissionService)
        {
            ...
            _nhansudmworktypeService = nhansudmworktypeRepository;
        ...
        public string tableName = "NhanSu_DM_WorkTypes";

        [HttpPost("GetByVM")]
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetByVM(string groupId, [FromBody] NhanSu_DM_WorkTypeModel? input = null)
        {
            try
            {
                var model = input ?? new NhanSu_DM_WorkTypeModel();
                var list = await _nhansudmworktypeService.GetAllByVM(model, groupId);
        ...
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetAll(string groupId)
        {
            try
            {
                var list = await _nhansudmworktypeService.GetAll(groupId);
                return Ok(new ApiResponse<IEnumerable<object>>(true, "Thành công", list));
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetById(string id)
        {
            try
            {
                var result = await _nhansudmworktypeService.GetById(id);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetHistory(string id)
        {
            try
            {
                var result = await _nhansudmworktypeService.GetHistory(id);
                return Ok(new ApiResponse<object>(true, "Lưu thành công .", result));
            }
            catch (Exception ex)
            {
        ...
        [HttpPost("Create")]
        public async Task<ActionResult<ApiResponse<object>>> Create([FromBody] NhanSu_DM_WorkType input, [FromQuery] bool isEdit)
        {
            try
            {
                ...
                if (editCheck)
                {
                    ...
                    if (!await _nhansudmworktypeService.CheckExclusive(new[] { input.Id }, now))
                    ...
                    await _nhansudmworktypeService.Update(input, "");
                    ...
                    {
                        await _approvalTaskService.Insert(CreateApprovalTask("Loại công- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }
                    else
                    {
                        await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Loại công- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }

                    await InsertEmailHistories(content, "Loại công- sửa !", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
                ...
                else
                {
                    ...
                    input.DateApproval = null;
                    input.ApprovalDept = null;
                    await _nhansudmworktypeService.Insert(input, "");
                    string content = await BuildEmailContentAdd(input.Id);
                    await _approvalTaskService.Insert(CreateApprovalTask("Loại côngThêm mơí !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    await InsertEmailHistories(content, "Loại công - Thêm", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            try
            {
                ...
                var query = await _nhansudmworktypeService.GetById(id);
                if (query == null)
                ...
                if (!await _nhansudmworktypeService.CheckDelete(query))
                ...
                if (!await _nhansudmworktypeService.CheckExclusive(new[] { id }, baseTime))
                ...
                await _nhansudmworktypeService.Update(query, "");

                var content = await BuildEmailContentDelete(id);
                var dataApproval = await _approvalTaskService.GetByOriginalId(id);
                if (dataApproval == null)
                {
                    await _approvalTaskService.Insert(CreateApprovalTask("Loại công - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }
                else
                {
                    await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Loại công - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }

                await InsertEmailHistories(content, "Loại công - xóa", query.GroupId, groupId, firstApproval.Id, query.Id, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> Duyet(string id)
        {
            try
            {
                var user = _applicationUserService.GetCurrentUser();
                var userId = user.Id;
                var entity = await _nhansudmworktypeService.GetById(id);
                if (entity == null)
                ...
                if (entity.IsActive != 3)
                {
                    ...
                    await _nhansudmworktypeService.Approval(entity, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> HuyDuyet(string id)
        {
            try
            {
                ...
                if (entity.IsActive != 3)
                {
                    await _nhansudmworktypeService.NoApproval(entity, userId);
                    var dataNoApproval = await _nhansudmworktypeService.GetById(entity.Id);
        ...
        {
            var isValidModel = await _nhansudmworktypeService.GetHistoryIsValidEdit(id);
            var sb = new StringBuilder();

            sb.AppendLine("Thông tin sửa NhanSuDMWorkType</h3>")
             .AppendLine("<table class=\"table table-hover table-bordered\">")
             .AppendLine("  <thead class=\"bg-info\">")
             .AppendLine("    <tr>")
             .AppendLine("     <th>Tên Loại Công</th>")
             .AppendLine("     <th>Nhóm Loại Công</th>")
             .AppendLine("     <th>Giá trị tính lương</th>")
            ...
            if (isValidModel.Any())
            {
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{oldData?.WorkTypeName}</td>")
                  .AppendLine($"      <td>{oldData?.WorkTypeGroup}</td>")
                  .AppendLine($"      <td>{oldData?.SalaryFactor}</td>")
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{newData?.WorkTypeName}</td>")
                  .AppendLine($"      <td>{newData?.WorkTypeGroup}</td>")
                  .AppendLine($"      <td>{newData?.SalaryFactor}</td>")
        ...
        {
            var detail = await _nhansudmworktypeService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Thêm - NhanSuDMWorkType</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Tên Loại Công</th>")
              ...
              .AppendLine("  <tbody>");
            sb.AppendLine($"    <tr>")
              .AppendLine($"     <td>{detail.WorkTypeName}</td>")
              .AppendLine($"     <td>{detail.WorkTypeGroup}</td>")
              .AppendLine($"     <td>{detail.SalaryFactor}</td>")
        ...
        private async Task<string> BuildEmailContentDelete(string id)
        {
            var detail = await _nhansudmworktypeService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Xóa - NhanSuDMWorkType</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Tên Loại Công</th>")
        ...
        private ApprovalTask CreateApprovalTask(string title, string content, NhanSu_DM_WorkType input, string tableName, string parentMajorId, string majorId, string userId)
        {
        ...
        private ApprovalTask UpdateApprovalTask(string Id, string title, string content, NhanSu_DM_WorkType input, string tableName, string parentMajorId, string majorId, string userId)
        {
            return new ApprovalTask
            {
                Id = Id,
