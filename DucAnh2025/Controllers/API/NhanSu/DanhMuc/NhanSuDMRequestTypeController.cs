...
namespace DucAnh2025.Controllers.API.NhanSu.DanhMuc
{
    ...
    [ApiController]
    public class NhanSuDMRequestTypeController : ControllerBase
    {
        private readonly IApprovalTaskRepository _approvalTaskService;
        private readonly INhanSu_DM_RequestTypeRepository _nhansudmrequesttypeService;
        ...
        public NhanSuDMRequestTypeController(IApprovalTaskRepository approvalTaskRepository,
            INhanSu_DM_RequestTypeRepository nhansudmrequesttypeRepository,
            ...
            IPermissionRepository permissionService)
        {
            ...
            _nhansudmrequesttypeService = nhansudmrequesttypeRepository;
        ...
        public string tableName = "NhanSu_DM_RequestTypes";

        [HttpPost("GetByVM")]
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetByVM(string groupId, [FromBody] NhanSu_DM_RequestTypeModel? input = null)
        {
            try
            {
                var model = input ?? new NhanSu_DM_RequestTypeModel();
                var list = await _nhansudmrequesttypeService.GetAllByVM(model, groupId);
        ...
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetAll(string groupId)
        {
            try
            {
                var list = await _nhansudmrequesttypeService.GetAll(groupId);
                return Ok(new ApiResponse<IEnumerable<object>>(true, "Thành công", list));
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetById(string id)
        {
            try
            {
                var result = await _nhansudmrequesttypeService.GetById(id);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetHistory(string id)
        {
            try
            {
                var result = await _nhansudmrequesttypeService.GetHistory(id);
                return Ok(new ApiResponse<object>(true, "Lưu thành công .", result));
            }
            catch (Exception ex)
            {
        ...
        [HttpPost("Create")]
        public async Task<ActionResult<ApiResponse<object>>> Create([FromBody] NhanSu_DM_RequestType input, [FromQuery] bool isEdit)
        {
            try
            {
                ...
                if (editCheck)
                {
                    ...
                    if (!await _nhansudmrequesttypeService.CheckExclusive(new[] { input.Id }, now))
                    ...
                    await _nhansudmrequesttypeService.Update(input, "");
                    ...
                    {
                        await _approvalTaskService.Insert(CreateApprovalTask("Loại đơn từ- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }
                    else
                    {
                        await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Loại đơn từ- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }

                    await InsertEmailHistories(content, "Loại đơn từ- sửa !", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
                ...
                else
                {
                    ...
                    input.DateApproval = null;
                    input.ApprovalDept = null;
                    await _nhansudmrequesttypeService.Insert(input, "");
                    string content = await BuildEmailContentAdd(input.Id);
                    await _approvalTaskService.Insert(CreateApprovalTask("Loại đơn từThêm mơí !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    await InsertEmailHistories(content, "Loại đơn từ - Thêm", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            try
            {
                ...
                var query = await _nhansudmrequesttypeService.GetById(id);
                if (query == null)
                ...
                if (!await _nhansudmrequesttypeService.CheckDelete(query))
                ...
                if (!await _nhansudmrequesttypeService.CheckExclusive(new[] { id }, baseTime))
                ...
                await _nhansudmrequesttypeService.Update(query, "");

                var content = await BuildEmailContentDelete(id);
                var dataApproval = await _approvalTaskService.GetByOriginalId(id);
                if (dataApproval == null)
                {
                    await _approvalTaskService.Insert(CreateApprovalTask("Loại đơn từ - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }
                else
                {
                    await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Loại đơn từ - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }

                await InsertEmailHistories(content, "Loại đơn từ - xóa", query.GroupId, groupId, firstApproval.Id, query.Id, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> Duyet(string id)
        {
            try
            {
                var user = _applicationUserService.GetCurrentUser();
                var userId = user.Id;
                var entity = await _nhansudmrequesttypeService.GetById(id);
                if (entity == null)
                ...
                if (entity.IsActive != 3)
                {
                    ...
                    await _nhansudmrequesttypeService.Approval(entity, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> HuyDuyet(string id)
        {
            try
            {
                ...
                if (entity.IsActive != 3)
                {
                    await _nhansudmrequesttypeService.NoApproval(entity, userId);
                    var dataNoApproval = await _nhansudmrequesttypeService.GetById(entity.Id);
        ...
        {
            var isValidModel = await _nhansudmrequesttypeService.GetHistoryIsValidEdit(id);
            var sb = new StringBuilder();

            sb.AppendLine("Thông tin sửa NhanSuDMRequestType</h3>")
             .AppendLine("<table class=\"table table-hover table-bordered\">")
             .AppendLine("  <thead class=\"bg-info\">")
             .AppendLine("    <tr>")
             .AppendLine("     <th>Tên Loại Đơn</th>")
             .AppendLine("     <th>Ảnh hưởng Chấm công</th>")
             .AppendLine("     <th>Mã Quy trình mặc định</th>")
            ...
            if (isValidModel.Any())
            {
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{oldData?.RequestTypeName}</td>")
                  .AppendLine($"      <td>{oldData?.IsTimeAttendanceImpact}</td>")
                  .AppendLine($"      <td>{oldData?.DefaultProcessId}</td>")
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{newData?.RequestTypeName}</td>")
                  .AppendLine($"      <td>{newData?.IsTimeAttendanceImpact}</td>")
                  .AppendLine($"      <td>{newData?.DefaultProcessId}</td>")
        ...
        {
            var detail = await _nhansudmrequesttypeService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Thêm - NhanSuDMRequestType</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Tên Loại Đơn</th>")
              ...
              .AppendLine("  <tbody>");
            sb.AppendLine($"    <tr>")
              .AppendLine($"     <td>{detail.RequestTypeName}</td>")
              .AppendLine($"     <td>{detail.IsTimeAttendanceImpact}</td>")
              .AppendLine($"     <td>{detail.DefaultProcessId}</td>")
        ...
        private async Task<string> BuildEmailContentDelete(string id)
        {
            var detail = await _nhansudmrequesttypeService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Xóa - NhanSuDMRequestType</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Tên Loại Đơn</th>")
        ...
        private ApprovalTask CreateApprovalTask(string title, string content, NhanSu_DM_RequestType input, string tableName, string parentMajorId, string majorId, string userId)
        {
        ...
        private ApprovalTask UpdateApprovalTask(string Id, string title, string content, NhanSu_DM_RequestType input, string tableName, string parentMajorId, string majorId, string userId)
        {
            return new ApprovalTask
            {
                Id = Id,
