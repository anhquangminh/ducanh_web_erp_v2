...
namespace DucAnh2025.Controllers.API.NhanSu.DanhMuc
{
    ...
    [ApiController]
    public class NhanSuDMLeaveTypeController : ControllerBase
    {
        private readonly IApprovalTaskRepository _approvalTaskService;
        private readonly INhanSu_DM_LeaveTypeRepository _nhansudmleavetypeService;
        ...
        public NhanSuDMLeaveTypeController(IApprovalTaskRepository approvalTaskRepository,
            INhanSu_DM_LeaveTypeRepository nhansudmleavetypeRepository,
            ...
            IPermissionRepository permissionService)
        {
            ...
            _nhansudmleavetypeService = nhansudmleavetypeRepository;
        ...
        public string tableName = "NhanSu_DM_LeaveTypes";

        [HttpPost("GetByVM")]
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetByVM(string groupId, [FromBody] NhanSu_DM_LeaveTypeModel? input = null)
        {
            try
            {
                var model = input ?? new NhanSu_DM_LeaveTypeModel();
                var list = await _nhansudmleavetypeService.GetAllByVM(model, groupId);
        ...
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetAll(string groupId)
        {
            try
            {
                var list = await _nhansudmleavetypeService.GetAll(groupId);
                return Ok(new ApiResponse<IEnumerable<object>>(true, "Thành công", list));
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetById(string id)
        {
            try
            {
                var result = await _nhansudmleavetypeService.GetById(id);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetHistory(string id)
        {
            try
            {
                var result = await _nhansudmleavetypeService.GetHistory(id);
                return Ok(new ApiResponse<object>(true, "Lưu thành công .", result));
            }
            catch (Exception ex)
            {
        ...
        [HttpPost("Create")]
        public async Task<ActionResult<ApiResponse<object>>> Create([FromBody] NhanSu_DM_LeaveType input, [FromQuery] bool isEdit)
        {
            try
            {
                ...
                if (editCheck)
                {
                    ...
                    if (!await _nhansudmleavetypeService.CheckExclusive(new[] { input.Id }, now))
                    ...
                    await _nhansudmleavetypeService.Update(input, "");
                    ...
                    {
                        await _approvalTaskService.Insert(CreateApprovalTask("Loại hình nghỉ phép- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }
                    else
                    {
                        await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Loại hình nghỉ phép- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }

                    await InsertEmailHistories(content, "Loại hình nghỉ phép- sửa !", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
                ...
                else
                {
                    ...
                    input.DateApproval = null;
                    input.ApprovalDept = null;
                    await _nhansudmleavetypeService.Insert(input, "");
                    string content = await BuildEmailContentAdd(input.Id);
                    await _approvalTaskService.Insert(CreateApprovalTask("Loại hình nghỉ phépThêm mơí !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    await InsertEmailHistories(content, "Loại hình nghỉ phép - Thêm", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            try
            {
                ...
                var query = await _nhansudmleavetypeService.GetById(id);
                if (query == null)
                ...
                if (!await _nhansudmleavetypeService.CheckDelete(query))
                ...
                if (!await _nhansudmleavetypeService.CheckExclusive(new[] { id }, baseTime))
                ...
                await _nhansudmleavetypeService.Update(query, "");

                var content = await BuildEmailContentDelete(id);
                var dataApproval = await _approvalTaskService.GetByOriginalId(id);
                if (dataApproval == null)
                {
                    await _approvalTaskService.Insert(CreateApprovalTask("Loại hình nghỉ phép - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }
                else
                {
                    await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Loại hình nghỉ phép - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }

                await InsertEmailHistories(content, "Loại hình nghỉ phép - xóa", query.GroupId, groupId, firstApproval.Id, query.Id, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> Duyet(string id)
        {
            try
            {
                var user = _applicationUserService.GetCurrentUser();
                var userId = user.Id;
                var entity = await _nhansudmleavetypeService.GetById(id);
                if (entity == null)
                ...
                if (entity.IsActive != 3)
                {
                    ...
                    await _nhansudmleavetypeService.Approval(entity, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> HuyDuyet(string id)
        {
            try
            {
                ...
                if (entity.IsActive != 3)
                {
                    await _nhansudmleavetypeService.NoApproval(entity, userId);
                    var dataNoApproval = await _nhansudmleavetypeService.GetById(entity.Id);
        ...
        {
            var isValidModel = await _nhansudmleavetypeService.GetHistoryIsValidEdit(id);
            var sb = new StringBuilder();

            sb.AppendLine("Thông tin sửa NhanSuDMLeaveType</h3>")
             .AppendLine("<table class=\"table table-hover table-bordered\">")
             .AppendLine("  <thead class=\"bg-info\">")
             .AppendLine("    <tr>")
             .AppendLine("     <th>Tên Loại phép</th>")
             .AppendLine("     <th>Có trừ phép tồn không?</th>")
             .AppendLine("     <th>Số lượng mặc định/năm</th>")
             .AppendLine("     <th>Ghi chú/Cơ sở pháp lý</th>")
            ...
            if (isValidModel.Any())
            {
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{oldData?.LeaveTypeName}</td>")
                  .AppendLine($"      <td>{oldData?.IsDeductible}</td>")
                  .AppendLine($"      <td>{oldData?.DefaultQuota}</td>")
                  .AppendLine($"      <td>{oldData?.LegalBasis}</td>")
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{newData?.LeaveTypeName}</td>")
                  .AppendLine($"      <td>{newData?.IsDeductible}</td>")
                  .AppendLine($"      <td>{newData?.DefaultQuota}</td>")
                  .AppendLine($"      <td>{newData?.LegalBasis}</td>")
        ...
        {
            var detail = await _nhansudmleavetypeService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Thêm - NhanSuDMLeaveType</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Tên Loại phép</th>")
              ...
              .AppendLine("  <tbody>");
            sb.AppendLine($"    <tr>")
              .AppendLine($"     <td>{detail.LeaveTypeName}</td>")
              .AppendLine($"     <td>{detail.IsDeductible}</td>")
              .AppendLine($"     <td>{detail.DefaultQuota}</td>")
              .AppendLine($"     <td>{detail.LegalBasis}</td>")
        ...
        private async Task<string> BuildEmailContentDelete(string id)
        {
            var detail = await _nhansudmleavetypeService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Xóa - NhanSuDMLeaveType</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Tên Loại phép</th>")
        ...
        private ApprovalTask CreateApprovalTask(string title, string content, NhanSu_DM_LeaveType input, string tableName, string parentMajorId, string majorId, string userId)
        {
        ...
        private ApprovalTask UpdateApprovalTask(string Id, string title, string content, NhanSu_DM_LeaveType input, string tableName, string parentMajorId, string majorId, string userId)
        {
            return new ApprovalTask
            {
                Id = Id,
