...
namespace DucAnh2025.Controllers.API.NhanSu.DanhMuc
{
    ...
    [ApiController]
    public class NhanSuDMDisciplineTypeController : ControllerBase
    {
        private readonly IApprovalTaskRepository _approvalTaskService;
        private readonly INhanSu_DM_DisciplineTypeRepository _nhansudmdisciplinetypeService;
        ...
        public NhanSuDMDisciplineTypeController(IApprovalTaskRepository approvalTaskRepository,
            INhanSu_DM_DisciplineTypeRepository nhansudmdisciplinetypeRepository,
            ...
            IPermissionRepository permissionService)
        {
            ...
            _nhansudmdisciplinetypeService = nhansudmdisciplinetypeRepository;
        ...
        public string tableName = "NhanSu_DM_DisciplineTypes";

        [HttpPost("GetByVM")]
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetByVM(string groupId, [FromBody] NhanSu_DM_DisciplineTypeModel? input = null)
        {
            try
            {
                var model = input ?? new NhanSu_DM_DisciplineTypeModel();
                var list = await _nhansudmdisciplinetypeService.GetAllByVM(model, groupId);
        ...
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetAll(string groupId)
        {
            try
            {
                var list = await _nhansudmdisciplinetypeService.GetAll(groupId);
                return Ok(new ApiResponse<IEnumerable<object>>(true, "Thành công", list));
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetById(string id)
        {
            try
            {
                var result = await _nhansudmdisciplinetypeService.GetById(id);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetHistory(string id)
        {
            try
            {
                var result = await _nhansudmdisciplinetypeService.GetHistory(id);
                return Ok(new ApiResponse<object>(true, "Lưu thành công .", result));
            }
            catch (Exception ex)
            {
        ...
        [HttpPost("Create")]
        public async Task<ActionResult<ApiResponse<object>>> Create([FromBody] NhanSu_DM_DisciplineType input, [FromQuery] bool isEdit)
        {
            try
            {
                ...
                if (editCheck)
                {
                    ...
                    if (!await _nhansudmdisciplinetypeService.CheckExclusive(new[] { input.Id }, now))
                    ...
                    await _nhansudmdisciplinetypeService.Update(input, "");
                    ...
                    {
                        await _approvalTaskService.Insert(CreateApprovalTask("Hình thức kỷ luật- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }
                    else
                    {
                        await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Hình thức kỷ luật- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }

                    await InsertEmailHistories(content, "Hình thức kỷ luật- sửa !", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
                ...
                else
                {
                    ...
                    input.DateApproval = null;
                    input.ApprovalDept = null;
                    await _nhansudmdisciplinetypeService.Insert(input, "");
                    string content = await BuildEmailContentAdd(input.Id);
                    await _approvalTaskService.Insert(CreateApprovalTask("Hình thức kỷ luậtThêm mơí !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    await InsertEmailHistories(content, "Hình thức kỷ luật - Thêm", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            try
            {
                ...
                var query = await _nhansudmdisciplinetypeService.GetById(id);
                if (query == null)
                ...
                if (!await _nhansudmdisciplinetypeService.CheckDelete(query))
                ...
                if (!await _nhansudmdisciplinetypeService.CheckExclusive(new[] { id }, baseTime))
                ...
                await _nhansudmdisciplinetypeService.Update(query, "");

                var content = await BuildEmailContentDelete(id);
                var dataApproval = await _approvalTaskService.GetByOriginalId(id);
                if (dataApproval == null)
                {
                    await _approvalTaskService.Insert(CreateApprovalTask("Hình thức kỷ luật - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }
                else
                {
                    await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Hình thức kỷ luật - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }

                await InsertEmailHistories(content, "Hình thức kỷ luật - xóa", query.GroupId, groupId, firstApproval.Id, query.Id, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> Duyet(string id)
        {
            try
            {
                var user = _applicationUserService.GetCurrentUser();
                var userId = user.Id;
                var entity = await _nhansudmdisciplinetypeService.GetById(id);
                if (entity == null)
                ...
                if (entity.IsActive != 3)
                {
                    ...
                    await _nhansudmdisciplinetypeService.Approval(entity, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> HuyDuyet(string id)
        {
            try
            {
                ...
                if (entity.IsActive != 3)
                {
                    await _nhansudmdisciplinetypeService.NoApproval(entity, userId);
                    var dataNoApproval = await _nhansudmdisciplinetypeService.GetById(entity.Id);
        ...
        {
            var isValidModel = await _nhansudmdisciplinetypeService.GetHistoryIsValidEdit(id);
            var sb = new StringBuilder();

            sb.AppendLine("Thông tin sửa NhanSuDMDisciplineType</h3>")
             .AppendLine("<table class=\"table table-hover table-bordered\">")
             .AppendLine("  <thead class=\"bg-info\">")
             .AppendLine("    <tr>")
             .AppendLine("     <th>Tên Hình thức Kỷ luật</th>")
             .AppendLine("     <th>Cấp độ Kỷ luật</th>")
             .AppendLine("     <th>Có tác động đến HĐLĐ không?</th>")
             .AppendLine("     <th>Ghi chú/Mô tả</th>")
            ...
            if (isValidModel.Any())
            {
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{oldData?.DisciplineTypeName}</td>")
                  .AppendLine($"      <td>{oldData?.Level}</td>")
                  .AppendLine($"      <td>{oldData?.IsTerminationRisk}</td>")
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{newData?.DisciplineTypeName}</td>")
                  .AppendLine($"      <td>{newData?.Level}</td>")
                  .AppendLine($"      <td>{newData?.IsTerminationRisk}</td>")
        ...
        {
            var detail = await _nhansudmdisciplinetypeService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Thêm - NhanSuDMDisciplineType</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Tên Hình thức Kỷ luật</th>")
              ...
              .AppendLine("  <tbody>");
            sb.AppendLine($"    <tr>")
              .AppendLine($"     <td>{detail.DisciplineTypeName}</td>")
              .AppendLine($"     <td>{detail.Level}</td>")
              .AppendLine($"     <td>{detail.IsTerminationRisk}</td>")
        ...
        private async Task<string> BuildEmailContentDelete(string id)
        {
            var detail = await _nhansudmdisciplinetypeService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Xóa - NhanSuDMDisciplineType</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Tên Hình thức Kỷ luật</th>")
        ...
        private ApprovalTask CreateApprovalTask(string title, string content, NhanSu_DM_DisciplineType input, string tableName, string parentMajorId, string majorId, string userId)
        {
        ...
        private ApprovalTask UpdateApprovalTask(string Id, string title, string content, NhanSu_DM_DisciplineType input, string tableName, string parentMajorId, string majorId, string userId)
        {
            return new ApprovalTask
            {
                Id = Id,
