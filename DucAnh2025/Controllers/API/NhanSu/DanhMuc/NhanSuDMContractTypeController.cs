...
namespace DucAnh2025.Controllers.API.NhanSu.DanhMuc
{
    [Authorize(AuthenticationSchemes = "JwtBearer")]
    [Route("api/[controller]")]
    [ApiController]
    public class NhanSuDMContractTypeController : ControllerBase
    {
        private readonly IApprovalTaskRepository _approvalTaskService;
        private readonly INhanSu_DM_ContractTypeRepository _nhansudmcontracttypeService;
        ...
        public NhanSuDMContractTypeController(IApprovalTaskRepository approvalTaskRepository,
            INhanSu_DM_ContractTypeRepository nhansudmcontracttypeRepository,
            ...
            IPermissionRepository permissionService)
        {
            ...
            _nhansudmcontracttypeService = nhansudmcontracttypeRepository;
        ...
        public string tableName = "NhanSu_DM_ContractTypes";

        [HttpPost("GetByVM")]
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetByVM(string groupId, [FromBody] NhanSu_DM_ContractTypeModel? input = null)
        {
            try
            {
                var model = input ?? new NhanSu_DM_ContractTypeModel();
                var list = await _nhansudmcontracttypeService.GetAllByVM(model, groupId);
        ...
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetAll(string groupId)
        {
            try
            {
                var list = await _nhansudmcontracttypeService.GetAll(groupId);
                return Ok(new ApiResponse<IEnumerable<object>>(true, "Thành công", list));
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetById(string id)
        {
            try
            {
                var result = await _nhansudmcontracttypeService.GetById(id);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetHistory(string id)
        {
            try
            {
                var result = await _nhansudmcontracttypeService.GetHistory(id);
                return Ok(new ApiResponse<object>(true, "Lưu thành công .", result));
            }
            catch (Exception ex)
            {
        ...
        [HttpPost("Create")]
        public async Task<ActionResult<ApiResponse<object>>> Create([FromBody] NhanSu_DM_ContractType input, [FromQuery] bool isEdit)
        {
            try
            {
                ...
                if (editCheck)
                {
                    ...
                    if (!await _nhansudmcontracttypeService.CheckExclusive(new[] { input.Id }, now))
                    ...
                    await _nhansudmcontracttypeService.Update(input, "");
                    ...
                    {
                        await _approvalTaskService.Insert(CreateApprovalTask("Loại hợp đồng- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }
                    else
                    {
                        await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Loại hợp đồng- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }

                    await InsertEmailHistories(content, "Loại hợp đồng- sửa !", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
                ...
                else
                {
                    ...
                    input.DateApproval = null;
                    input.ApprovalDept = null;
                    await _nhansudmcontracttypeService.Insert(input, "");
                    string content = await BuildEmailContentAdd(input.Id);
                    await _approvalTaskService.Insert(CreateApprovalTask("Loại hợp đồngThêm mơí !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    await InsertEmailHistories(content, "Loại hợp đồng - Thêm", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            try
            {
                ...
                var query = await _nhansudmcontracttypeService.GetById(id);
                if (query == null)
                ...
                if (!await _nhansudmcontracttypeService.CheckDelete(query))
                ...
                if (!await _nhansudmcontracttypeService.CheckExclusive(new[] { id }, baseTime))
                ...
                await _nhansudmcontracttypeService.Update(query, "");

                var content = await BuildEmailContentDelete(id);
                var dataApproval = await _approvalTaskService.GetByOriginalId(id);
                if (dataApproval == null)
                {
                    await _approvalTaskService.Insert(CreateApprovalTask("Loại hợp đồng - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }
                else
                {
                    await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Loại hợp đồng - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }

                await InsertEmailHistories(content, "Loại hợp đồng - xóa", query.GroupId, groupId, firstApproval.Id, query.Id, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> Duyet(string id)
        {
            try
            {
                var user = _applicationUserService.GetCurrentUser();
                var userId = user.Id;
                var entity = await _nhansudmcontracttypeService.GetById(id);
                if (entity == null)
                ...
                if (entity.IsActive != 3)
                {
                    ...
                    await _nhansudmcontracttypeService.Approval(entity, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> HuyDuyet(string id)
        {
            try
            {
                ...
                if (entity.IsActive != 3)
                {
                    await _nhansudmcontracttypeService.NoApproval(entity, userId);
                    var dataNoApproval = await _nhansudmcontracttypeService.GetById(entity.Id);
        ...
        {
            var isValidModel = await _nhansudmcontracttypeService.GetHistoryIsValidEdit(id);
            var sb = new StringBuilder();

            sb.AppendLine("Thông tin sửa NhanSuDMContractType</h3>")
             .AppendLine("<table class=\"table table-hover table-bordered\">")
             .AppendLine("  <thead class=\"bg-info\">")
             .AppendLine("    <tr>")
             .AppendLine("     <th>Tên Loại Hợp đồng</th>")
             .AppendLine("     <th>Mô tả</th>")
             .AppendLine("     <th>Thời hạn (tháng)</th>")
             .AppendLine("     <th>Cảnh báo hết hạn (ngày)</th>")
             .AppendLine("     <th>Cho phép gia hạn</th>")
             .AppendLine("     <th>Loại HĐ kế tiếp</th>")
             .AppendLine("     <th>Tỷ lệ Lương thử việc</th>")
            ...
            if (isValidModel.Any())
            {
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{oldData?.ContractTypeName}</td>")
                  .AppendLine($"      <td>{oldData?.Description}</td>")
                  .AppendLine($"      <td>{oldData?.DurationMonths}</td>")
                  .AppendLine($"      <td>{oldData?.WarningDays}</td>")
                  .AppendLine($"      <td>{oldData?.IsRenewable}</td>")
                  .AppendLine($"      <td>{oldData?.NextContractTypeId}</td>")
                  .AppendLine($"      <td>{oldData?.ProbationSalaryRate}</td>")
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{newData?.ContractTypeName}</td>")
                  .AppendLine($"      <td>{newData?.Description}</td>")
                  .AppendLine($"      <td>{newData?.DurationMonths}</td>")
                  .AppendLine($"      <td>{newData?.WarningDays}</td>")
                  .AppendLine($"      <td>{newData?.IsRenewable}</td>")
                  .AppendLine($"      <td>{newData?.NextContractTypeId}</td>")
                  .AppendLine($"      <td>{newData?.ProbationSalaryRate}</td>")
        ...
        {
            var detail = await _nhansudmcontracttypeService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Thêm - NhanSuDMContractType</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Tên Loại Hợp đồng</th>")
              ...
              .AppendLine("  <tbody>");
            sb.AppendLine($"    <tr>")
              .AppendLine($"     <td>{detail.ContractTypeName}</td>")
              .AppendLine($"     <td>{detail.Description}</td>")
              .AppendLine($"     <td>{detail.DurationMonths}</td>")
              .AppendLine($"     <td>{detail.WarningDays}</td>")
              .AppendLine($"     <td>{detail.IsRenewable}</td>")
              .AppendLine($"     <td>{detail.NextContractTypeId}</td>")
              .AppendLine($"     <td>{detail.ProbationSalaryRate}</td>")
        ...
        private async Task<string> BuildEmailContentDelete(string id)
        {
            var detail = await _nhansudmcontracttypeService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Xóa - NhanSuDMContractType</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Tên Loại Hợp đồng</th>")
        ...
        private ApprovalTask CreateApprovalTask(string title, string content, NhanSu_DM_ContractType input, string tableName, string parentMajorId, string majorId, string userId)
        {
        ...
        private ApprovalTask UpdateApprovalTask(string Id, string title, string content, NhanSu_DM_ContractType input, string tableName, string parentMajorId, string majorId, string userId)
        {
            return new ApprovalTask
            {
                Id = Id,
