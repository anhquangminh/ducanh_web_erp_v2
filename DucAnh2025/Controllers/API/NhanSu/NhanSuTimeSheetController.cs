...
namespace DucAnh2025.Controllers.API.NhanSu
{
    ...
    [ApiController]
    public class NhanSuTimeSheetController : ControllerBase
    {
        private readonly IApprovalTaskRepository _approvalTaskService;
        private readonly INhanSu_TimeSheetRepository _nhansutimesheetService;
        ...
        public NhanSuTimeSheetController(IApprovalTaskRepository approvalTaskRepository,
            INhanSu_TimeSheetRepository nhansutimesheetRepository,
            ...
            IPermissionRepository permissionService)
        {
            ...
            _nhansutimesheetService = nhansutimesheetRepository;
        ...
        public string tableName = "NhanSu_TimeSheets";

        [HttpPost("GetByVM")]
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetByVM(string groupId, [FromBody] NhanSu_TimeSheetModel? input = null)
        {
            try
            {
                var model = input ?? new NhanSu_TimeSheetModel();
                var list = await _nhansutimesheetService.GetAllByVM(model, groupId);
        ...
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetAll(string groupId)
        {
            try
            {
                var list = await _nhansutimesheetService.GetAll(groupId);
                return Ok(new ApiResponse<IEnumerable<object>>(true, "Thành công", list));
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetById(string id)
        {
            try
            {
                var result = await _nhansutimesheetService.GetById(id);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetHistory(string id)
        {
            try
            {
                var result = await _nhansutimesheetService.GetHistory(id);
                return Ok(new ApiResponse<object>(true, "Lưu thành công .", result));
            }
            catch (Exception ex)
            {
        ...
        [HttpPost("Create")]
        public async Task<ActionResult<ApiResponse<object>>> Create([FromBody] NhanSu_TimeSheet input, [FromQuery] bool isEdit)
        {
            try
            {
                ...
                if (editCheck)
                {
                    ...
                    if (!await _nhansutimesheetService.CheckExclusive(new[] { input.Id }, now))
                    ...
                    await _nhansutimesheetService.Update(input, "");
                    ...
                    {
                        await _approvalTaskService.Insert(CreateApprovalTask("Chấm công- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }
                    else
                    {
                        await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Chấm công- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }

                    await InsertEmailHistories(content, "Chấm công- sửa !", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
                ...
                else
                {
                    ...
                    input.DateApproval = null;
                    input.ApprovalDept = null;
                    await _nhansutimesheetService.Insert(input, "");
                    string content = await BuildEmailContentAdd(input.Id);
                    await _approvalTaskService.Insert(CreateApprovalTask("Chấm côngThêm mơí !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    await InsertEmailHistories(content, "Chấm công - Thêm", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            try
            {
                ...
                var query = await _nhansutimesheetService.GetById(id);
                if (query == null)
                ...
                if (!await _nhansutimesheetService.CheckDelete(query))
                ...
                if (!await _nhansutimesheetService.CheckExclusive(new[] { id }, baseTime))
                ...
                await _nhansutimesheetService.Update(query, "");

                var content = await BuildEmailContentDelete(id);
                var dataApproval = await _approvalTaskService.GetByOriginalId(id);
                if (dataApproval == null)
                {
                    await _approvalTaskService.Insert(CreateApprovalTask("Chấm công - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }
                else
                {
                    await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Chấm công - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }

                await InsertEmailHistories(content, "Chấm công - xóa", query.GroupId, groupId, firstApproval.Id, query.Id, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> Duyet(string id)
        {
            try
            {
                var user = _applicationUserService.GetCurrentUser();
                var userId = user.Id;
                var entity = await _nhansutimesheetService.GetById(id);
                if (entity == null)
                ...
                if (entity.IsActive != 3)
                {
                    ...
                    await _nhansutimesheetService.Approval(entity, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> HuyDuyet(string id)
        {
            try
            {
                ...
                if (entity.IsActive != 3)
                {
                    await _nhansutimesheetService.NoApproval(entity, userId);
                    var dataNoApproval = await _nhansutimesheetService.GetById(entity.Id);
        ...
        {
            var isValidModel = await _nhansutimesheetService.GetHistoryIsValidEdit(id);
            var sb = new StringBuilder();

            sb.AppendLine("Thông tin sửa NhanSuTimeSheet</h3>")
             .AppendLine("<table class=\"table table-hover table-bordered\">")
             .AppendLine("  <thead class=\"bg-info\">")
             .AppendLine("    <tr>")
             .AppendLine("     <th>Mã nhân viên</th>")
             .AppendLine("     <th>Ngày Chấm công</th>")
             .AppendLine("     <th>Giờ Vào</th>")
             .AppendLine("     <th>Giờ Ra</th>")
             .AppendLine("     <th>Mã Loại công</th>")
             .AppendLine("     <th>Tổng Giờ công</th>")
             .AppendLine("     <th>Tổng Công thực tế</th>")
             .AppendLine("     <th>Nguồn dữ liệu</th>")
            ...
            if (isValidModel.Any())
            {
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{oldData?.TimeSheetDate}</td>")
                  .AppendLine($"      <td>{oldData?.CheckInTime}</td>")
                  .AppendLine($"      <td>{oldData?.CheckOutTime}</td>")
                  .AppendLine($"      <td>{oldData?.WorkTypeId}</td>")
                  .AppendLine($"      <td>{oldData?.TotalHours}</td>")
                  .AppendLine($"      <td>{oldData?.TotalDays}</td>")
                  .AppendLine($"      <td>{oldData?.Source}</td>")
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{newData?.TimeSheetDate}</td>")
                  .AppendLine($"      <td>{newData?.CheckInTime}</td>")
                  .AppendLine($"      <td>{newData?.CheckOutTime}</td>")
                  .AppendLine($"      <td>{newData?.WorkTypeId}</td>")
                  .AppendLine($"      <td>{newData?.TotalHours}</td>")
                  .AppendLine($"      <td>{newData?.TotalDays}</td>")
                  .AppendLine($"      <td>{newData?.Source}</td>")
        ...
        {
            var detail = await _nhansutimesheetService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Thêm - NhanSuTimeSheet</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Mã nhân viên</th>")
            ...
            sb.AppendLine($"    <tr>")
              ...
              .AppendLine($"     <td>{detail.TimeSheetDate}</td>")
              .AppendLine($"     <td>{detail.CheckInTime}</td>")
              .AppendLine($"     <td>{detail.CheckOutTime}</td>")
              .AppendLine($"     <td>{detail.WorkTypeId}</td>")
              .AppendLine($"     <td>{detail.TotalHours}</td>")
              .AppendLine($"     <td>{detail.TotalDays}</td>")
              .AppendLine($"     <td>{detail.Source}</td>")
        ...
        private async Task<string> BuildEmailContentDelete(string id)
        {
            var detail = await _nhansutimesheetService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Xóa - NhanSuTimeSheet</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Mã nhân viên</th>")
        ...
        private ApprovalTask CreateApprovalTask(string title, string content, NhanSu_TimeSheet input, string tableName, string parentMajorId, string majorId, string userId)
        {
        ...
        private ApprovalTask UpdateApprovalTask(string Id, string title, string content, NhanSu_TimeSheet input, string tableName, string parentMajorId, string majorId, string userId)
        {
            return new ApprovalTask
            {
                Id = Id,
