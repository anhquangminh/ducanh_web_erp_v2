...
namespace DucAnh2025.Controllers.API.NhanSu
{
    ...
    [ApiController]
    public class NhanSuDisciplineController : ControllerBase
    {
        private readonly IApprovalTaskRepository _approvalTaskService;
        private readonly INhanSu_DisciplineRepository _nhansudisciplineService;
        ...
        public NhanSuDisciplineController(IApprovalTaskRepository approvalTaskRepository,
            INhanSu_DisciplineRepository nhansudisciplineRepository,
            ...
            IPermissionRepository permissionService)
        {
            ...
            _nhansudisciplineService = nhansudisciplineRepository;
        ...
        public string tableName = "NhanSu_Disciplines";

        [HttpPost("GetByVM")]
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetByVM(string groupId, [FromBody] NhanSu_DisciplineModel? input = null)
        {
            try
            {
                var model = input ?? new NhanSu_DisciplineModel();
                var list = await _nhansudisciplineService.GetAllByVM(model, groupId);
        ...
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetAll(string groupId)
        {
            try
            {
                var list = await _nhansudisciplineService.GetAll(groupId);
                return Ok(new ApiResponse<IEnumerable<object>>(true, "Thành công", list));
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetById(string id)
        {
            try
            {
                var result = await _nhansudisciplineService.GetById(id);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetHistory(string id)
        {
            try
            {
                var result = await _nhansudisciplineService.GetHistory(id);
                return Ok(new ApiResponse<object>(true, "Lưu thành công .", result));
            }
            catch (Exception ex)
            {
        ...
        [HttpPost("Create")]
        public async Task<ActionResult<ApiResponse<object>>> Create([FromBody] NhanSu_Discipline input, [FromQuery] bool isEdit)
        {
            try
            {
                ...
                if (editCheck)
                {
                    ...
                    if (!await _nhansudisciplineService.CheckExclusive(new[] { input.Id }, now))
                    ...
                    await _nhansudisciplineService.Update(input, "");
                    ...
                    {
                        await _approvalTaskService.Insert(CreateApprovalTask("Kỷ luật- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }
                    else
                    {
                        await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Kỷ luật- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }

                    await InsertEmailHistories(content, "Kỷ luật- sửa !", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
                ...
                else
                {
                    ...
                    input.DateApproval = null;
                    input.ApprovalDept = null;
                    await _nhansudisciplineService.Insert(input, "");
                    string content = await BuildEmailContentAdd(input.Id);
                    await _approvalTaskService.Insert(CreateApprovalTask("Kỷ luậtThêm mơí !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    await InsertEmailHistories(content, "Kỷ luật - Thêm", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            try
            {
                ...
                var query = await _nhansudisciplineService.GetById(id);
                if (query == null)
                ...
                if (!await _nhansudisciplineService.CheckDelete(query))
                ...
                if (!await _nhansudisciplineService.CheckExclusive(new[] { id }, baseTime))
                ...
                await _nhansudisciplineService.Update(query, "");

                var content = await BuildEmailContentDelete(id);
                var dataApproval = await _approvalTaskService.GetByOriginalId(id);
                if (dataApproval == null)
                {
                    await _approvalTaskService.Insert(CreateApprovalTask("Kỷ luật - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }
                else
                {
                    await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Kỷ luật - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }

                await InsertEmailHistories(content, "Kỷ luật - xóa", query.GroupId, groupId, firstApproval.Id, query.Id, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> Duyet(string id)
        {
            try
            {
                var user = _applicationUserService.GetCurrentUser();
                var userId = user.Id;
                var entity = await _nhansudisciplineService.GetById(id);
                if (entity == null)
                ...
                if (entity.IsActive != 3)
                {
                    ...
                    await _nhansudisciplineService.Approval(entity, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> HuyDuyet(string id)
        {
            try
            {
                ...
                if (entity.IsActive != 3)
                {
                    await _nhansudisciplineService.NoApproval(entity, userId);
                    var dataNoApproval = await _nhansudisciplineService.GetById(entity.Id);
        ...
        {
            var isValidModel = await _nhansudisciplineService.GetHistoryIsValidEdit(id);
            var sb = new StringBuilder();

            sb.AppendLine("Thông tin sửa NhanSuDiscipline</h3>")
             .AppendLine("<table class=\"table table-hover table-bordered\">")
             .AppendLine("  <thead class=\"bg-info\">")
             .AppendLine("    <tr>")
             .AppendLine("     <th>Mã nhân viên</th>")
             .AppendLine("     <th>Loại Kỷ luật</th>")
             .AppendLine("     <th>Mã Quyết định Kỷ luật</th>")
             .AppendLine("     <th>Tên Quyết định/Sự cố</th>")
             .AppendLine("     <th>Ngày Phát hiện Sự cố</th>")
             .AppendLine("     <th>Ngày Kỷ luật (Hiệu lực)</th>")
             .AppendLine("     <th>Thời hạn Kỷ luật</th>")
             .AppendLine("     <th>Tác động Tài chính</th>")
             .AppendLine("     <th>Chi tiết Vi phạm</th>")
            ...
            if (isValidModel.Any())
            {
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{oldData?.DisciplineTypeId}</td>")
                  .AppendLine($"      <td>{oldData?.DecisionCode}</td>")
                  .AppendLine($"      <td>{oldData?.IncidentName}</td>")
                  .AppendLine($"      <td>{oldData?.IncidentDate}</td>")
                  .AppendLine($"      <td>{oldData?.DisciplineDate}</td>")
                  .AppendLine($"      <td>{oldData?.DurationDays}</td>")
                  .AppendLine($"      <td>{oldData?.FinancialImpact}</td>")
                  .AppendLine($"      <td>{oldData?.ViolationDetail}</td>")
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{newData?.DisciplineTypeId}</td>")
                  .AppendLine($"      <td>{newData?.DecisionCode}</td>")
                  .AppendLine($"      <td>{newData?.IncidentName}</td>")
                  .AppendLine($"      <td>{newData?.IncidentDate}</td>")
                  .AppendLine($"      <td>{newData?.DisciplineDate}</td>")
                  .AppendLine($"      <td>{newData?.DurationDays}</td>")
                  .AppendLine($"      <td>{newData?.FinancialImpact}</td>")
                  .AppendLine($"      <td>{newData?.ViolationDetail}</td>")
        ...
        {
            var detail = await _nhansudisciplineService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Thêm - NhanSuDiscipline</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Mã nhân viên</th>")
            ...
            sb.AppendLine($"    <tr>")
              ...
              .AppendLine($"     <td>{detail.DisciplineTypeId}</td>")
              .AppendLine($"     <td>{detail.DecisionCode}</td>")
              .AppendLine($"     <td>{detail.IncidentName}</td>")
              .AppendLine($"     <td>{detail.IncidentDate}</td>")
              .AppendLine($"     <td>{detail.DisciplineDate}</td>")
              .AppendLine($"     <td>{detail.DurationDays}</td>")
              .AppendLine($"     <td>{detail.FinancialImpact}</td>")
              .AppendLine($"     <td>{detail.ViolationDetail}</td>")
        ...
        private async Task<string> BuildEmailContentDelete(string id)
        {
            var detail = await _nhansudisciplineService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Xóa - NhanSuDiscipline</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Mã nhân viên</th>")
        ...
        private ApprovalTask CreateApprovalTask(string title, string content, NhanSu_Discipline input, string tableName, string parentMajorId, string majorId, string userId)
        {
        ...
        private ApprovalTask UpdateApprovalTask(string Id, string title, string content, NhanSu_Discipline input, string tableName, string parentMajorId, string majorId, string userId)
        {
            return new ApprovalTask
            {
                Id = Id,
