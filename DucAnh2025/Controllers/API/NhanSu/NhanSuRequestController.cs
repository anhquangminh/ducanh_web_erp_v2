...
namespace DucAnh2025.Controllers.API.NhanSu
{
    ...
    [ApiController]
    public class NhanSuRequestController : ControllerBase
    {
        private readonly IApprovalTaskRepository _approvalTaskService;
        private readonly INhanSu_RequestRepository _nhansurequestService;
        ...
        public NhanSuRequestController(IApprovalTaskRepository approvalTaskRepository,
            INhanSu_RequestRepository nhansurequestRepository,
            ...
            IPermissionRepository permissionService)
        {
            ...
            _nhansurequestService = nhansurequestRepository;
        ...
        public string tableName = "NhanSu_Requests";

        [HttpPost("GetByVM")]
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetByVM(string groupId, [FromBody] NhanSu_RequestModel? input = null)
        {
            try
            {
                var model = input ?? new NhanSu_RequestModel();
                var list = await _nhansurequestService.GetAllByVM(model, groupId);
        ...
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetAll(string groupId)
        {
            try
            {
                var list = await _nhansurequestService.GetAll(groupId);
                return Ok(new ApiResponse<IEnumerable<object>>(true, "Thành công", list));
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetById(string id)
        {
            try
            {
                var result = await _nhansurequestService.GetById(id);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetHistory(string id)
        {
            try
            {
                var result = await _nhansurequestService.GetHistory(id);
                return Ok(new ApiResponse<object>(true, "Lưu thành công .", result));
            }
            catch (Exception ex)
            {
        ...
        [HttpPost("Create")]
        public async Task<ActionResult<ApiResponse<object>>> Create([FromBody] NhanSu_Request input, [FromQuery] bool isEdit)
        {
            try
            {
                ...
                if (editCheck)
                {
                    ...
                    if (!await _nhansurequestService.CheckExclusive(new[] { input.Id }, now))
                    ...
                    await _nhansurequestService.Update(input, "");
                    ...
                    {
                        await _approvalTaskService.Insert(CreateApprovalTask("Đơn từ- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }
                    else
                    {
                        await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Đơn từ- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }

                    await InsertEmailHistories(content, "Đơn từ- sửa !", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
                ...
                else
                {
                    ...
                    input.DateApproval = null;
                    input.ApprovalDept = null;
                    await _nhansurequestService.Insert(input, "");
                    string content = await BuildEmailContentAdd(input.Id);
                    await _approvalTaskService.Insert(CreateApprovalTask("Đơn từThêm mơí !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    await InsertEmailHistories(content, "Đơn từ - Thêm", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            try
            {
                ...
                var query = await _nhansurequestService.GetById(id);
                if (query == null)
                ...
                if (!await _nhansurequestService.CheckDelete(query))
                ...
                if (!await _nhansurequestService.CheckExclusive(new[] { id }, baseTime))
                ...
                await _nhansurequestService.Update(query, "");

                var content = await BuildEmailContentDelete(id);
                var dataApproval = await _approvalTaskService.GetByOriginalId(id);
                if (dataApproval == null)
                {
                    await _approvalTaskService.Insert(CreateApprovalTask("Đơn từ - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }
                else
                {
                    await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Đơn từ - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }

                await InsertEmailHistories(content, "Đơn từ - xóa", query.GroupId, groupId, firstApproval.Id, query.Id, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> Duyet(string id)
        {
            try
            {
                var user = _applicationUserService.GetCurrentUser();
                var userId = user.Id;
                var entity = await _nhansurequestService.GetById(id);
                if (entity == null)
                ...
                if (entity.IsActive != 3)
                {
                    ...
                    await _nhansurequestService.Approval(entity, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> HuyDuyet(string id)
        {
            try
            {
                ...
                if (entity.IsActive != 3)
                {
                    await _nhansurequestService.NoApproval(entity, userId);
                    var dataNoApproval = await _nhansurequestService.GetById(entity.Id);
        ...
        {
            var isValidModel = await _nhansurequestService.GetHistoryIsValidEdit(id);
            var sb = new StringBuilder();

            sb.AppendLine("Thông tin sửa NhanSuRequest</h3>")
             .AppendLine("<table class=\"table table-hover table-bordered\">")
             .AppendLine("  <thead class=\"bg-info\">")
             .AppendLine("    <tr>")
             .AppendLine("     <th>Mã nhân viên</th>")
             .AppendLine("     <th>Loại Đơn từ</th>")
             .AppendLine("     <th>Ngày nộp đơn</th>")
             .AppendLine("     <th>Ngày Bắt đầu</th>")
             .AppendLine("     <th>Ngày Kết thúc</th>")
             .AppendLine("     <th>Tổng thời gian</th>")
             .AppendLine("     <th>Phép được dùng</th>")
             .AppendLine("     <th>Lý do/Nội dung</th>")
             .AppendLine("     <th>Người duyệt Hiện tại</th>")
            ...
            if (isValidModel.Any())
            {
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{oldData?.RequestTypeId}</td>")
                  .AppendLine($"      <td>{oldData?.RequestDate}</td>")
                  .AppendLine($"      <td>{oldData?.StartDate}</td>")
                  .AppendLine($"      <td>{oldData?.EndDate}</td>")
                  .AppendLine($"      <td>{oldData?.TotalDuration}</td>")
                  .AppendLine($"      <td>{oldData?.LeaveTypeId}</td>")
                  .AppendLine($"      <td>{oldData?.Reason}</td>")
                  .AppendLine($"      <td>{oldData?.CurrentApproverId}</td>")
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{newData?.RequestTypeId}</td>")
                  .AppendLine($"      <td>{newData?.RequestDate}</td>")
                  .AppendLine($"      <td>{newData?.StartDate}</td>")
                  .AppendLine($"      <td>{newData?.EndDate}</td>")
                  .AppendLine($"      <td>{newData?.TotalDuration}</td>")
                  .AppendLine($"      <td>{newData?.LeaveTypeId}</td>")
                  .AppendLine($"      <td>{newData?.Reason}</td>")
                  .AppendLine($"      <td>{newData?.CurrentApproverId}</td>")
        ...
        {
            var detail = await _nhansurequestService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Thêm - NhanSuRequest</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Mã nhân viên</th>")
            ...
            sb.AppendLine($"    <tr>")
              ...
              .AppendLine($"     <td>{detail.RequestTypeId}</td>")
              .AppendLine($"     <td>{detail.RequestDate}</td>")
              .AppendLine($"     <td>{detail.StartDate}</td>")
              .AppendLine($"     <td>{detail.EndDate}</td>")
              .AppendLine($"     <td>{detail.TotalDuration}</td>")
              .AppendLine($"     <td>{detail.LeaveTypeId}</td>")
              .AppendLine($"     <td>{detail.Reason}</td>")
              .AppendLine($"     <td>{detail.CurrentApproverId}</td>")
        ...
        private async Task<string> BuildEmailContentDelete(string id)
        {
            var detail = await _nhansurequestService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Xóa - NhanSuRequest</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Mã nhân viên</th>")
        ...
        private ApprovalTask CreateApprovalTask(string title, string content, NhanSu_Request input, string tableName, string parentMajorId, string majorId, string userId)
        {
        ...
        private ApprovalTask UpdateApprovalTask(string Id, string title, string content, NhanSu_Request input, string tableName, string parentMajorId, string majorId, string userId)
        {
            return new ApprovalTask
            {
                Id = Id,
