...
namespace DucAnh2025.Controllers.API.NhanSu
{
    ...
    [ApiController]
    public class NhanSuSalaryHistoryController : ControllerBase
    {
        private readonly IApprovalTaskRepository _approvalTaskService;
        private readonly INhanSu_SalaryHistoryRepository _nhansusalaryhistoryService;
        ...
        public NhanSuSalaryHistoryController(IApprovalTaskRepository approvalTaskRepository,
            INhanSu_SalaryHistoryRepository nhansusalaryhistoryRepository,
            ...
            IPermissionRepository permissionService)
        {
            ...
            _nhansusalaryhistoryService = nhansusalaryhistoryRepository;
        ...
        public string tableName = "NhanSu_SalaryHistorys";

        [HttpPost("GetByVM")]
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetByVM(string groupId, [FromBody] NhanSu_SalaryHistoryModel? input = null)
        {
            try
            {
                var model = input ?? new NhanSu_SalaryHistoryModel();
                var list = await _nhansusalaryhistoryService.GetAllByVM(model, groupId);
        ...
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetAll(string groupId)
        {
            try
            {
                var list = await _nhansusalaryhistoryService.GetAll(groupId);
                return Ok(new ApiResponse<IEnumerable<object>>(true, "Thành công", list));
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetById(string id)
        {
            try
            {
                var result = await _nhansusalaryhistoryService.GetById(id);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetHistory(string id)
        {
            try
            {
                var result = await _nhansusalaryhistoryService.GetHistory(id);
                return Ok(new ApiResponse<object>(true, "Lưu thành công .", result));
            }
            catch (Exception ex)
            {
        ...
        [HttpPost("Create")]
        public async Task<ActionResult<ApiResponse<object>>> Create([FromBody] NhanSu_SalaryHistory input, [FromQuery] bool isEdit)
        {
            try
            {
                ...
                if (editCheck)
                {
                    ...
                    if (!await _nhansusalaryhistoryService.CheckExclusive(new[] { input.Id }, now))
                    ...
                    await _nhansusalaryhistoryService.Update(input, "");
                    ...
                    {
                        await _approvalTaskService.Insert(CreateApprovalTask("Lịch sử Lương- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }
                    else
                    {
                        await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Lịch sử Lương- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }

                    await InsertEmailHistories(content, "Lịch sử Lương- sửa !", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
                ...
                else
                {
                    ...
                    input.DateApproval = null;
                    input.ApprovalDept = null;
                    await _nhansusalaryhistoryService.Insert(input, "");
                    string content = await BuildEmailContentAdd(input.Id);
                    await _approvalTaskService.Insert(CreateApprovalTask("Lịch sử LươngThêm mơí !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    await InsertEmailHistories(content, "Lịch sử Lương - Thêm", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            try
            {
                ...
                var query = await _nhansusalaryhistoryService.GetById(id);
                if (query == null)
                ...
                if (!await _nhansusalaryhistoryService.CheckDelete(query))
                ...
                if (!await _nhansusalaryhistoryService.CheckExclusive(new[] { id }, baseTime))
                ...
                await _nhansusalaryhistoryService.Update(query, "");

                var content = await BuildEmailContentDelete(id);
                var dataApproval = await _approvalTaskService.GetByOriginalId(id);
                if (dataApproval == null)
                {
                    await _approvalTaskService.Insert(CreateApprovalTask("Lịch sử Lương - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }
                else
                {
                    await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Lịch sử Lương - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }

                await InsertEmailHistories(content, "Lịch sử Lương - xóa", query.GroupId, groupId, firstApproval.Id, query.Id, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> Duyet(string id)
        {
            try
            {
                var user = _applicationUserService.GetCurrentUser();
                var userId = user.Id;
                var entity = await _nhansusalaryhistoryService.GetById(id);
                if (entity == null)
                ...
                if (entity.IsActive != 3)
                {
                    ...
                    await _nhansusalaryhistoryService.Approval(entity, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> HuyDuyet(string id)
        {
            try
            {
                ...
                if (entity.IsActive != 3)
                {
                    await _nhansusalaryhistoryService.NoApproval(entity, userId);
                    var dataNoApproval = await _nhansusalaryhistoryService.GetById(entity.Id);
        ...
        {
            var isValidModel = await _nhansusalaryhistoryService.GetHistoryIsValidEdit(id);
            var sb = new StringBuilder();

            sb.AppendLine("Thông tin sửa NhanSuSalaryHistory</h3>")
             ...
             .AppendLine("     <th>Mức Lương Cơ bản</th>")
             .AppendLine("     <th>Tổng Phụ cấp Cố định</th>")
             .AppendLine("     <th>Chi tiết Phụ cấp</th>")
             .AppendLine("     <th>Mã Quyết định Lương</th>")
             .AppendLine("     <th>Lý do thay đổi</th>")
            ...
            if (isValidModel.Any())
            {
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{oldData?.BasicSalary}</td>")
                  .AppendLine($"      <td>{oldData?.TotalFixedAllowance}</td>")
                  .AppendLine($"      <td>{oldData?.AllowanceDetails}</td>")
                  .AppendLine($"      <td>{oldData?.DecisionCode}</td>")
                  .AppendLine($"      <td>{oldData?.ChangeReason}</td>")
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{newData?.BasicSalary}</td>")
                  .AppendLine($"      <td>{newData?.TotalFixedAllowance}</td>")
                  .AppendLine($"      <td>{newData?.AllowanceDetails}</td>")
                  .AppendLine($"      <td>{newData?.DecisionCode}</td>")
                  .AppendLine($"      <td>{newData?.ChangeReason}</td>")
        ...
        {
            var detail = await _nhansusalaryhistoryService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Thêm - NhanSuSalaryHistory</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Mã nhân viên</th>")
            ...
            sb.AppendLine($"    <tr>")
              ...
              .AppendLine($"     <td>{detail.BasicSalary}</td>")
              .AppendLine($"     <td>{detail.TotalFixedAllowance}</td>")
              .AppendLine($"     <td>{detail.AllowanceDetails}</td>")
              .AppendLine($"     <td>{detail.DecisionCode}</td>")
              .AppendLine($"     <td>{detail.ChangeReason}</td>")
        ...
        private async Task<string> BuildEmailContentDelete(string id)
        {
            var detail = await _nhansusalaryhistoryService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Xóa - NhanSuSalaryHistory</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Mã nhân viên</th>")
        ...
        private ApprovalTask CreateApprovalTask(string title, string content, NhanSu_SalaryHistory input, string tableName, string parentMajorId, string majorId, string userId)
        {
        ...
        private ApprovalTask UpdateApprovalTask(string Id, string title, string content, NhanSu_SalaryHistory input, string tableName, string parentMajorId, string majorId, string userId)
        {
            return new ApprovalTask
            {
                Id = Id,
