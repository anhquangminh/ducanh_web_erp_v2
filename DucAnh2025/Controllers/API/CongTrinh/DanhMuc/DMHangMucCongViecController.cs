...
namespace DucAnh2025.Controllers.API.CongTrinh.DanhMuc
{
    ...
    [ApiController]
    public class DMHangMucCongViecController : ControllerBase
    {
        ...
        public DMHangMucCongViecController(IApprovalTaskRepository approvalTaskRepository, 
        ...
        public string tableName = "CT_DM_HangMucCongViecs";
        ...
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetAll(string groupId)
        {
            try
            {
                var list = await _hangMucCongViecService.GetAll(groupId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetById(string id)
        {
            try
            {
                var result = await _hangMucCongViecService.GetById(id);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetHistory(string id)
        {
            try
            {
                var result = await _hangMucCongViecService.GetHistory(id);
        ...
        [HttpPost("Create")]
        public async Task<ActionResult<ApiResponse<object>>> Create([FromBody] CT_DM_HangMucCongViec input, [FromQuery] bool isEdit)
        {
            try
            {
                ...
                if (editCheck)
                {
                    ...
                    // Check dữ liệu
                    if (!await _hangMucCongViecService.CheckExclusive(new[] { input.Id }, now))
                    ...
                    await _hangMucCongViecService.Update(input, "");
                    ...
                    {
                        await _approvalTaskService.Insert(CreateApprovalTask("Danh mục hạng mục công việc - Sửa", contentApp, input, tableName, parentMajorId, majorId, userId), userId);
                    }
                    else
                    {
                        await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Danh mục hạng mục công việc - Sửa", contentApp, input, tableName, parentMajorId, majorId, userId), userId);
                    }
                    
                    // Gửi email
                    await InsertEmailHistories(content, "Danh mục hạng mục công việc - Sửa.", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
                ...
                else
                {
                    ...
                    input.DateApproval = null;
                    input.ApprovalDept = null;
                    await _hangMucCongViecService.Insert(input, "");
                    string content = await BuildEmailContentAdd(input.Id);
                    string contentApp = await BuildEmailContentAppAdd(input.Id);
                    await _approvalTaskService.Insert(CreateApprovalTask("Danh mục hạng mục công việc - Thêm mới", contentApp, input, tableName, parentMajorId, majorId, userId), userId);

                    await InsertEmailHistories(content, "Danh mục hạng mục công việc - Thêm", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            try
            {
                ...
                var query = await _hangMucCongViecService.GetById(id);
                if (query == null)
                ...
                if (!await _hangMucCongViecService.CheckDelete(query))
                ...
                if (!await _hangMucCongViecService.CheckExclusive(new[] { id }, baseTime))
                ...
                await _hangMucCongViecService.Update(query, "");
                ...
                if (dataApproval == null)
                {
                    await _approvalTaskService.Insert(CreateApprovalTask("Danh mục hạng mục công việc - xóa", contentApp, query, tableName, parentMajorId, majorId, userId), userId);
                }
                else
                {
                    await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Danh mục hạng mục công việc - xóa", contentApp, query, tableName, parentMajorId, majorId, userId), userId);
                }
                
                // Hàm gửi email đã có → dùng lại
                await InsertEmailHistories(content, "Danh mục hạng mục công việc - xóa", query.GroupId, groupId, firstApproval.Id, query.Id, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> Duyet(string id)
        {
            try
            {
                var user = _applicationUserService.GetCurrentUser();
                var userId = user.Id;
                var entity = await _hangMucCongViecService.GetById(id);
                if (entity == null)
                ...
                if (entity.IsActive != 3)
                {
                    ...
                    await _hangMucCongViecService.Approval(entity, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> HuyDuyet(string id)
        {
            try
            {
                ...
                if (entity.IsActive != 3)
                {
                    await _hangMucCongViecService.NoApproval(entity, userId);
                    var dataNoApproval = await _hangMucCongViecService.GetById(entity.Id);
        ...
        {
            var isValidModel = await _hangMucCongViecService.GetHistoryIsValidEdit(id);
            var sb = new StringBuilder();

            sb.AppendLine("<h3>Thông tin sửa - Danh mục hạng mục công việc</h3>")
        ...
        {
            var detail = await _hangMucCongViecService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Thêm - Danh mục hạng mục công việc</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Tên danh mục</th>")
        ...
        private async Task<string> BuildEmailContentDelete(string id)
        {
            var detail = await _hangMucCongViecService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Xóa - Danh mục hạng mục công việc</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Tên danh mục</th>")
        ...
        private async Task<string> BuildEmailContentAppEdit(string id)
        {
            ...
            sb.AppendLine("<h3>Thông tin sửa - Danh mục hạng mục công việc</h3>")
              .AppendLine("<table class='table table-hover table-bordered' style='width: 70%; border-collapse: collapse;'>")
              .AppendLine("  <tbody>");

            if (isValidModel.Any())
            {
                var oldData = isValidModel.FirstOrDefault();
                var newData = await _hangMucCongViecService.GetDetails(id);

                // Dòng tiêu đề phân tách
                sb.AppendLine("    <tr><th colspan='2' style='background-color: #d9edf7; text-align:center;'>Dữ liệu cũ</th></tr>")
                  .AppendLine("    <tr><th style='width:35%; background-color:#e0f0ff;'>Tên danh mục</th><td>" + (oldData?.TenDanhMuc ?? "") + "</td></tr>")
                  .AppendLine("    <tr><th style='background-color:#e0f0ff;'>Trạng thái</th><td>" + (oldData?.IsStatus ?? "") + "</td></tr>")
                  .AppendLine("    <tr><th style='background-color:#e0f0ff;'>Người thao tác</th><td>" + (oldData?.CreateBy ?? "") + "</td></tr>")
                  .AppendLine("    <tr><th style='background-color:#e0f0ff;'>Thời gian</th><td>" + (oldData?.CreateAt ) + "</td></tr>");

                sb.AppendLine("    <tr><th colspan='2' style='background-color: #dff0d8; text-align:center;'>Dữ liệu mới</th></tr>")
                  .AppendLine("    <tr><th style='width:35%; background-color:#e0f0ff;'>Tên danh mục</th><td>" + (newData?.TenDanhMuc ?? "") + "</td></tr>")
                  .AppendLine("    <tr><th style='background-color:#e0f0ff;'>Trạng thái</th><td>" + (newData?.IsStatus ?? "") + "</td></tr>")
                  .AppendLine("    <tr><th style='background-color:#e0f0ff;'>Người thao tác</th><td>" + (newData?.CreateBy ?? "") + "</td></tr>")
                  .AppendLine("    <tr><th style='background-color:#e0f0ff;'>Thời gian</th><td>" + (newData?.CreateAt) + "</td></tr>");
            }
            else
            {
                sb.AppendLine("    <tr><td class='text-danger' colspan='2' style='text-align:center;'>Không có dữ liệu</td></tr>");
            }
        ...
        private async Task<string> BuildEmailContentAppAdd(string id)
        {
            ...
            sb.AppendLine("<h3>Thêm - Danh mục hạng mục công việc</h3>")
              ...
              .AppendLine("  <tbody>")
              .AppendLine("    <tr><th style='width:35%; background-color:#e0f0ff;'>Tên danh mục</th><td>" + detail.TenDanhMuc + "</td></tr>")
              .AppendLine("    <tr><th style='background-color:#e0f0ff;'>Trạng thái</th><td>" + detail.IsStatus + "</td></tr>")
              .AppendLine("    <tr><th style='background-color:#e0f0ff;'>Người thao tác</th><td>" + detail.CreateBy + "</td></tr>")
              .AppendLine("    <tr><th style='background-color:#e0f0ff;'>Thời gian</th><td>" + detail.CreateAt + "</td></tr>")
              .AppendLine("  </tbody>")
              .AppendLine("</table>");
        ...
        private async Task<string> BuildEmailContentAppDelete(string id)
        {
            ...
            sb.AppendLine("<h3>Xóa - Danh mục hạng mục công việc</h3>")
              .AppendLine("<table class='table table-hover table-bordered' style='width: 60%; border-collapse: collapse;'>")
              .AppendLine("  <tbody>")
              .AppendLine("    <tr><th style='width:35%; background-color:#e0f0ff;'>Tên danh mục</th><td><span style='color:#888; text-decoration: line-through;'>" + detail.TenDanhMuc + "</span></td></tr>")
              .AppendLine("    <tr><th style='background-color:#e0f0ff;'>Trạng thái</th><td><span style='color:#888; text-decoration: line-through;'>" + detail.IsStatus + "</span></td></tr>")
              .AppendLine("    <tr><th style='background-color:#e0f0ff;'>Người thao tác</th><td>" + detail.CreateBy + "</td></tr>")
              .AppendLine("    <tr><th style='background-color:#e0f0ff;'>Thời gian</th><td>" + detail.CreateAt + "</td></tr>")
              .AppendLine("  </tbody>")
              .AppendLine("</table>");
        ...
        private ApprovalTask CreateApprovalTask(string title, string content, CT_DM_HangMucCongViec input, string tableName, string parentMajorId, string majorId, string userId)
        {
        ...
        private ApprovalTask UpdateApprovalTask(string Id, string title, string content, CT_DM_HangMucCongViec input, string tableName, string parentMajorId, string majorId, string userId)
        {
            return new ApprovalTask
            {
                Id = Id,
