...
namespace DucAnh2025.Controllers.API.CongTrinh.DanhMuc
{
    ...
    [ApiController]
    public class DMHinhThucDayHoGaController : ControllerBase
    {
        private readonly IApprovalTaskRepository _approvalTaskService;
        private readonly IDM_HinhThucDayHoGaRepository _hinhThucDayHoGaCongService;
        ...
        public DMHinhThucDayHoGaController(IApprovalTaskRepository approvalTaskRepository, 
            IDM_HinhThucDayHoGaRepository dM_HinhThucDayHoGaRepository,
            ...
            IPermissionRepository permissionService)
        {
            ...
            _hinhThucDayHoGaCongService = dM_HinhThucDayHoGaRepository;
        ...
        public string tableName = "CT_DM_HinhThucDayHoGas";
        ...
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetAll(string groupId)
        {
            try
            {
                var list = await _hinhThucDayHoGaCongService.GetAll(groupId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetById(string id)
        {
            try
            {
                var result = await _hinhThucDayHoGaCongService.GetById(id);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetHistory(string id)
        {
            try
            {
                var result = await _hinhThucDayHoGaCongService.GetHistory(id);
                return Ok(new ApiResponse<object>(true, "Lưu thành công.", result));
        ...
        [HttpPost("Create")]
        public async Task<ActionResult<ApiResponse<object>>> Create([FromBody] CT_DM_HinhThucDayHoGa input, [FromQuery] bool isEdit)
        {
            try
            {
                ...
                if (editCheck)
                {
                    ...
                    // Check dữ liệu
                    if (!await _hinhThucDayHoGaCongService.CheckExclusive(new[] { input.Id }, now))
                    ...
                    await _hinhThucDayHoGaCongService.Update(input, "");
                    ...
                    {
                        await _approvalTaskService.Insert(CreateApprovalTask("Danh mục hình thức đậy hố ga - Sửa", ContentApp, input, tableName, parentMajorId, majorId, userId), userId);
                    }
                    else
                    {
                        await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Danh mục hình thức đậy hố ga - Sửa", ContentApp, input, tableName, parentMajorId, majorId, userId), userId);
                    }

                    // Gửi email
                    await InsertEmailHistories(content, "Danh mục hình thức đậy hố ga - Sửa.", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
                ...
                else
                {
                    ...
                    input.DateApproval = null;
                    input.ApprovalDept = null;
                    await _hinhThucDayHoGaCongService.Insert(input, "");
                    string content = await BuildEmailContentAdd(input.Id);
                    await _approvalTaskService.Insert(CreateApprovalTask("Danh mục hình thức đậy hố ga - Thêm mới", content, input, tableName, parentMajorId, majorId, userId), userId);

                    await InsertEmailHistories(content, "Danh mục hình thức đậy hố ga - Thêm", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            try
            {
                ...
                var query = await _hinhThucDayHoGaCongService.GetById(id);
                if (query == null)
                ...
                if (!await _hinhThucDayHoGaCongService.CheckDelete(query))
                ...
                if (!await _hinhThucDayHoGaCongService.CheckExclusive(new[] { id }, baseTime))
                ...
                await _hinhThucDayHoGaCongService.Update(query, "");
                ...
                await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Danh mục hình thức đậy hố ga - xóa", ContentApp, query, tableName, parentMajorId, majorId, userId), userId);

                // Hàm gửi email đã có → dùng lại
                await InsertEmailHistories(content, "Danh mục hình thức đậy hố ga - xóa", query.GroupId, groupId, firstApproval.Id, query.Id, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> Duyet(string id)
        {
            try
            {
                var user = _applicationUserService.GetCurrentUser();
                var userId = user.Id;
                var entity = await _hinhThucDayHoGaCongService.GetById(id);
                if (entity == null)
                ...
                if (entity.IsActive != 3)
                {
                    ...
                    await _hinhThucDayHoGaCongService.Approval(entity, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> HuyDuyet(string id)
        {
            try
            {
                ...
                if (entity.IsActive != 3)
                {
                    await _hinhThucDayHoGaCongService.NoApproval(entity, userId);
                    var dataNoApproval = await _hinhThucDayHoGaCongService.GetById(entity.Id);
        ...
        {
            var isValidModel = await _hinhThucDayHoGaCongService.GetHistoryIsValidEdit(id);
            var sb = new StringBuilder();

            sb.AppendLine("<h3>Thông tin sửa - Danh mục hình thức đậy hố ga</h3>")
        ...
        {
            var detail = await _hinhThucDayHoGaCongService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Thêm - Danh mục hình thức đậy hố ga</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
                .AppendLine("  <thead class=\"bg-info\">")
                .AppendLine("    <tr>")
                .AppendLine("     <th>Tên danh mục</th>")
        ...
        private async Task<string> BuildEmailContentDelete(string id)
        {
            var detail = await _hinhThucDayHoGaCongService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Xóa - Danh mục hình thức đậy hố ga</h3>")
                .AppendLine("  <thead class=\"bg-info\">")
                .AppendLine("    <tr>")
                .AppendLine("     <th>Tên danh mục</th>")
                .AppendLine("     <th>Trạng thái</th>")
        ...
        {
            var oldDataList = await _hinhThucDayHoGaCongService.GetHistoryIsValidEdit(id);
            var newData = await _hinhThucDayHoGaCongService.GetDetails(id);

            var sb = new StringBuilder();
            sb.AppendLine("<h3 style='color:#0d6efd;'>Thông tin sửa - Danh mục hình thức đậy hố ga</h3>")
              .AppendLine("<table class='table table-hover table-bordered' style='width:70%; border-collapse: collapse;'>")
              .AppendLine("  <tbody>");

            if (oldDataList.Any() && newData != null)
            {
                var oldData = oldDataList.FirstOrDefault();

                // --- Dữ liệu cũ ---
                sb.AppendLine("    <tr><th colspan='2' style='background-color:#f8d7da; text-align:center;'>DỮ LIỆU CŨ</th></tr>")
                  .AppendLine("    <tr><th style='width:35%; background-color:#f8d7da;'>Tên danh mục</th><td>" + (oldData?.TenDanhMuc ?? "") + "</td></tr>")
                  .AppendLine("    <tr><th style='background-color:#f8d7da;'>Trạng thái</th><td>" + (oldData?.IsStatus ) + "</td></tr>")
                  .AppendLine("    <tr><th style='background-color:#f8d7da;'>Người thao tác</th><td>" + (oldData?.CreateBy ?? "") + "</td></tr>")
                  .AppendLine("    <tr><th style='background-color:#f8d7da;'>Thời gian</th><td>" + (oldData?.CreateAt ) + "</td></tr>");

                // --- Dữ liệu mới ---
                sb.AppendLine("    <tr><th colspan='2' style='background-color:#d1e7dd; text-align:center;'>DỮ LIỆU MỚI</th></tr>")
                  .AppendLine("    <tr><th style='background-color:#d1e7dd;'>Tên danh mục</th><td>" + (newData?.TenDanhMuc ?? "") + "</td></tr>")
                  .AppendLine("    <tr><th style='background-color:#d1e7dd;'>Trạng thái</th><td>" + (newData?.IsStatus ) + "</td></tr>")
                  .AppendLine("    <tr><th style='background-color:#d1e7dd;'>Người thao tác</th><td>" + (newData?.CreateBy ?? "") + "</td></tr>")
                  .AppendLine("    <tr><th style='background-color:#d1e7dd;'>Thời gian</th><td>" + (newData?.CreateAt) + "</td></tr>");
            }
            else
            {
        ...
        private async Task<string> BuildEmailContentAppAdd(string id)
        {
            ...
            sb.AppendLine("<h3>Thêm - Danh mục hình thức đậy hố ga</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\" style=\"width: 60%; text-align: left;\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("      <th style=\"width: 30%; text-align: center;\">Tên trường</th>")
              .AppendLine("      <th style=\"text-align: center;\">Giá trị</th>")
              .AppendLine("    </tr>")
              .AppendLine("  </thead>")
              .AppendLine("  <tbody>")
              .AppendLine("    <tr>")
              .AppendLine("      <td><b>Tên danh mục</b></td>")
              .AppendLine($"     <td>{detail?.TenDanhMuc ?? ""}</td>")
              .AppendLine("    </tr>")
              .AppendLine("    <tr>")
              .AppendLine("      <td><b>Trạng thái</b></td>")
              .AppendLine($"     <td>{detail?.IsStatus}</td>")
              .AppendLine("    </tr>")
              .AppendLine("    <tr>")
              .AppendLine("      <td><b>Người thao tác</b></td>")
              .AppendLine($"     <td>{detail?.CreateBy}</td>")
              .AppendLine("    </tr>")
              .AppendLine("    <tr>")
              .AppendLine("      <td><b>Thời gian</b></td>")
              .AppendLine($"     <td>{detail?.CreateAt}</td>")
              .AppendLine("    </tr>")
              .AppendLine("  </tbody>")
              .AppendLine("</table>");
        ...
        private ApprovalTask CreateApprovalTask(string title, string content, CT_DM_HinhThucDayHoGa input, string tableName, string parentMajorId, string majorId, string userId)
        {
        ...
        private ApprovalTask UpdateApprovalTask(string Id, string title, string content, CT_DM_HinhThucDayHoGa input, string tableName, string parentMajorId, string majorId, string userId)
        {
            return new ApprovalTask
            {
                Id = Id,
