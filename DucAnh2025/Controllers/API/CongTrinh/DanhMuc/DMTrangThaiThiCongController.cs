...
namespace DucAnh2025.Controllers.API.CongTrinh.DanhMuc
{
    ...
    [ApiController]
    public class DMTrangThaiThiCongController : ControllerBase
    {
        ...
        public DMTrangThaiThiCongController(IApprovalTaskRepository approvalTaskRepository, 
            IDM_TrangThaiThiCongRepository _TrangThaiThiCongRepository,
            ...
            IPermissionRepository permissionService)
        {
            ...
            _trangThaiThiCongService = _TrangThaiThiCongRepository;
        ...
        public string tableName = "CT_DM_TrangThaiThiCongs";
        ...
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetAll(string groupId)
        {
            try
            {
                var list = await _trangThaiThiCongService.GetAll(groupId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetById(string id)
        {
            try
            {
                var result = await _trangThaiThiCongService.GetById(id);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetHistory(string id)
        {
            try
            {
                var result = await _trangThaiThiCongService.GetHistory(id);
                return Ok(new ApiResponse<object>(true, "Lưu thành công.", result));
        ...
        [HttpPost("Create")]
        public async Task<ActionResult<ApiResponse<object>>> Create([FromBody] CT_DM_TrangThaiThiCong input, [FromQuery] bool isEdit)
        {
            try
            {
                ...
                if (editCheck)
                {
                    ...
                    // Check dữ liệu
                    if (!await _trangThaiThiCongService.CheckExclusive(new[] { input.Id }, now))
                    ...
                    await _trangThaiThiCongService.Update(input, "");
                    ...
                    {
                        await _approvalTaskService.Insert(CreateApprovalTask("Danh mục trạng thái thi công - Sửa", contentApp, input, tableName, parentMajorId, majorId, userId), userId);
                    }
                    else
                    {
                        await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Danh mục trạng thái thi công - Sửa", contentApp, input, tableName, parentMajorId, majorId, userId), userId);
                    }
                    
                    // Gửi email
                    await InsertEmailHistories(content, "Danh mục trạng thái thi công - Sửa.", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
                ...
                else
                {
                    ...
                    input.DateApproval = null;
                    input.ApprovalDept = null;
                    await _trangThaiThiCongService.Insert(input, "");
                    string content = await BuildEmailContentAdd(input.Id);
                    string contentApp = await BuildEmailContentAppAdd(input.Id);
                    await _approvalTaskService.Insert(CreateApprovalTask("Danh mục trạng thái thi công - Thêm mới", contentApp, input, tableName, parentMajorId, majorId, userId), userId);

                    await InsertEmailHistories(content, "Danh mục trạng thái thi công - Thêm", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            try
            {
                ...
                var query = await _trangThaiThiCongService.GetById(id);
                if (query == null)
                ...
                if (!await _trangThaiThiCongService.CheckDelete(query))
                ...
                if (!await _trangThaiThiCongService.CheckExclusive(new[] { id }, baseTime))
                ...
                await _trangThaiThiCongService.Update(query, "");
                ...
                if (dataApproval == null)
                {
                    await _approvalTaskService.Insert(CreateApprovalTask("Danh mục trạng thái thi công - xóa", contentApp, query, tableName, parentMajorId, majorId, userId), userId);
                }
                else
                {
                    await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Danh mục trạng thái thi công - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }
                // Hàm gửi email đã có → dùng lại
                await InsertEmailHistories(content, "Danh mục trạng thái thi công - xóa", query.GroupId, groupId, firstApproval.Id, query.Id, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> Duyet(string id)
        {
            try
            {
                var user = _applicationUserService.GetCurrentUser();
                var userId = user.Id;
                var entity = await _trangThaiThiCongService.GetById(id);
                if (entity == null)
                ...
                if (entity.IsActive != 3)
                {
                    ...
                    await _trangThaiThiCongService.Approval(entity, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> HuyDuyet(string id)
        {
            try
            {
                ...
                if (entity.IsActive != 3)
                {
                    await _trangThaiThiCongService.NoApproval(entity, userId);
                    var dataNoApproval = await _trangThaiThiCongService.GetById(entity.Id);
        ...
        {
            var isValidModel = await _trangThaiThiCongService.GetHistoryIsValidEdit(id);
            var sb = new StringBuilder();

            sb.AppendLine("<h3>Thông tin sửa - Danh mục trạng thái thi công</h3>")
        ...
        {
            var detail = await _trangThaiThiCongService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Thêm - Danh mục trạng thái thi công</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
                .AppendLine("  <thead class=\"bg-info\">")
                .AppendLine("    <tr>")
                .AppendLine("     <th>Tên danh mục</th>")
        ...
        private async Task<string> BuildEmailContentDelete(string id)
        {
            var detail = await _trangThaiThiCongService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Xóa - Danh mục trạng thái thi công</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
                .AppendLine("  <thead class=\"bg-info\">")
                .AppendLine("    <tr>")
                .AppendLine("     <th>Tên danh mục</th>")
        ...
        {
            var isValidModel = await _trangThaiThiCongService.GetHistoryIsValidEdit(id);
            var newData = await _trangThaiThiCongService.GetDetails(id);
            var sb = new StringBuilder();
            ...
            if (isValidModel.Any())
            {
                ...
                // --- Dữ liệu mới ---
                sb.AppendLine("    <tr style='background-color:#e6f7ff;'>")
                  .AppendLine("      <td class=\"text-center\">Dữ liệu mới</td>")
                  .AppendLine($"      <td>{newData?.TenDanhMuc ?? ""}</td>")
        ...
        private ApprovalTask CreateApprovalTask(string title, string content, CT_DM_TrangThaiThiCong input, string tableName, string parentMajorId, string majorId, string userId)
        {
        ...
        private ApprovalTask UpdateApprovalTask(string Id, string title, string content, CT_DM_TrangThaiThiCong input, string tableName, string parentMajorId, string majorId, string userId)
        {
            return new ApprovalTask
            {
                Id = Id,
