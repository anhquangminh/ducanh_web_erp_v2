...
namespace DucAnh2025.Controllers.API.CongTrinh.DanhMuc
{
    ...
    [ApiController]
    public class DMLyTrinhController : ControllerBase
    {
        ...
        public DMLyTrinhController(IApprovalTaskRepository approvalTaskRepository,
        ...
        public string tableName = "CT_DM_LyTrinhs";
        ...
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetAll(string groupId)
        {
            try
            {
                var list = await _lyTrinhService.GetAll(groupId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetById(string id)
        {
            try
            {
                var result = await _lyTrinhService.GetById(id);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetHistory(string id)
        {
            try
            {
                var result = await _lyTrinhService.GetHistory(id);
                return Ok(new ApiResponse<object>(true, "Lưu thành công.", result));
        ...
        [HttpPost("Create")]
        public async Task<ActionResult<ApiResponse<object>>> Create([FromBody] CT_DM_LyTrinh input, [FromQuery] bool isEdit)
        {
            try
            {
                ...
                if (editCheck)
                {
                    ...
                    // Check dữ liệu
                    if (!await _lyTrinhService.CheckExclusive(new[] { input.Id }, now))
                    ...
                    await _lyTrinhService.Update(input, "");
                    ...
                    {
                        await _approvalTaskService.Insert(CreateApprovalTask("Danh mục lý trình - Sửa", contentApp, input, tableName, parentMajorId, majorId, userId), userId);
                    }
                    else
                    {
                        await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Danh mục lý trình - Sửa", contentApp, input, tableName, parentMajorId, majorId, userId), userId);
                    }
                   
                    // Gửi email
                    await InsertEmailHistories(content, "Danh mục lý trình - Sửa.", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
                ...
                else
                {
                    ...
                    input.DateApproval = null;
                    input.ApprovalDept = null;
                    await _lyTrinhService.Insert(input, "");
                    string content = await BuildEmailContentAdd(input.Id);
                    string contentApp = await BuildEmailContentAppAdd(input.Id);
                    await _approvalTaskService.Insert(CreateApprovalTask("Danh mục lý trình - Thêm mới", contentApp, input, tableName, parentMajorId, majorId, userId), userId);

                    await InsertEmailHistories(content, "Danh mục lý trình - Thêm", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            try
            {
                ...
                var query = await _lyTrinhService.GetById(id);
                if (query == null)
                ...
                if (!await _lyTrinhService.CheckDelete(query))
                ...
                if (!await _lyTrinhService.CheckExclusive(new[] { id }, baseTime))
                ...
                await _lyTrinhService.Update(query, "");
                ...
                if (dataApproval == null)
                {
                    await _approvalTaskService.Insert(CreateApprovalTask("Danh mục lý trình - xóa", contentApp, query, tableName, parentMajorId, majorId, userId), userId);
                }
                else
                {
                    await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Danh mục lý trình - xóa", contentApp, query, tableName, parentMajorId, majorId, userId), userId);
                }
                
                // Hàm gửi email đã có → dùng lại
                await InsertEmailHistories(content, "Danh mục lý trình - xóa", query.GroupId, groupId, firstApproval.Id, query.Id, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> Duyet(string id)
        {
            try
            {
                var user = _applicationUserService.GetCurrentUser();
                var userId = user.Id;
                var entity = await _lyTrinhService.GetById(id);
                if (entity == null)
                ...
                if (entity.IsActive != 3)
                {
                    ...
                    await _lyTrinhService.Approval(entity, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> HuyDuyet(string id)
        {
            try
            {
                ...
                if (entity.IsActive != 3)
                {
                    await _lyTrinhService.NoApproval(entity, userId);
                    var dataNoApproval = await _lyTrinhService.GetById(entity.Id);
        ...
        {
            var isValidModel = await _lyTrinhService.GetHistoryIsValidEdit(id);
            var sb = new StringBuilder();

            sb.AppendLine("<h3>Thông tin sửa - Danh mục lý trình</h3>")
        ...
        {
            var detail = await _lyTrinhService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Thêm - Danh mục lý trình</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
                .AppendLine("  <thead class=\"bg-info\">")
                .AppendLine("    <tr>")
                .AppendLine("     <th>Tên danh mục</th>")
        ...
        private async Task<string> BuildEmailContentDelete(string id)
        {
            var detail = await _lyTrinhService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Xóa - Danh mục lý trình</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
                .AppendLine("  <thead class=\"bg-info\">")
                .AppendLine("    <tr>")
                .AppendLine("     <th>Tên danh mục</th>")
        ...
        private async Task<string> BuildEmailContentAppEdit(string id)
        {
            ...
            sb.AppendLine("<h3>Thông tin sửa - Danh mục lý trình</h3>")
              .AppendLine("<table class='table table-hover table-bordered'>")
              .AppendLine("<thead class='bg-info'>")
              .AppendLine("<tr><th>Trường dữ liệu</th><th>Dữ liệu cũ</th><th>Dữ liệu mới</th></tr>")
              .AppendLine("</thead><tbody>");

            if (isValidModel.Any())
            {
                var oldData = isValidModel.FirstOrDefault();
                var newData = await _lyTrinhService.GetDetails(id);

                sb.AppendLine("<tr><td><b>Tên danh mục</b></td>")
                  .AppendLine($"<td>{oldData?.TenDanhMuc ?? ""}</td>")
                  .AppendLine($"<td>{newData?.TenDanhMuc ?? ""}</td></tr>");

                sb.AppendLine("<tr><td><b>Trạng thái</b></td>")
                  .AppendLine($"<td>{oldData?.IsStatus}</td>")
                  .AppendLine($"<td>{newData?.IsStatus}</td></tr>");

                sb.AppendLine("<tr><td><b>Người thao tác</b></td>")
                  .AppendLine($"<td>{oldData?.CreateBy}</td>")
                  .AppendLine($"<td>{newData?.CreateBy}</td></tr>");

                sb.AppendLine("<tr><td><b>Thời gian</b></td>")
                  .AppendLine($"<td>{oldData?.CreateAt}</td>")
                  .AppendLine($"<td>{newData?.CreateAt}</td></tr>");
            }
            else
            {
        ...
        private async Task<string> BuildEmailContentAppAdd(string id)
        {
            ...
            sb.AppendLine("<h3>Thêm - Danh mục lý trình</h3>")
              .AppendLine("<table class='table table-hover table-bordered'>")
              .AppendLine("<tbody>")
              .AppendLine("<tr><td><b>Tên danh mục</b></td><td>" + detail.TenDanhMuc + "</td></tr>")
              .AppendLine("<tr><td><b>Trạng thái</b></td><td>" + detail.IsStatus + "</td></tr>")
              .AppendLine("<tr><td><b>Người thao tác</b></td><td>" + detail.CreateBy + "</td></tr>")
              .AppendLine("<tr><td><b>Thời gian</b></td><td>" + detail.CreateAt + "</td></tr>")
              .AppendLine("</tbody></table>");
        ...
        private ApprovalTask CreateApprovalTask(string title, string content, CT_DM_LyTrinh input, string tableName, string parentMajorId, string majorId, string userId)
        {
        ...
        private ApprovalTask UpdateApprovalTask(string Id, string title, string content, CT_DM_LyTrinh input, string tableName, string parentMajorId, string majorId, string userId)
        {
            return new ApprovalTask
            {
                Id = Id,
