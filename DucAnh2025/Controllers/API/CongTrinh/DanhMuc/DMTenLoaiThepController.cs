...
namespace DucAnh2025.Controllers.API.CongTrinh.DanhMuc
{
    ...
    [ApiController]
    public class DMTenLoaiThepController : ControllerBase
    {
        private readonly IApprovalTaskRepository _approvalTaskService;
        private readonly IDM_TenLoaiThepRepository _tenLoaiThepCongService;
        ...
        public DMTenLoaiThepController(IApprovalTaskRepository approvalTaskRepository, 
            IDM_TenLoaiThepRepository dM_TenLoaiThepRepository,
            ...
            IPermissionRepository permissionService)
        {
            ...
            _tenLoaiThepCongService = dM_TenLoaiThepRepository;
        ...
        public string tableName = "CT_DM_TenLoaiTheps";
        ...
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetAll(string groupId)
        {
            try
            {
                var list = await _tenLoaiThepCongService.GetAll(groupId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetById(string id)
        {
            try
            {
                var result = await _tenLoaiThepCongService.GetById(id);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetHistory(string id)
        {
            try
            {
                var result = await _tenLoaiThepCongService.GetHistory(id);
                return Ok(new ApiResponse<object>(true, "Lưu thành công.", result));
        ...
        [HttpPost("Create")]
        public async Task<ActionResult<ApiResponse<object>>> Create([FromBody] CT_DM_TenLoaiThep input, [FromQuery] bool isEdit)
        {
            try
            {
                ...
                if (editCheck)
                {
                    ...
                    // Check dữ liệu
                    if (!await _tenLoaiThepCongService.CheckExclusive(new[] { input.Id }, now))
                    ...
                    await _tenLoaiThepCongService.Update(input, "");
                    ...
                    {
                        await _approvalTaskService.Insert(CreateApprovalTask("Danh mục tên loại thép - Sửa", contentApp, input, tableName, parentMajorId, majorId, userId), userId);
                    }
                    else
                    {
                        await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Danh mục tên loại thép - Sửa", contentApp, input, tableName, parentMajorId, majorId, userId), userId);
                    }
                    
                    // Gửi email
                    await InsertEmailHistories(content, "Danh mục tên loại thép - Sửa.", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
                ...
                else
                {
                    ...
                    input.DateApproval = null;
                    input.ApprovalDept = null;
                    await _tenLoaiThepCongService.Insert(input, "");
                    string content = await BuildEmailContentAdd(input.Id);
                    await _approvalTaskService.Insert(CreateApprovalTask("Danh mục tên loại thép - Thêm mới", content, input, tableName, parentMajorId, majorId, userId), userId);

                    await InsertEmailHistories(content, "Danh mục tên loại thép - Thêm", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            try
            {
                ...
                var query = await _tenLoaiThepCongService.GetById(id);
                if (query == null)
                ...
                if (!await _tenLoaiThepCongService.CheckDelete(query))
                ...
                if (!await _tenLoaiThepCongService.CheckExclusive(new[] { id }, baseTime))
                ...
                await _tenLoaiThepCongService.Update(query, "");
                ...
                if (dataApproval == null)
                {
                    await _approvalTaskService.Insert(CreateApprovalTask("Danh mục tên loại thép - xóa", contentApp, query, tableName, parentMajorId, majorId, userId), userId);
                }
                else
                {
                    await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Danh mục tên loại thép - xóa", contentApp, query, tableName, parentMajorId, majorId, userId), userId);
                }
               
                // Hàm gửi email đã có → dùng lại
                await InsertEmailHistories(content, "Danh mục tên loại thép - xóa", query.GroupId, groupId, firstApproval.Id, query.Id, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> Duyet(string id)
        {
            try
            {
                var user = _applicationUserService.GetCurrentUser();
                var userId = user.Id;
                var entity = await _tenLoaiThepCongService.GetById(id);
                if (entity == null)
                ...
                if (entity.IsActive != 3)
                {
                    ...
                    await _tenLoaiThepCongService.Approval(entity, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> HuyDuyet(string id)
        {
            try
            {
                ...
                if (entity.IsActive != 3)
                {
                    await _tenLoaiThepCongService.NoApproval(entity, userId);
                    var dataNoApproval = await _tenLoaiThepCongService.GetById(entity.Id);
        ...
        {
            var isValidModel = await _tenLoaiThepCongService.GetHistoryIsValidEdit(id);
            var sb = new StringBuilder();

            sb.AppendLine("<h3>Thông tin sửa - Danh mục tên loại thép</h3>")
        ...
        {
            var detail = await _tenLoaiThepCongService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Thêm - Danh mục tên loại thép</h3>")
            .AppendLine("<table class=\"table table-hover table-bordered\">")
            .AppendLine("  <thead class=\"bg-info\">")
            .AppendLine("    <tr>")
            .AppendLine("     <th>Tên danh mục</th>")
        ...
        private async Task<string> BuildEmailContentDelete(string id)
        {
            var detail = await _tenLoaiThepCongService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Xóa - Danh mục tên loại thép</h3>")
            .AppendLine("<table class=\"table table-hover table-bordered\">")
            .AppendLine("  <thead class=\"bg-info\">")
            .AppendLine("    <tr>")
            .AppendLine("     <th>Tên danh mục</th>")
        ...
        private async Task<string> BuildEmailContentAppEdit(string id)
        {
            ...
            sb.AppendLine("<h3>Thông tin sửa - Danh mục tên loại thép</h3>")
              ...
              .AppendLine("<tr><th style='width:30%;'>Trường dữ liệu</th><th>Dữ liệu cũ</th><th>Dữ liệu mới</th></tr>")
            ...
            if (isValidModel.Any())
            {
                ...
                sb.AppendLine($"<tr><td>Tên danh mục</td><td>{oldData?.TenDanhMuc ?? ""}</td><td>{newData?.TenDanhMuc ?? ""}</td></tr>")
        ...
        private async Task<string> BuildEmailContentAppAdd(string id)
        {
            ...
            sb.AppendLine("<h3>Thêm - Danh mục tên loại thép</h3>")
              .AppendLine("<table border='1' cellspacing='0' cellpadding='5' style='border-collapse: collapse; text-align: left; width: 100%;'>")
              .AppendLine("<thead style='background-color: #e0f0ff; text-align:center;'>")
              .AppendLine("<tr><th style='width:30%;'>Trường dữ liệu</th><th>Giá trị</th></tr>")
              .AppendLine("</thead><tbody>")
              .AppendLine($"<tr><td>Tên danh mục</td><td>{detail?.TenDanhMuc ?? ""}</td></tr>")
        ...
        private async Task<string> BuildEmailContentAppDelete(string id)
        {
            ...
            sb.AppendLine("<h3>Xóa - Danh mục tên loại thép</h3>")
              ...
              .AppendLine("<tr><th style='width:30%;'>Trường dữ liệu</th><th>Giá trị đã xóa</th></tr>")
              .AppendLine("</thead><tbody>")
              .AppendLine($"<tr><td>Tên danh mục</td><td>{detail?.TenDanhMuc ?? ""}</td></tr>")
              .AppendLine($"<tr><td>Trạng thái</td><td>{detail?.IsStatus}</td></tr>")
              .AppendLine($"<tr><td>Người thao tác</td><td>{detail?.CreateBy}</td></tr>")
        ...
        private ApprovalTask CreateApprovalTask(string title, string content, CT_DM_TenLoaiThep input, string tableName, string parentMajorId, string majorId, string userId)
        {
        ...
        private ApprovalTask UpdateApprovalTask(string Id, string title, string content, CT_DM_TenLoaiThep input, string tableName, string parentMajorId, string majorId, string userId)
        {
            return new ApprovalTask
            {
                Id = Id,
