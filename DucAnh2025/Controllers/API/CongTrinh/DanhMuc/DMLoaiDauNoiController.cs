...
namespace DucAnh2025.Controllers.API.CongTrinh.DanhMuc
{
    ...
    [ApiController]
    public class DMLoaiDauNoiController : ControllerBase
    {
        private readonly IApprovalTaskRepository _approvalTaskService;
        private readonly IDM_LoaiDauNoiRepository _loaiDauNoiCongService;
        ...
        public DMLoaiDauNoiController(IApprovalTaskRepository approvalTaskRepository, 
            IDM_LoaiDauNoiRepository _LoaiDauNoiRepository,
            ...
            IPermissionRepository permissionService)
        {
            ...
            _loaiDauNoiCongService = _LoaiDauNoiRepository;
        ...
        public string tableName = "CT_DM_LoaiDauNois";
        ...
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetAll(string groupId)
        {
            try
            {
                var list = await _loaiDauNoiCongService.GetAll(groupId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetById(string id)
        {
            try
            {
                var result = await _loaiDauNoiCongService.GetById(id);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetHistory(string id)
        {
            try
            {
                var result = await _loaiDauNoiCongService.GetHistory(id);
                return Ok(new ApiResponse<object>(true, "Lưu thành công.", result));
        ...
        [HttpPost("Create")]
        public async Task<ActionResult<ApiResponse<object>>> Create([FromBody] CT_DM_LoaiDauNoi input, [FromQuery] bool isEdit)
        {
            try
            {
                ...
                if (editCheck)
                {
                    ...
                    // Check dữ liệu
                    if (!await _loaiDauNoiCongService.CheckExclusive(new[] { input.Id }, now))
                    ...
                    await _loaiDauNoiCongService.Update(input, "");
                    ...
                    {
                        await _approvalTaskService.Insert(CreateApprovalTask("Danh mục loại đấu nối - Sửa", contentApp, input, tableName, parentMajorId, majorId, userId), userId);
                    }
                    else
                    {
                        await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Danh mục loại đấu nối - Sửa", contentApp, input, tableName, parentMajorId, majorId, userId), userId);
                    }
                    
                    // Gửi email
                    await InsertEmailHistories(content, "Danh mục loại đấu nối - Sửa.", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
                ...
                else
                {
                    ...
                    input.DateApproval = null;
                    input.ApprovalDept = null;
                    await _loaiDauNoiCongService.Insert(input, "");
                    string content = await BuildEmailContentAdd(input.Id);
                    string contentApp = await BuildEmailContentAppAdd(input.Id);
                    await _approvalTaskService.Insert(CreateApprovalTask("Danh mục loại đấu nối - Thêm mới", contentApp, input, tableName, parentMajorId, majorId, userId), userId);

                    await InsertEmailHistories(content, "Danh mục loại đấu nối - Thêm", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            try
            {
                ...
                var query = await _loaiDauNoiCongService.GetById(id);
                if (query == null)
                ...
                if (!await _loaiDauNoiCongService.CheckDelete(query))
                ...
                if (!await _loaiDauNoiCongService.CheckExclusive(new[] { id }, baseTime))
                ...
                await _loaiDauNoiCongService.Update(query, "");
                ...
                if (dataApproval == null)
                {
                    await _approvalTaskService.Insert(CreateApprovalTask("Danh mục loại đấu nối - xóa", contentApp, query, tableName, parentMajorId, majorId, userId), userId);
                }
                else
                {
                    await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Danh mục loại đấu nối - xóa", contentApp, query, tableName, parentMajorId, majorId, userId), userId);
                }
                
                // Hàm gửi email đã có → dùng lại
                await InsertEmailHistories(content, "Danh mục loại đấu nối - xóa", query.GroupId, groupId, firstApproval.Id, query.Id, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> Duyet(string id)
        {
            try
            {
                var user = _applicationUserService.GetCurrentUser();
                var userId = user.Id;
                var entity = await _loaiDauNoiCongService.GetById(id);
                if (entity == null)
                ...
                if (entity.IsActive != 3)
                {
                    ...
                    await _loaiDauNoiCongService.Approval(entity, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> HuyDuyet(string id)
        {
            try
            {
                ...
                if (entity.IsActive != 3)
                {
                    await _loaiDauNoiCongService.NoApproval(entity, userId);
                    var dataNoApproval = await _loaiDauNoiCongService.GetById(entity.Id);
        ...
        {
            var isValidModel = await _loaiDauNoiCongService.GetHistoryIsValidEdit(id);
            var sb = new StringBuilder();

            sb.AppendLine("<h3>Thông tin sửa - Danh mục loại đấu nối</h3>")
        ...
        {
            var detail = await _loaiDauNoiCongService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Thêm - Danh mục loại đấu nối</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
                .AppendLine("  <thead class=\"bg-info\">")
                .AppendLine("    <tr>")
                .AppendLine("     <th>Tên danh mục</th>")
        ...
        private async Task<string> BuildEmailContentDelete(string id)
        {
            var detail = await _loaiDauNoiCongService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Xóa - Danh mục loại đấu nối</h3>")
            .AppendLine("<table class=\"table table-hover table-bordered\">")
            .AppendLine("  <thead class=\"bg-info\">")
            .AppendLine("    <tr>")
            .AppendLine("     <th>Tên danh mục</th>")
        ...
        {
            var isValidModel = await _loaiDauNoiCongService.GetHistoryIsValidEdit(id);
            var newData = await _loaiDauNoiCongService.GetDetails(id); 
            var sb = new StringBuilder();
        ...
        private ApprovalTask CreateApprovalTask(string title, string content, CT_DM_LoaiDauNoi input, string tableName, string parentMajorId, string majorId, string userId)
        {
        ...
        private ApprovalTask UpdateApprovalTask(string Id, string title, string content, CT_DM_LoaiDauNoi input, string tableName, string parentMajorId, string majorId, string userId)
        {
            return new ApprovalTask
            {
                Id = Id,
