...
namespace DucAnh2025.Controllers.API.CongTrinh.DanhMuc
{
    ...
    [ApiController]
    public class DMLoaiCauKienController : ControllerBase
    {
        ...
        public DMLoaiCauKienController(IApprovalTaskRepository approvalTaskRepository, 
        ...
        public string tableName = "CT_DM_LoaiCauKiens";
        ...
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetAll(string groupId)
        {
            try
            {
                var list = await _loaiCauKienService.GetAll(groupId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetById(string id)
        {
            try
            {
                var result = await _loaiCauKienService.GetById(id);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetHistory(string id)
        {
            try
            {
                var result = await _loaiCauKienService.GetHistory(id);
                return Ok(new ApiResponse<object>(true, "Lưu thành công.", result));
        ...
        [HttpPost("Create")]
        public async Task<ActionResult<ApiResponse<object>>> Create([FromBody] CT_DM_LoaiCauKien input, [FromQuery] bool isEdit)
        {
            try
            {
                ...
                if (editCheck)
                {
                    ...
                    // Check dữ liệu
                    if (!await _loaiCauKienService.CheckExclusive(new[] { input.Id }, now))
                    ...
                    await _loaiCauKienService.Update(input, "");
                    ...
                    {
                        await _approvalTaskService.Insert(CreateApprovalTask("Danh mục loại cấu kiện - Sửa", contentApp, input, tableName, parentMajorId, majorId, userId), userId);
                    }
                    else
                    {
                        await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Danh mục loại cấu kiện - Sửa", contentApp, input, tableName, parentMajorId, majorId, userId), userId);
                    }

                    // Gửi email
                    await InsertEmailHistories(content, "Danh mục loại cấu kiện - Sửa.", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
                ...
                else
                {
                    ...
                    input.DateApproval = null;
                    input.ApprovalDept = null;
                    await _loaiCauKienService.Insert(input, "");
                    string content = await BuildEmailContentAdd(input.Id);
                    string contentApp = await BuildEmailContentAppAdd(input.Id);
                    await _approvalTaskService.Insert(CreateApprovalTask("Danh mục loại cấu kiện - Thêm mới", contentApp, input, tableName, parentMajorId, majorId, userId), userId);

                    await InsertEmailHistories(content, "Danh mục loại cấu kiện - Thêm", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            try
            {
                ...
                var query = await _loaiCauKienService.GetById(id);
                if (query == null)
                ...
                if (!await _loaiCauKienService.CheckDelete(query))
                ...
                if (!await _loaiCauKienService.CheckExclusive(new[] { id }, baseTime))
                ...
                await _loaiCauKienService.Update(query, "");
                ...
                if (dataApproval == null)
                {
                    await _approvalTaskService.Insert(CreateApprovalTask("Danh mục loại cấu kiện - xóa", contentApp, query, tableName, parentMajorId, majorId, userId), userId);
                }
                else
                {
                    await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Danh mục loại cấu kiện - xóa", contentApp, query, tableName, parentMajorId, majorId, userId), userId);
                }

                // Hàm gửi email đã có → dùng lại
                await InsertEmailHistories(content, "Danh mục loại cấu kiện - xóa", query.GroupId, groupId, firstApproval.Id, query.Id, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> Duyet(string id)
        {
            try
            {
                var user = _applicationUserService.GetCurrentUser();
                var userId = user.Id;
                var entity = await _loaiCauKienService.GetById(id);
                if (entity == null)
                ...
                if (entity.IsActive != 3)
                {
                    ...
                    await _loaiCauKienService.Approval(entity, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> HuyDuyet(string id)
        {
            try
            {
                ...
                if (entity.IsActive != 3)
                {
                    await _loaiCauKienService.NoApproval(entity, userId);
                    var dataNoApproval = await _loaiCauKienService.GetById(entity.Id);
        ...
        {
            var isValidModel = await _loaiCauKienService.GetHistoryIsValidEdit(id);
            var sb = new StringBuilder();

            sb.AppendLine("<h3>Thông tin sửa - Danh mục loại cấu kiện</h3>")
        ...
        {
            var detail = await _loaiCauKienService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Thêm - Danh mục loại cấu kiện</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
                .AppendLine("  <thead class=\"bg-info\">")
                .AppendLine("    <tr>")
                .AppendLine("     <th>Tên danh mục</th>")
        ...
        private async Task<string> BuildEmailContentDelete(string id)
        {
            var detail = await _loaiCauKienService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Xóa - Danh mục loại cấu kiện</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
                .AppendLine("  <thead class=\"bg-info\">")
                .AppendLine("    <tr>")
                .AppendLine("     <th>Tên danh mục</th>")
        ...
        {
            var isValidModel = await _loaiCauKienService.GetHistoryIsValidEdit(id);
            var newData = await _loaiCauKienService.GetDetails(id);

            var sb = new StringBuilder();
            sb.AppendLine("<h3>Thông tin sửa - Danh mục loại cấu kiện</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\" style=\"width: 70%; text-align: left;\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("      <th style=\"width: 35%; text-align: center;\">Tên trường</th>")
              .AppendLine("      <th style=\"text-align: center;\">Dữ liệu cũ</th>")
              .AppendLine("      <th style=\"text-align: center;\">Dữ liệu mới</th>")
              .AppendLine("    </tr>")
              .AppendLine("  </thead>")
              .AppendLine("  <tbody>");
        ...
        private async Task<string> BuildEmailContentAppDelete(string id)
        {
            ...
            sb.AppendLine("<h3>Xóa - Danh mục loại cấu kiện</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\" style=\"width: 70%; text-align: left;\">")
              .AppendLine("  <thead class=\"bg-danger text-white\">")
              .AppendLine("    <tr>")
              .AppendLine("      <th style=\"width: 35%; text-align: center;\">Tên trường</th>")
              .AppendLine("      <th style=\"text-align: center;\">Giá trị đã xóa</th>")
              .AppendLine("    </tr>")
              .AppendLine("  </thead>")
              .AppendLine("  <tbody>")
              .AppendLine("    <tr>")
        ...
        private ApprovalTask CreateApprovalTask(string title, string content, CT_DM_LoaiCauKien input, string tableName, string parentMajorId, string majorId, string userId)
        {
        ...
        private ApprovalTask UpdateApprovalTask(string Id, string title, string content, CT_DM_LoaiCauKien input, string tableName, string parentMajorId, string majorId, string userId)
        {
            return new ApprovalTask
            {
                Id = Id,
