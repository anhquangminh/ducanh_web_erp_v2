...
namespace DucAnh2025.Controllers.API.CongTrinh.DanhMuc
{
    ...
    [ApiController]
    public class DMHangMucKhoiLuongController : ControllerBase
    {
        ...
        public DMHangMucKhoiLuongController(IApprovalTaskRepository approvalTaskRepository, 
        ...
        public string tableName = "CT_DM_HangMucKhoiLuongs";
        ...
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetAll(string groupId)
        {
            try
            {
                var list = await _hangMucKhoiLuongService.GetAll(groupId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetById(string id)
        {
            try
            {
                var result = await _hangMucKhoiLuongService.GetById(id);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetHistory(string id)
        {
            try
            {
                var result = await _hangMucKhoiLuongService.GetHistory(id);
                return Ok(new ApiResponse<object>(true, "Lưu thành công.", result));
        ...
        [HttpPost("Create")]
        public async Task<ActionResult<ApiResponse<object>>> Create([FromBody] CT_DM_HangMucKhoiLuong input, [FromQuery] bool isEdit)
        {
            try
            {
                ...
                if (editCheck)
                {
                    ...
                    // Check dữ liệu
                    if (!await _hangMucKhoiLuongService.CheckExclusive(new[] { input.Id }, now))
                    ...
                    await _hangMucKhoiLuongService.Update(input, "");
                    ...
                    {
                        await _approvalTaskService.Insert(CreateApprovalTask("Danh mục hạng mục khối lượng - Sửa", contentApp, input, tableName, parentMajorId, majorId, userId), userId);
                    }
                    else
                    {
                        await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Danh mục hạng mục khối lượng - Sửa", contentApp, input, tableName, parentMajorId, majorId, userId), userId);
                    }
                    
                    // Gửi email
                    await InsertEmailHistories(content, "Danh mục hạng mục khối lượng - Sửa.", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
                ...
                else
                {
                    ...
                    input.DateApproval = null;
                    input.ApprovalDept = null;
                    await _hangMucKhoiLuongService.Insert(input, "");
                    string content = await BuildEmailContentAdd(input.Id);
                    string contentApp = await BuildEmailContentAppAdd(input.Id);
                    await _approvalTaskService.Insert(CreateApprovalTask("Danh mục hạng mục khối lượng - Thêm mới", contentApp, input, tableName, parentMajorId, majorId, userId), userId);

                    await InsertEmailHistories(content, "Danh mục hạng mục khối lượng - Thêm", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            try
            {
                ...
                var query = await _hangMucKhoiLuongService.GetById(id);
                if (query == null)
                ...
                if (!await _hangMucKhoiLuongService.CheckDelete(query))
                ...
                if (!await _hangMucKhoiLuongService.CheckExclusive(new[] { id }, baseTime))
                ...
                await _hangMucKhoiLuongService.Update(query, "");
                ...
                if (dataApproval == null)
                {
                    await _approvalTaskService.Insert(CreateApprovalTask("Danh mục hạng mục khối lượng - xóa", contentApp, query, tableName, parentMajorId, majorId, userId), userId);
                }
                else
                {
                    await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Danh mục hạng mục khối lượng - xóa", contentApp, query, tableName, parentMajorId, majorId, userId), userId);
                }

                // Hàm gửi email đã có → dùng lại
                await InsertEmailHistories(content, "Danh mục hạng mục khối lượng - xóa", query.GroupId, groupId, firstApproval.Id, query.Id, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> Duyet(string id)
        {
            try
            {
                var user = _applicationUserService.GetCurrentUser();
                var userId = user.Id;
                var entity = await _hangMucKhoiLuongService.GetById(id);
                if (entity == null)
                ...
                if (entity.IsActive != 3)
                {
                    ...
                    await _hangMucKhoiLuongService.Approval(entity, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> HuyDuyet(string id)
        {
            try
            {
                ...
                if (entity.IsActive != 3)
                {
                    await _hangMucKhoiLuongService.NoApproval(entity, userId);
                    var dataNoApproval = await _hangMucKhoiLuongService.GetById(entity.Id);
        ...
        {
            var isValidModel = await _hangMucKhoiLuongService.GetHistoryIsValidEdit(id);
            var sb = new StringBuilder();

            sb.AppendLine("<h3>Thông tin sửa - Danh mục hạng mục khối lượng</h3>")
        ...
        {
            var detail = await _hangMucKhoiLuongService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Thêm - Danh mục hạng mục khối lượng</h3>")
            .AppendLine("<table class=\"table table-hover table-bordered\">")
            .AppendLine("  <thead class=\"bg-info\">")
            .AppendLine("    <tr>")
            .AppendLine("     <th>Tên danh mục</th>")
        ...
        private async Task<string> BuildEmailContentDelete(string id)
        {
            var detail = await _hangMucKhoiLuongService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Xóa - Danh mục hạng mục khối lượng</h3>")
            .AppendLine("<table class=\"table table-hover table-bordered\">")
            .AppendLine("  <thead class=\"bg-info\">")
            .AppendLine("    <tr>")
            .AppendLine("     <th>Tên danh mục</th>")
        ...
        private async Task<string> BuildEmailContentAppEdit(string id)
        {
            ...
            {
                var oldData = isValidModel.FirstOrDefault();
                var newData = await _hangMucKhoiLuongService.GetDetails(id);

                sb.AppendLine("    <tr><th style='width:35%; background-color:#e0f0ff;'>Loại dữ liệu</th><td>Dữ liệu cũ</td><td>Dữ liệu mới</td></tr>");
                sb.AppendLine("    <tr><th style='background-color:#e0f0ff;'>Tên danh mục</th><td>" + (oldData?.TenDanhMuc ?? "") + "</td><td>" + (newData?.TenDanhMuc ?? "") + "</td></tr>");
                sb.AppendLine("    <tr><th style='background-color:#e0f0ff;'>Trạng thái</th><td>" + (oldData?.IsStatus ?? "") + "</td><td>" + (newData?.IsStatus ?? "") + "</td></tr>");
                sb.AppendLine("    <tr><th style='background-color:#e0f0ff;'>Người thao tác</th><td>" + (oldData?.CreateBy ?? "") + "</td><td>" + (newData?.CreateBy ?? "") + "</td></tr>");
                sb.AppendLine("    <tr><th style='background-color:#e0f0ff;'>Thời gian</th><td>" + (oldData?.CreateAt) + "</td><td>" + (newData?.CreateAt ) + "</td></tr>");
            }
            else
            {
                sb.AppendLine("    <tr><td class='text-danger' colspan='3'>Không có dữ liệu</td></tr>");
            }
        ...
        private async Task<string> BuildEmailContentAppDelete(string id)
        {
            ...
            sb.AppendLine("<h3 style='color:#c9302c;'>Xóa - Danh mục hạng mục khối lượng</h3>")
              .AppendLine("<table class='table table-hover table-bordered' style='width: 60%; border-collapse: collapse;'>")
              .AppendLine("  <tbody>")
              .AppendLine("    <tr><th style='width:35%; background-color:#f8d7da;'>Tên danh mục</th><td>" + detail.TenDanhMuc + "</td></tr>")
        ...
        private ApprovalTask CreateApprovalTask(string title, string content, CT_DM_HangMucKhoiLuong input, string tableName, string parentMajorId, string majorId, string userId)
        {
        ...
        private ApprovalTask UpdateApprovalTask(string Id, string title, string content, CT_DM_HangMucKhoiLuong input, string tableName, string parentMajorId, string majorId, string userId)
        {
            return new ApprovalTask
            {
                Id = Id,
