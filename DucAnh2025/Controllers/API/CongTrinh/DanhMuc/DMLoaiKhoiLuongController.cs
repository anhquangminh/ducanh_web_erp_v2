...
namespace DucAnh2025.Controllers.API.CongTrinh.DanhMuc
{
    ...
    [ApiController]
    public class DMLoaiKhoiLuongController : ControllerBase
    {
        ...
        public DMLoaiKhoiLuongController(IApprovalTaskRepository approvalTaskRepository, 
        ...
        public string tableName = "CT_DM_LoaiKhoiLuongs";
        ...
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetAll(string groupId)
        {
            try
            {
                var list = await _loaiKhoiLuongService.GetAll(groupId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetById(string id)
        {
            try
            {
                var result = await _loaiKhoiLuongService.GetById(id);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetHistory(string id)
        {
            try
            {
                var result = await _loaiKhoiLuongService.GetHistory(id);
                return Ok(new ApiResponse<object>(true, "Lưu thành công.", result));
        ...
        [HttpPost("Create")]
        public async Task<ActionResult<ApiResponse<object>>> Create([FromBody] CT_DM_LoaiKhoiLuong input, [FromQuery] bool isEdit)
        {
            try
            {
                ...
                if (editCheck)
                {
                    ...
                    // Check dữ liệu
                    if (!await _loaiKhoiLuongService.CheckExclusive(new[] { input.Id }, now))
                    ...
                    await _loaiKhoiLuongService.Update(input, "");
                    ...
                    {
                        await _approvalTaskService.Insert(CreateApprovalTask("Danh mục loại khối lượng - Sửa", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }
                    else
                    {
                        await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Danh mục loại khối lượng - Sửa", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }
                    
                    // Gửi email
                    await InsertEmailHistories(content, "Danh mục loại khối lượng - Sửa.", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
                ...
                else
                {
                    ...
                    input.DateApproval = null;
                    input.ApprovalDept = null;
                    await _loaiKhoiLuongService.Insert(input, "");
                    string content = await BuildEmailContentAdd(input.Id);
                    await _approvalTaskService.Insert(CreateApprovalTask("Danh mục loại khối lượng - Thêm mới", content, input, tableName, parentMajorId, majorId, userId), userId);

                    await InsertEmailHistories(content, "Danh mục loại khối lượng - Thêm", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            try
            {
                ...
                var query = await _loaiKhoiLuongService.GetById(id);
                if (query == null)
                ...
                if (!await _loaiKhoiLuongService.CheckDelete(query))
                ...
                if (!await _loaiKhoiLuongService.CheckExclusive(new[] { id }, baseTime))
                ...
                await _loaiKhoiLuongService.Update(query, "");

                var content = await BuildEmailContentDelete(id);
                var dataApproval = await _approvalTaskService.GetByOriginalId(id);
                if (dataApproval == null)
                {
                    await _approvalTaskService.Insert(CreateApprovalTask("Danh mục loại khối lượng - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }
                else
                {
                    await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Danh mục loại khối lượng - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }
                
                // Hàm gửi email đã có → dùng lại
                await InsertEmailHistories(content, "Danh mục loại khối lượng - xóa", query.GroupId, groupId, firstApproval.Id, query.Id, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> Duyet(string id)
        {
            try
            {
                var user = _applicationUserService.GetCurrentUser();
                var userId = user.Id;
                var entity = await _loaiKhoiLuongService.GetById(id);
                if (entity == null)
                ...
                if (entity.IsActive != 3)
                {
                    ...
                    await _loaiKhoiLuongService.Approval(entity, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> HuyDuyet(string id)
        {
            try
            {
                ...
                if (entity.IsActive != 3)
                {
                    await _loaiKhoiLuongService.NoApproval(entity, userId);
                    var dataNoApproval = await _loaiKhoiLuongService.GetById(entity.Id);
        ...
        {
            var isValidModel = await _loaiKhoiLuongService.GetHistoryIsValidEdit(id);
            var sb = new StringBuilder();

            sb.AppendLine("<h3>Thông tin sửa - Danh mục loại khối lượng</h3>")
        ...
        {
            var detail = await _loaiKhoiLuongService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Thêm - Danh mục loại khối lượng</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
                .AppendLine("  <thead class=\"bg-info\">")
                .AppendLine("    <tr>")
                .AppendLine("     <th>Tên danh mục</th>")
        ...
        private async Task<string> BuildEmailContentDelete(string id)
        {
            var detail = await _loaiKhoiLuongService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Xóa - Danh mục loại khối lượng</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
                .AppendLine("  <thead class=\"bg-info\">")
                .AppendLine("    <tr>")
                .AppendLine("     <th>Tên danh mục</th>")
        ...
        private ApprovalTask CreateApprovalTask(string title, string content, CT_DM_LoaiKhoiLuong input, string tableName, string parentMajorId, string majorId, string userId)
        {
        ...
        private ApprovalTask UpdateApprovalTask(string Id, string title, string content, CT_DM_LoaiKhoiLuong input, string tableName, string parentMajorId, string majorId, string userId)
        {
            return new ApprovalTask
            {
                Id = Id,
