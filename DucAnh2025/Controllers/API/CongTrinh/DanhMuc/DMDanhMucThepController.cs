...
namespace DucAnh2025.Controllers.API.CongTrinh.DanhMuc
{
    ...
    [ApiController]
    public class DMDanhMucThepController : ControllerBase
    {
        ...
        public DMDanhMucThepController(IApprovalTaskRepository approvalTaskRepository,
        ...
        public string tableName = "CT_DM_DanhMucTheps";

        [HttpPost("GetByVM")]
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetByVM(string groupId, [FromBody] DM_DanhMucThepModel input)
        {
            try
            {
                var list = await _danhMucThepService.GetAllByVM(input, groupId);
        ...
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetAll(string groupId)
        {
            try
            {
                var list = await _danhMucThepService.GetAll(groupId);
                return Ok(new ApiResponse<IEnumerable<object>>(true, "Thành công", list));
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetById(string id)
        {
            try
            {
                var result = await _danhMucThepService.GetById(id);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetHistory(string id)
        {
            try
            {
                var result = await _danhMucThepService.GetHistory(id);
                return Ok(new ApiResponse<object>(true, "Lưu thành công.", result));
        ...
        [HttpPost("Create")]
        public async Task<ActionResult<ApiResponse<object>>> Create([FromBody] CT_DM_DanhMucThep input, [FromQuery] bool isEdit)
        {
            try
            {
                ...
                if (editCheck)
                {
                    ...
                    // Check dữ liệu
                    if (!await _danhMucThepService.CheckExclusive(new[] { input.Id }, now))
                    ...
                    await _danhMucThepService.Update(input, "");
                    ...
                    {
                        await _approvalTaskService.Insert(CreateApprovalTask("Danh mục thép - Sửa", contentApp, input, tableName, parentMajorId, majorId, userId), userId);
                    }
                    else
                    {
                        await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Danh mục thép - Sửa", contentApp, input, tableName, parentMajorId, majorId, userId), userId);
                    }

                    // Gửi email
                    await InsertEmailHistories(content, "Danh mục thép - Sửa.", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
                ...
                else
                {
                    ...
                    input.DateApproval = null;
                    input.ApprovalDept = null;
                    await _danhMucThepService.Insert(input, "");
                    string content = await BuildEmailContentAdd(input.Id);
                    string contentApp = await BuildEmailContentAppAdd(input.Id);
                    await _approvalTaskService.Insert(CreateApprovalTask("Danh mục thép - Thêm mới", contentApp, input, tableName, parentMajorId, majorId, userId), userId);

                    await InsertEmailHistories(content, "Danh mục thép - Thêm", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            try
            {
                ...
                var query = await _danhMucThepService.GetById(id);
                if (query == null)
                ...
                if (!await _danhMucThepService.CheckDelete(query))
                ...
                if (!await _danhMucThepService.CheckExclusive(new[] { id }, baseTime))
                ...
                await _danhMucThepService.Update(query, "");
                ...
                if (dataApproval == null)
                {
                    await _approvalTaskService.Insert(CreateApprovalTask("Danh mục - xóa", contentApp, query, tableName, parentMajorId, majorId, userId), userId);
                }
                else
                {
                    await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Danh mục - xóa", contentApp, query, tableName, parentMajorId, majorId, userId), userId);
                }
               

                // Hàm gửi email đã có → dùng lại
                await InsertEmailHistories(content, "Danh mục - xóa", query.GroupId, groupId, firstApproval.Id, query.Id, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> Duyet(string id)
        {
            try
            {
                var user = _applicationUserService.GetCurrentUser();
                var userId = user.Id;
                var entity = await _danhMucThepService.GetById(id);
                if (entity == null)
                ...
                if (entity.IsActive != 3)
                {
                    ...
                    await _danhMucThepService.Approval(entity, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> HuyDuyet(string id)
        {
            try
            {
                ...
                if (entity.IsActive != 3)
                {
                    await _danhMucThepService.NoApproval(entity, userId);
                    var dataNoApproval = await _danhMucThepService.GetById(entity.Id);
        ...
        {
            var isValidModel = await _danhMucThepService.GetHistoryIsValidEdit(id);
            var sb = new StringBuilder();

            sb.AppendLine("<h3>Thông tin sửa - Danh mục thép</h3>")
              ...
              .AppendLine("      <th>Tên loại thép</th>")
              .AppendLine("      <th>Đường kính</th>")
              .AppendLine("      <th>Đơn vị</th>")
              .AppendLine("      <th>Trọng lượng</th>")
            ...
            if (isValidModel.Any())
            {
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{oldData?.DuongKinh ?? ""}</td>")
                  .AppendLine($"      <td>{oldData?.DonVi ?? ""}</td>")
                  .AppendLine($"      <td>{oldData?.TrongLuong ?? 0}</td>")
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{newData?.DuongKinh ?? ""}</td>")
                  .AppendLine($"      <td>{newData?.DonVi ?? ""}</td>")
                  .AppendLine($"      <td>{newData?.TrongLuong ?? 0}</td>")
        ...
        {
            var detail = await _danhMucThepService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Thêm - Danh mục thép</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Tên loại thép</th>")
              ...
              .AppendLine($"     <td>{detail.DuongKinh}</td>")
        ...
        private async Task<string> BuildEmailContentDelete(string id)
        {
            ...
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Xóa - Danh mục thép</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Tên loại thép</th>")
        ...
        private async Task<string> BuildEmailContentAppEdit(string id)
        {
            ...
            sb.AppendLine("<h3>Thông tin sửa - Danh mục thép</h3>")
              ...
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("      <th class=\"text-center\" style=\"width: 30%;\">Loại dữ liệu</th>")
              .AppendLine("      <th class=\"text-center\">Dữ liệu cũ</th>")
              .AppendLine("      <th class=\"text-center\">Dữ liệu mới</th>")
              .AppendLine("    </tr>")
              .AppendLine("  </thead>")
              .AppendLine("  <tbody>");
            ...
            {
                var oldData = isValidModel.FirstOrDefault();
                var newData = await _danhMucThepService.GetDetails(id);

                sb.AppendLine($"    <tr><td>Tên loại thép</td><td>{oldData?.TenLoaiThep ?? ""}</td><td>{newData?.TenLoaiThep ?? ""}</td></tr>")
                  .AppendLine($"    <tr><td>Đường kính</td><td>{oldData?.DuongKinh ?? ""}</td><td>{newData?.DuongKinh ?? ""}</td></tr>")
                  .AppendLine($"    <tr><td>Đơn vị</td><td>{oldData?.DonVi ?? ""}</td><td>{newData?.DonVi ?? ""}</td></tr>")
                  .AppendLine($"    <tr><td>Trọng lượng</td><td>{oldData?.TrongLuong ?? 0}</td><td>{newData?.TrongLuong ?? 0}</td></tr>")
        ...
        private async Task<string> BuildEmailContentAppAdd(string id)
        {
            ...
            sb.AppendLine("<h3>Thêm - Danh mục thép</h3>")
              ...
              .AppendLine("    <tr>")
              .AppendLine("      <th class=\"text-center\" style=\"width: 30%;\">Loại dữ liệu</th>")
              .AppendLine("      <th class=\"text-center\">Giá trị</th>")
              .AppendLine("    </tr>")
              .AppendLine("  </thead>")
              .AppendLine("  <tbody>")
              .AppendLine($"    <tr><td>Tên loại thép</td><td>{detail.TenLoaiThep}</td></tr>")
              .AppendLine($"    <tr><td>Đường kính</td><td>{detail.DuongKinh}</td></tr>")
              .AppendLine($"    <tr><td>Đơn vị</td><td>{detail.DonVi}</td></tr>")
              .AppendLine($"    <tr><td>Trọng lượng</td><td>{detail.TrongLuong}</td></tr>")
        ...
        private ApprovalTask CreateApprovalTask(string title, string content, CT_DM_DanhMucThep input, string tableName, string parentMajorId, string majorId, string userId)
        {
        ...
        private ApprovalTask UpdateApprovalTask(string Id,string title, string content, CT_DM_DanhMucThep input, string tableName, string parentMajorId, string majorId, string userId)
        {
            return new ApprovalTask
            {
                Id = Id,
