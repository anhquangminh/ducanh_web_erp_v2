...
namespace DucAnh2025.Controllers.API.DanhMuc
{
    ...
    [ApiController]
    public class KhoDMNhomNhienLieuController : ControllerBase
    {
        private readonly IApprovalTaskRepository _approvalTaskService;
        private readonly IKho_DM_NhomNhienLieuRepository _khodmnhomnhienlieuService;
        ...
        public KhoDMNhomNhienLieuController(IApprovalTaskRepository approvalTaskRepository,
            IKho_DM_NhomNhienLieuRepository khodmnhomnhienlieuRepository,
            ...
            IPermissionRepository permissionService)
        {
            ...
            _khodmnhomnhienlieuService = khodmnhomnhienlieuRepository;
        ...
        public string tableName = "Kho_DM_NhomNhienLieus";

        [HttpPost("GetByVM")]
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetByVM(string groupId, [FromBody] Kho_DM_NhomNhienLieuModel? input = null)
        {
            try
            {
                var model = input ?? new Kho_DM_NhomNhienLieuModel();
                var list = await _khodmnhomnhienlieuService.GetAllByVM(model, groupId);
        ...
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetAll(string groupId)
        {
            try
            {
                var list = await _khodmnhomnhienlieuService.GetAll(groupId);
                return Ok(new ApiResponse<IEnumerable<object>>(true, "Thành công", list));
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetById(string id)
        {
            try
            {
                var result = await _khodmnhomnhienlieuService.GetById(id);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetHistory(string id)
        {
            try
            {
                var result = await _khodmnhomnhienlieuService.GetHistory(id);
                return Ok(new ApiResponse<object>(true, "Lưu thành công .", result));
            }
            catch (Exception ex)
            {
        ...
        [HttpPost("Create")]
        public async Task<ActionResult<ApiResponse<object>>> Create([FromBody] Kho_DM_NhomNhienLieu input, [FromQuery] bool isEdit)
        {
            try
            {
                ...
                if (editCheck)
                {
                    ...
                    if (!await _khodmnhomnhienlieuService.CheckExclusive(new[] { input.Id }, now))
                    ...
                    await _khodmnhomnhienlieuService.Update(input, "");
                    ...
                    {
                        await _approvalTaskService.Insert(CreateApprovalTask("Nhóm nhiên liệu- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }
                    else
                    {
                        await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Nhóm nhiên liệu- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }

                    await InsertEmailHistories(content, "Nhóm nhiên liệu- sửa !", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
                ...
                else
                {
                    ...
                    input.DateApproval = null;
                    input.ApprovalDept = null;
                    await _khodmnhomnhienlieuService.Insert(input, "");
                    string content = await BuildEmailContentAdd(input.Id);
                    await _approvalTaskService.Insert(CreateApprovalTask("Nhóm nhiên liệuThêm mơí !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    await InsertEmailHistories(content, "Nhóm nhiên liệu - Thêm", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            try
            {
                ...
                var query = await _khodmnhomnhienlieuService.GetById(id);
                if (query == null)
                ...
                if (!await _khodmnhomnhienlieuService.CheckDelete(query))
                ...
                if (!await _khodmnhomnhienlieuService.CheckExclusive(new[] { id }, baseTime))
                ...
                await _khodmnhomnhienlieuService.Update(query, "");

                var content = await BuildEmailContentDelete(id);
                var dataApproval = await _approvalTaskService.GetByOriginalId(id);
                if (dataApproval == null)
                {
                    await _approvalTaskService.Insert(CreateApprovalTask("Nhóm nhiên liệu - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }
                else
                {
                    await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Nhóm nhiên liệu - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }

                await InsertEmailHistories(content, "Nhóm nhiên liệu - xóa", query.GroupId, groupId, firstApproval.Id, query.Id, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> Duyet(string id)
        {
            try
            {
                var user = _applicationUserService.GetCurrentUser();
                var userId = user.Id;
                var entity = await _khodmnhomnhienlieuService.GetById(id);
                if (entity == null)
                ...
                if (entity.IsActive != 3)
                {
                    ...
                    await _khodmnhomnhienlieuService.Approval(entity, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> HuyDuyet(string id)
        {
            try
            {
                ...
                if (entity.IsActive != 3)
                {
                    await _khodmnhomnhienlieuService.NoApproval(entity, userId);
                    var dataNoApproval = await _khodmnhomnhienlieuService.GetById(entity.Id);
        ...
        {
            var isValidModel = await _khodmnhomnhienlieuService.GetHistoryIsValidEdit(id);
            var sb = new StringBuilder();

            sb.AppendLine("Thông tin sửa KhoDMNhomNhienLieu</h3>")
             .AppendLine("<table class=\"table table-hover table-bordered\">")
             .AppendLine("  <thead class=\"bg-info\">")
             .AppendLine("    <tr>")
             .AppendLine("     <th>Tên nhiên liệu</th>")
            ...
            if (isValidModel.Any())
            {
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{oldData?.TenNhienLieu}</td>")
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{newData?.TenNhienLieu}</td>")
        ...
        {
            var detail = await _khodmnhomnhienlieuService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Thêm - KhoDMNhomNhienLieu</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Tên nhiên liệu</th>")
              ...
              .AppendLine("  <tbody>");
            sb.AppendLine($"    <tr>")
              .AppendLine($"     <td>{detail.TenNhienLieu}</td>")
        ...
        private async Task<string> BuildEmailContentDelete(string id)
        {
            var detail = await _khodmnhomnhienlieuService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Xóa - KhoDMNhomNhienLieu</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Tên nhiên liệu</th>")
        ...
        private ApprovalTask CreateApprovalTask(string title, string content, Kho_DM_NhomNhienLieu input, string tableName, string parentMajorId, string majorId, string userId)
        {
        ...
        private ApprovalTask UpdateApprovalTask(string Id, string title, string content, Kho_DM_NhomNhienLieu input, string tableName, string parentMajorId, string majorId, string userId)
        {
            return new ApprovalTask
            {
                Id = Id,
