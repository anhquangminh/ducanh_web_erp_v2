...
namespace DucAnh2025.Controllers.API
{
    ...
    [ApiController]
    public class KhoNhapKhoPhuTungController : ControllerBase
    {
        private readonly IApprovalTaskRepository _approvalTaskService;
        private readonly IKho_NhapKhoPhuTungRepository _khonhapkhophutungService;
        ...
        public KhoNhapKhoPhuTungController(IApprovalTaskRepository approvalTaskRepository,
            IKho_NhapKhoPhuTungRepository khonhapkhophutungRepository,
            ...
            IPermissionRepository permissionService)
        {
            ...
            _khonhapkhophutungService = khonhapkhophutungRepository;
        ...
        public string tableName = "Kho_NhapKhoPhuTungs";

        [HttpPost("GetByVM")]
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetByVM(string groupId, [FromBody] Kho_NhapKhoPhuTungModel? input = null)
        {
            try
            {
                var model = input ?? new Kho_NhapKhoPhuTungModel();
                var list = await _khonhapkhophutungService.GetAllByVM(model, groupId);
        ...
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetAll(string groupId)
        {
            try
            {
                var list = await _khonhapkhophutungService.GetAll(groupId);
                return Ok(new ApiResponse<IEnumerable<object>>(true, "Thành công", list));
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetById(string id)
        {
            try
            {
                var result = await _khonhapkhophutungService.GetById(id);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetHistory(string id)
        {
            try
            {
                var result = await _khonhapkhophutungService.GetHistory(id);
                return Ok(new ApiResponse<object>(true, "Lưu thành công .", result));
            }
            catch (Exception ex)
            {
        ...
        [HttpPost("Create")]
        public async Task<ActionResult<ApiResponse<object>>> Create([FromBody] Kho_NhapKhoPhuTung input, [FromQuery] bool isEdit)
        {
            try
            {
                ...
                if (editCheck)
                {
                    ...
                    if (!await _khonhapkhophutungService.CheckExclusive(new[] { input.Id }, now))
                    ...
                    await _khonhapkhophutungService.Update(input, "");
                    ...
                    {
                        await _approvalTaskService.Insert(CreateApprovalTask("Nhập kho phụ tùng- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }
                    else
                    {
                        await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Nhập kho phụ tùng- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }

                    await InsertEmailHistories(content, "Nhập kho phụ tùng- sửa !", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
                ...
                else
                {
                    ...
                    input.DateApproval = null;
                    input.ApprovalDept = null;
                    await _khonhapkhophutungService.Insert(input, "");
                    string content = await BuildEmailContentAdd(input.Id);
                    await _approvalTaskService.Insert(CreateApprovalTask("Nhập kho phụ tùngThêm mơí !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    await InsertEmailHistories(content, "Nhập kho phụ tùng - Thêm", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            try
            {
                ...
                var query = await _khonhapkhophutungService.GetById(id);
                if (query == null)
                ...
                if (!await _khonhapkhophutungService.CheckDelete(query))
                ...
                if (!await _khonhapkhophutungService.CheckExclusive(new[] { id }, baseTime))
                ...
                await _khonhapkhophutungService.Update(query, "");

                var content = await BuildEmailContentDelete(id);
                var dataApproval = await _approvalTaskService.GetByOriginalId(id);
                if (dataApproval == null)
                {
                    await _approvalTaskService.Insert(CreateApprovalTask("Nhập kho phụ tùng - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }
                else
                {
                    await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Nhập kho phụ tùng - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }

                await InsertEmailHistories(content, "Nhập kho phụ tùng - xóa", query.GroupId, groupId, firstApproval.Id, query.Id, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> Duyet(string id)
        {
            try
            {
                var user = _applicationUserService.GetCurrentUser();
                var userId = user.Id;
                var entity = await _khonhapkhophutungService.GetById(id);
                if (entity == null)
                ...
                if (entity.IsActive != 3)
                {
                    ...
                    await _khonhapkhophutungService.Approval(entity, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> HuyDuyet(string id)
        {
            try
            {
                ...
                if (entity.IsActive != 3)
                {
                    await _khonhapkhophutungService.NoApproval(entity, userId);
                    var dataNoApproval = await _khonhapkhophutungService.GetById(entity.Id);
        ...
        {
            var isValidModel = await _khonhapkhophutungService.GetHistoryIsValidEdit(id);
            var sb = new StringBuilder();

            sb.AppendLine("Thông tin sửa KhoNhapKhoPhuTung</h3>")
             .AppendLine("<table class=\"table table-hover table-bordered\">")
             .AppendLine("  <thead class=\"bg-info\">")
             .AppendLine("    <tr>")
             .AppendLine("     <th>Nơi nhập</th>")
             .AppendLine("     <th>Dùng cho thiết bị</th>")
             .AppendLine("     <th>Ngày nhập kho</th>")
             .AppendLine("     <th>Số phiếu</th>")
             .AppendLine("     <th>Số HĐ</th>")
             .AppendLine("     <th>Nhóm phụ tùng</th>")
             .AppendLine("     <th>Loại phụ tùng</th>")
             .AppendLine("     <th>Nhãn hiệu</th>")
             .AppendLine("     <th>Seri</th>")
             .AppendLine("     <th>Đơn vị</th>")
             .AppendLine("     <th>KL có thuế</th>")
             .AppendLine("     <th>KL không thuế</th>")
             ...
             .AppendLine("     <th>KL có thuế (quy đổi)</th>")
             .AppendLine("     <th>KL không thuế (quy đổi)</th>")
             .AppendLine("     <th>Đơn giá có thuế (quy đổi)</th>")
             .AppendLine("     <th>Đơn giá không thuế (quy đổi)</th>")
             .AppendLine("     <th>Thành tiền có thuế (quy đổi)</th>")
             .AppendLine("     <th>Thành tiền không thuế (quy đổi)</th>")
            ...
            if (isValidModel.Any())
            {
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{oldData?.Id_Tram}</td>")
                  .AppendLine($"      <td>{oldData?.DungChoThietBi}</td>")
                  .AppendLine($"      <td>{oldData?.NgayNhapKho}</td>")
                  .AppendLine($"      <td>{oldData?.SoPhieu}</td>")
                  .AppendLine($"      <td>{oldData?.SoHopDong}</td>")
                  .AppendLine($"      <td>{oldData?.Id_NhomPhuTung}</td>")
                  .AppendLine($"      <td>{oldData?.Id_LoaiPhuTung}</td>")
                  .AppendLine($"      <td>{oldData?.Id_NhanHieu}</td>")
                  .AppendLine($"      <td>{oldData?.Seri}</td>")
                  .AppendLine($"      <td>{oldData?.Id_DonVi}</td>")
                  .AppendLine($"      <td>{oldData?.KLCoThue}</td>")
                  .AppendLine($"      <td>{oldData?.KLKhongThue}</td>")
                  ...
                  .AppendLine($"      <td>{oldData?.DVSauQD}</td>")
                  .AppendLine($"      <td>{oldData?.KLCoThueQD}</td>")
                  .AppendLine($"      <td>{oldData?.KLKhongThueQD}</td>")
                  .AppendLine($"      <td>{oldData?.DonGiaCoThueQD}</td>")
                  .AppendLine($"      <td>{oldData?.DonGiaKhongThueQD}</td>")
                  .AppendLine($"      <td>{oldData?.ThanhTienCoThueQD}</td>")
                  .AppendLine($"      <td>{oldData?.ThanhTienKhongThueQD}</td>")
                  .AppendLine($"      <td>{oldData?.TongTien}</td>")
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{newData?.Id_Tram}</td>")
                  .AppendLine($"      <td>{newData?.DungChoThietBi}</td>")
                  .AppendLine($"      <td>{newData?.NgayNhapKho}</td>")
                  .AppendLine($"      <td>{newData?.SoPhieu}</td>")
                  .AppendLine($"      <td>{newData?.SoHopDong}</td>")
                  .AppendLine($"      <td>{newData?.Id_NhomPhuTung}</td>")
                  .AppendLine($"      <td>{newData?.Id_LoaiPhuTung}</td>")
                  .AppendLine($"      <td>{newData?.Id_NhanHieu}</td>")
                  .AppendLine($"      <td>{newData?.Seri}</td>")
                  .AppendLine($"      <td>{newData?.Id_DonVi}</td>")
                  .AppendLine($"      <td>{newData?.KLCoThue}</td>")
                  .AppendLine($"      <td>{newData?.KLKhongThue}</td>")
                  ...
                  .AppendLine($"      <td>{newData?.DVSauQD}</td>")
                  .AppendLine($"      <td>{newData?.KLCoThueQD}</td>")
                  .AppendLine($"      <td>{newData?.KLKhongThueQD}</td>")
                  .AppendLine($"      <td>{newData?.DonGiaCoThueQD}</td>")
                  .AppendLine($"      <td>{newData?.DonGiaKhongThueQD}</td>")
                  .AppendLine($"      <td>{newData?.ThanhTienCoThueQD}</td>")
                  .AppendLine($"      <td>{newData?.ThanhTienKhongThueQD}</td>")
                  .AppendLine($"      <td>{newData?.TongTien}</td>")
        ...
        {
            var detail = await _khonhapkhophutungService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Thêm - KhoNhapKhoPhuTung</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Nơi nhập</th>")
              ...
              .AppendLine("  <tbody>");
            sb.AppendLine($"    <tr>")
              .AppendLine($"     <td>{detail.Id_Tram}</td>")
              .AppendLine($"     <td>{detail.DungChoThietBi}</td>")
              .AppendLine($"     <td>{detail.NgayNhapKho}</td>")
              .AppendLine($"     <td>{detail.SoPhieu}</td>")
              .AppendLine($"     <td>{detail.SoHopDong}</td>")
              .AppendLine($"     <td>{detail.Id_NhomPhuTung}</td>")
              .AppendLine($"     <td>{detail.Id_LoaiPhuTung}</td>")
              .AppendLine($"     <td>{detail.Id_NhanHieu}</td>")
              .AppendLine($"     <td>{detail.Seri}</td>")
              .AppendLine($"     <td>{detail.Id_DonVi}</td>")
              .AppendLine($"     <td>{detail.KLCoThue}</td>")
              .AppendLine($"     <td>{detail.KLKhongThue}</td>")
              ...
              .AppendLine($"     <td>{detail.DVSauQD}</td>")
              .AppendLine($"     <td>{detail.KLCoThueQD}</td>")
              .AppendLine($"     <td>{detail.KLKhongThueQD}</td>")
              .AppendLine($"     <td>{detail.DonGiaCoThueQD}</td>")
              .AppendLine($"     <td>{detail.DonGiaKhongThueQD}</td>")
              .AppendLine($"     <td>{detail.ThanhTienCoThueQD}</td>")
              .AppendLine($"     <td>{detail.ThanhTienKhongThueQD}</td>")
              .AppendLine($"     <td>{detail.TongTien}</td>")
        ...
        private async Task<string> BuildEmailContentDelete(string id)
        {
            var detail = await _khonhapkhophutungService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Xóa - KhoNhapKhoPhuTung</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Nơi nhập</th>")
        ...
        private ApprovalTask CreateApprovalTask(string title, string content, Kho_NhapKhoPhuTung input, string tableName, string parentMajorId, string majorId, string userId)
        {
        ...
        private ApprovalTask UpdateApprovalTask(string Id, string title, string content, Kho_NhapKhoPhuTung input, string tableName, string parentMajorId, string majorId, string userId)
        {
            return new ApprovalTask
            {
                Id = Id,
