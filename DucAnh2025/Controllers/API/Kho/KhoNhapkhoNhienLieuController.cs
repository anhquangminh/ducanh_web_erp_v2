...
namespace DucAnh2025.Controllers.API
{
    ...
    [ApiController]
    public class KhoNhapkhoNhienLieuController : ControllerBase
    {
        private readonly IApprovalTaskRepository _approvalTaskService;
        private readonly IKho_NhapkhoNhienLieuRepository _khonhapkhonhienlieuService;
        ...
        public KhoNhapkhoNhienLieuController(IApprovalTaskRepository approvalTaskRepository,
            IKho_NhapkhoNhienLieuRepository khonhapkhonhienlieuRepository,
            ...
            IPermissionRepository permissionService)
        {
            ...
            _khonhapkhonhienlieuService = khonhapkhonhienlieuRepository;
        ...
        public string tableName = "Kho_NhapkhoNhienLieus";

        [HttpPost("GetByVM")]
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetByVM(string groupId, [FromBody] Kho_NhapkhoNhienLieuModel? input = null)
        {
            try
            {
                var model = input ?? new Kho_NhapkhoNhienLieuModel();
                var list = await _khonhapkhonhienlieuService.GetAllByVM(model, groupId);
        ...
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetAll(string groupId)
        {
            try
            {
                var list = await _khonhapkhonhienlieuService.GetAll(groupId);
                return Ok(new ApiResponse<IEnumerable<object>>(true, "Thành công", list));
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetById(string id)
        {
            try
            {
                var result = await _khonhapkhonhienlieuService.GetById(id);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetHistory(string id)
        {
            try
            {
                var result = await _khonhapkhonhienlieuService.GetHistory(id);
                return Ok(new ApiResponse<object>(true, "Lưu thành công .", result));
            }
            catch (Exception ex)
            {
        ...
        [HttpPost("Create")]
        public async Task<ActionResult<ApiResponse<object>>> Create([FromBody] Kho_NhapkhoNhienLieu input, [FromQuery] bool isEdit)
        {
            try
            {
                ...
                if (editCheck)
                {
                    ...
                    if (!await _khonhapkhonhienlieuService.CheckExclusive(new[] { input.Id }, now))
                    ...
                    await _khonhapkhonhienlieuService.Update(input, "");
                    ...
                    {
                        await _approvalTaskService.Insert(CreateApprovalTask("Nhập kho nhiên liệu- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }
                    else
                    {
                        await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Nhập kho nhiên liệu- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }

                    await InsertEmailHistories(content, "Nhập kho nhiên liệu- sửa !", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
                ...
                else
                {
                    ...
                    input.DateApproval = null;
                    input.ApprovalDept = null;
                    await _khonhapkhonhienlieuService.Insert(input, "");
                    string content = await BuildEmailContentAdd(input.Id);
                    await _approvalTaskService.Insert(CreateApprovalTask("Nhập kho nhiên liệuThêm mơí !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    await InsertEmailHistories(content, "Nhập kho nhiên liệu - Thêm", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            try
            {
                ...
                var query = await _khonhapkhonhienlieuService.GetById(id);
                if (query == null)
                ...
                if (!await _khonhapkhonhienlieuService.CheckDelete(query))
                ...
                if (!await _khonhapkhonhienlieuService.CheckExclusive(new[] { id }, baseTime))
                ...
                await _khonhapkhonhienlieuService.Update(query, "");

                var content = await BuildEmailContentDelete(id);
                var dataApproval = await _approvalTaskService.GetByOriginalId(id);
                if (dataApproval == null)
                {
                    await _approvalTaskService.Insert(CreateApprovalTask("Nhập kho nhiên liệu - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }
                else
                {
                    await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Nhập kho nhiên liệu - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }

                await InsertEmailHistories(content, "Nhập kho nhiên liệu - xóa", query.GroupId, groupId, firstApproval.Id, query.Id, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> Duyet(string id)
        {
            try
            {
                var user = _applicationUserService.GetCurrentUser();
                var userId = user.Id;
                var entity = await _khonhapkhonhienlieuService.GetById(id);
                if (entity == null)
                ...
                if (entity.IsActive != 3)
                {
                    ...
                    await _khonhapkhonhienlieuService.Approval(entity, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> HuyDuyet(string id)
        {
            try
            {
                ...
                if (entity.IsActive != 3)
                {
                    await _khonhapkhonhienlieuService.NoApproval(entity, userId);
                    var dataNoApproval = await _khonhapkhonhienlieuService.GetById(entity.Id);
        ...
        {
            var isValidModel = await _khonhapkhonhienlieuService.GetHistoryIsValidEdit(id);
            var sb = new StringBuilder();

            sb.AppendLine("Thông tin sửa KhoNhapkhoNhienLieu</h3>")
             .AppendLine("<table class=\"table table-hover table-bordered\">")
             .AppendLine("  <thead class=\"bg-info\">")
             .AppendLine("    <tr>")
             .AppendLine("     <th>Tên trạm trộn</th>")
             .AppendLine("     <th>Ngày nhập kho</th>")
             .AppendLine("     <th>Chốt số cơ cây dầu trước khi nhập nhiên liệu Diezel</th>")
             .AppendLine("     <th>Số phiếu</th>")
             .AppendLine("     <th>Biển số xe</th>")
             .AppendLine("     <th>Số HĐ</th>")
             .AppendLine("     <th>Loại nhiên liệu</th>")
             .AppendLine("     <th>Nhãn hiệu</th>")
             .AppendLine("     <th>Đơn vị</th>")
             .AppendLine("     <th>Số lượng nhập kho có thuế</th>")
             .AppendLine("     <th>Số lượng nhập kho không thuế</th>")
             ...
             .AppendLine("     <th>Tổng thành tiền</th>")
             .AppendLine("     <th>Biển số xe (vận chuyển)</th>")
             .AppendLine("     <th>Đơn giá cước vận chuyển có thuế</th>")
             .AppendLine("     <th>Đơn giá cước vận chuyển không thuế</th>")
             .AppendLine("     <th>Thành tiền cước vận chuyển có thuế</th>")
             .AppendLine("     <th>Thành tiền cước vận chuyển không thuế</th>")
             .AppendLine("     <th>Tổng tiền hàng có thuế</th>")
             .AppendLine("     <th>Tổng tiền hàng không thuế</th>")
             .AppendLine("     <th>Tổng tiền hàng</th>")
            ...
            if (isValidModel.Any())
            {
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{oldData?.BienSo}</td>")
                  .AppendLine($"      <td>{oldData?.SoHopDong}</td>")
                  .AppendLine($"      <td>{oldData?.Id_LoaiNhienLieu}</td>")
                  .AppendLine($"      <td>{oldData?.Id_NhanHieu}</td>")
                  .AppendLine($"      <td>{oldData?.Id_DonVi}</td>")
                  .AppendLine($"      <td>{oldData?.SLNhapKhoCoThue}</td>")
                  .AppendLine($"      <td>{oldData?.SLNhapKhoKhongThue}</td>")
                  .AppendLine($"      <td>{oldData?.DonViSauQuyDoi}</td>")
                  .AppendLine($"      <td>{oldData?.SoLuongThucCoThue}</td>")
                  .AppendLine($"      <td>{oldData?.SoLuongThucKhongThue}</td>")
                  ...
                  .AppendLine($"      <td>{oldData?.TongThanhTien}</td>")
                  .AppendLine($"      <td>{oldData?.BienSoXe}</td>")
                  .AppendLine($"      <td>{oldData?.DonGiaCuocCoThue}</td>")
                  .AppendLine($"      <td>{oldData?.DonGiaCuocKhongThue}</td>")
                  .AppendLine($"      <td>{oldData?.ThanhTienCuocCoThue}</td>")
                  .AppendLine($"      <td>{oldData?.ThanhTienCuocKhongThue}</td>")
                  .AppendLine($"      <td>{oldData?.TongTienHangCoThue}</td>")
                  .AppendLine($"      <td>{oldData?.TongTienHangKhongThue}</td>")
                  .AppendLine($"      <td>{oldData?.TongTienHang}</td>")
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{newData?.BienSo}</td>")
                  .AppendLine($"      <td>{newData?.SoHopDong}</td>")
                  .AppendLine($"      <td>{newData?.Id_LoaiNhienLieu}</td>")
                  .AppendLine($"      <td>{newData?.Id_NhanHieu}</td>")
                  .AppendLine($"      <td>{newData?.Id_DonVi}</td>")
                  .AppendLine($"      <td>{newData?.SLNhapKhoCoThue}</td>")
                  .AppendLine($"      <td>{newData?.SLNhapKhoKhongThue}</td>")
                  .AppendLine($"      <td>{newData?.DonViSauQuyDoi}</td>")
                  .AppendLine($"      <td>{newData?.SoLuongThucCoThue}</td>")
                  .AppendLine($"      <td>{newData?.SoLuongThucKhongThue}</td>")
                  ...
                  .AppendLine($"      <td>{newData?.TongThanhTien}</td>")
                  .AppendLine($"      <td>{newData?.BienSoXe}</td>")
                  .AppendLine($"      <td>{newData?.DonGiaCuocCoThue}</td>")
                  .AppendLine($"      <td>{newData?.DonGiaCuocKhongThue}</td>")
                  .AppendLine($"      <td>{newData?.ThanhTienCuocCoThue}</td>")
                  .AppendLine($"      <td>{newData?.ThanhTienCuocKhongThue}</td>")
                  .AppendLine($"      <td>{newData?.TongTienHangCoThue}</td>")
                  .AppendLine($"      <td>{newData?.TongTienHangKhongThue}</td>")
                  .AppendLine($"      <td>{newData?.TongTienHang}</td>")
        ...
        {
            var detail = await _khonhapkhonhienlieuService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Thêm - KhoNhapkhoNhienLieu</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Tên trạm trộn</th>")
            ...
            sb.AppendLine($"    <tr>")
              ...
              .AppendLine($"     <td>{detail.BienSo}</td>")
              .AppendLine($"     <td>{detail.SoHopDong}</td>")
              .AppendLine($"     <td>{detail.Id_LoaiNhienLieu}</td>")
              .AppendLine($"     <td>{detail.Id_NhanHieu}</td>")
              .AppendLine($"     <td>{detail.Id_DonVi}</td>")
              .AppendLine($"     <td>{detail.SLNhapKhoCoThue}</td>")
              .AppendLine($"     <td>{detail.SLNhapKhoKhongThue}</td>")
              .AppendLine($"     <td>{detail.DonViSauQuyDoi}</td>")
              .AppendLine($"     <td>{detail.SoLuongThucCoThue}</td>")
              .AppendLine($"     <td>{detail.SoLuongThucKhongThue}</td>")
              ...
              .AppendLine($"     <td>{detail.TongThanhTien}</td>")
              .AppendLine($"     <td>{detail.BienSoXe}</td>")
              .AppendLine($"     <td>{detail.DonGiaCuocCoThue}</td>")
              .AppendLine($"     <td>{detail.DonGiaCuocKhongThue}</td>")
              .AppendLine($"     <td>{detail.ThanhTienCuocCoThue}</td>")
              .AppendLine($"     <td>{detail.ThanhTienCuocKhongThue}</td>")
              .AppendLine($"     <td>{detail.TongTienHangCoThue}</td>")
              .AppendLine($"     <td>{detail.TongTienHangKhongThue}</td>")
              .AppendLine($"     <td>{detail.TongTienHang}</td>")
        ...
        private async Task<string> BuildEmailContentDelete(string id)
        {
            var detail = await _khonhapkhonhienlieuService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Xóa - KhoNhapkhoNhienLieu</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Tên trạm trộn</th>")
        ...
        private ApprovalTask CreateApprovalTask(string title, string content, Kho_NhapkhoNhienLieu input, string tableName, string parentMajorId, string majorId, string userId)
        {
        ...
        private ApprovalTask UpdateApprovalTask(string Id, string title, string content, Kho_NhapkhoNhienLieu input, string tableName, string parentMajorId, string majorId, string userId)
        {
            return new ApprovalTask
            {
                Id = Id,
