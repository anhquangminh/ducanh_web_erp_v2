...
namespace DucAnh2025.Controllers.API.DanhMuc
{
    ...
    [ApiController]
    public class khoDMLoaiPhuTungController : ControllerBase
    {
        private readonly IApprovalTaskRepository _approvalTaskService;
        private readonly Ikho_DM_LoaiPhuTungRepository _khodmloaiphutungService;
        ...
        public khoDMLoaiPhuTungController(IApprovalTaskRepository approvalTaskRepository,
            Ikho_DM_LoaiPhuTungRepository khodmloaiphutungRepository,
            ...
            IPermissionRepository permissionService)
        {
            ...
            _khodmloaiphutungService = khodmloaiphutungRepository;
        ...
        public string tableName = "kho_DM_LoaiPhuTungs";

        [HttpPost("GetByVM")]
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetByVM(string groupId, [FromBody] kho_DM_LoaiPhuTungModel? input = null)
        {
            try
            {
                var model = input ?? new kho_DM_LoaiPhuTungModel();
                var list = await _khodmloaiphutungService.GetAllByVM(model, groupId);
        ...
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetAll(string groupId)
        {
            try
            {
                var list = await _khodmloaiphutungService.GetAll(groupId);
                return Ok(new ApiResponse<IEnumerable<object>>(true, "Thành công", list));
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetById(string id)
        {
            try
            {
                var result = await _khodmloaiphutungService.GetById(id);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetHistory(string id)
        {
            try
            {
                var result = await _khodmloaiphutungService.GetHistory(id);
                return Ok(new ApiResponse<object>(true, "Lưu thành công .", result));
            }
            catch (Exception ex)
            {
        ...
        [HttpPost("Create")]
        public async Task<ActionResult<ApiResponse<object>>> Create([FromBody] kho_DM_LoaiPhuTung input, [FromQuery] bool isEdit)
        {
            try
            {
                ...
                if (editCheck)
                {
                    ...
                    if (!await _khodmloaiphutungService.CheckExclusive(new[] { input.Id }, now))
                    ...
                    await _khodmloaiphutungService.Update(input, "");
                    ...
                    {
                        await _approvalTaskService.Insert(CreateApprovalTask("Loại phụ tùng- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }
                    else
                    {
                        await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Loại phụ tùng- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }

                    await InsertEmailHistories(content, "Loại phụ tùng- sửa !", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
                ...
                else
                {
                    ...
                    input.DateApproval = null;
                    input.ApprovalDept = null;
                    await _khodmloaiphutungService.Insert(input, "");
                    string content = await BuildEmailContentAdd(input.Id);
                    await _approvalTaskService.Insert(CreateApprovalTask("Loại phụ tùngThêm mơí !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    await InsertEmailHistories(content, "Loại phụ tùng - Thêm", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            try
            {
                ...
                var query = await _khodmloaiphutungService.GetById(id);
                if (query == null)
                ...
                if (!await _khodmloaiphutungService.CheckDelete(query))
                ...
                if (!await _khodmloaiphutungService.CheckExclusive(new[] { id }, baseTime))
                ...
                await _khodmloaiphutungService.Update(query, "");

                var content = await BuildEmailContentDelete(id);
                var dataApproval = await _approvalTaskService.GetByOriginalId(id);
                if (dataApproval == null)
                {
                    await _approvalTaskService.Insert(CreateApprovalTask("Loại phụ tùng - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }
                else
                {
                    await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Loại phụ tùng - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }

                await InsertEmailHistories(content, "Loại phụ tùng - xóa", query.GroupId, groupId, firstApproval.Id, query.Id, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> Duyet(string id)
        {
            try
            {
                var user = _applicationUserService.GetCurrentUser();
                var userId = user.Id;
                var entity = await _khodmloaiphutungService.GetById(id);
                if (entity == null)
                ...
                if (entity.IsActive != 3)
                {
                    ...
                    await _khodmloaiphutungService.Approval(entity, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> HuyDuyet(string id)
        {
            try
            {
                ...
                if (entity.IsActive != 3)
                {
                    await _khodmloaiphutungService.NoApproval(entity, userId);
                    var dataNoApproval = await _khodmloaiphutungService.GetById(entity.Id);
        ...
        {
            var isValidModel = await _khodmloaiphutungService.GetHistoryIsValidEdit(id);
            var sb = new StringBuilder();

            sb.AppendLine("Thông tin sửa khoDMLoaiPhuTung</h3>")
             .AppendLine("<table class=\"table table-hover table-bordered\">")
             .AppendLine("  <thead class=\"bg-info\">")
             .AppendLine("    <tr>")
             .AppendLine("     <th>Tên nhóm phụ tùng</th>")
             .AppendLine("     <th>Tên loại phụ tùng</th>")
            ...
            if (isValidModel.Any())
            {
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{oldData?.TenLoaiPhuTung}</td>")
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{newData?.TenLoaiPhuTung}</td>")
        ...
        {
            var detail = await _khodmloaiphutungService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Thêm - khoDMLoaiPhuTung</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Tên nhóm phụ tùng</th>")
            ...
            sb.AppendLine($"    <tr>")
              ...
              .AppendLine($"     <td>{detail.TenLoaiPhuTung}</td>")
        ...
        private async Task<string> BuildEmailContentDelete(string id)
        {
            var detail = await _khodmloaiphutungService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Xóa - khoDMLoaiPhuTung</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Tên nhóm phụ tùng</th>")
        ...
        private ApprovalTask CreateApprovalTask(string title, string content, kho_DM_LoaiPhuTung input, string tableName, string parentMajorId, string majorId, string userId)
        {
        ...
        private ApprovalTask UpdateApprovalTask(string Id, string title, string content, kho_DM_LoaiPhuTung input, string tableName, string parentMajorId, string majorId, string userId)
        {
            return new ApprovalTask
            {
                Id = Id,
