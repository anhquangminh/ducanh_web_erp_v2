...
namespace DucAnh2025.Controllers.API.DanhMuc
{
    ...
    [ApiController]
    public class KhoDMTenBaoCaoController : ControllerBase
    {
        private readonly IApprovalTaskRepository _approvalTaskService;
        private readonly IKho_DM_TenBaoCaoRepository _khodmtenbaocaoService;
        ...
        public KhoDMTenBaoCaoController(IApprovalTaskRepository approvalTaskRepository,
            IKho_DM_TenBaoCaoRepository khodmtenbaocaoRepository,
            ...
            IPermissionRepository permissionService)
        {
            ...
            _khodmtenbaocaoService = khodmtenbaocaoRepository;
        ...
        public string tableName = "Kho_DM_TenBaoCaos";

        [HttpPost("GetByVM")]
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetByVM(string groupId, [FromBody] Kho_DM_TenBaoCaoModel? input = null)
        {
            try
            {
                var model = input ?? new Kho_DM_TenBaoCaoModel();
                var list = await _khodmtenbaocaoService.GetAllByVM(model, groupId);
        ...
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetAll(string groupId)
        {
            try
            {
                var list = await _khodmtenbaocaoService.GetAll(groupId);
                return Ok(new ApiResponse<IEnumerable<object>>(true, "Thành công", list));
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetById(string id)
        {
            try
            {
                var result = await _khodmtenbaocaoService.GetById(id);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetHistory(string id)
        {
            try
            {
                var result = await _khodmtenbaocaoService.GetHistory(id);
                return Ok(new ApiResponse<object>(true, "Lưu thành công .", result));
            }
            catch (Exception ex)
            {
        ...
        [HttpPost("Create")]
        public async Task<ActionResult<ApiResponse<object>>> Create([FromBody] Kho_DM_TenBaoCao input, [FromQuery] bool isEdit)
        {
            try
            {
                ...
                if (editCheck)
                {
                    ...
                    if (!await _khodmtenbaocaoService.CheckExclusive(new[] { input.Id }, now))
                    ...
                    await _khodmtenbaocaoService.Update(input, "");
                    ...
                    {
                        await _approvalTaskService.Insert(CreateApprovalTask("Tên báo cáo- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }
                    else
                    {
                        await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Tên báo cáo- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }

                    await InsertEmailHistories(content, "Tên báo cáo- sửa !", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
                ...
                else
                {
                    ...
                    input.DateApproval = null;
                    input.ApprovalDept = null;
                    await _khodmtenbaocaoService.Insert(input, "");
                    string content = await BuildEmailContentAdd(input.Id);
                    await _approvalTaskService.Insert(CreateApprovalTask("Tên báo cáoThêm mơí !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    await InsertEmailHistories(content, "Tên báo cáo - Thêm", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            try
            {
                ...
                var query = await _khodmtenbaocaoService.GetById(id);
                if (query == null)
                ...
                if (!await _khodmtenbaocaoService.CheckDelete(query))
                ...
                if (!await _khodmtenbaocaoService.CheckExclusive(new[] { id }, baseTime))
                ...
                await _khodmtenbaocaoService.Update(query, "");

                var content = await BuildEmailContentDelete(id);
                var dataApproval = await _approvalTaskService.GetByOriginalId(id);
                if (dataApproval == null)
                {
                    await _approvalTaskService.Insert(CreateApprovalTask("Tên báo cáo - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }
                else
                {
                    await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Tên báo cáo - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }

                await InsertEmailHistories(content, "Tên báo cáo - xóa", query.GroupId, groupId, firstApproval.Id, query.Id, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> Duyet(string id)
        {
            try
            {
                var user = _applicationUserService.GetCurrentUser();
                var userId = user.Id;
                var entity = await _khodmtenbaocaoService.GetById(id);
                if (entity == null)
                ...
                if (entity.IsActive != 3)
                {
                    ...
                    await _khodmtenbaocaoService.Approval(entity, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> HuyDuyet(string id)
        {
            try
            {
                ...
                if (entity.IsActive != 3)
                {
                    await _khodmtenbaocaoService.NoApproval(entity, userId);
                    var dataNoApproval = await _khodmtenbaocaoService.GetById(entity.Id);
        ...
        {
            var isValidModel = await _khodmtenbaocaoService.GetHistoryIsValidEdit(id);
            var sb = new StringBuilder();

            sb.AppendLine("Thông tin sửa KhoDMTenBaoCao</h3>")
            ...
            if (isValidModel.Any())
            {
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{oldData?.Id_DanhMuc}</td>")
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{newData?.Id_DanhMuc}</td>")
        ...
        {
            var detail = await _khodmtenbaocaoService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Thêm - KhoDMTenBaoCao</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Tên danh mục</th>")
              ...
              .AppendLine("  <tbody>");
            sb.AppendLine($"    <tr>")
              .AppendLine($"     <td>{detail.Id_DanhMuc}</td>")
        ...
        private async Task<string> BuildEmailContentDelete(string id)
        {
            var detail = await _khodmtenbaocaoService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Xóa - KhoDMTenBaoCao</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Tên danh mục</th>")
        ...
        private ApprovalTask CreateApprovalTask(string title, string content, Kho_DM_TenBaoCao input, string tableName, string parentMajorId, string majorId, string userId)
        {
        ...
        private ApprovalTask UpdateApprovalTask(string Id, string title, string content, Kho_DM_TenBaoCao input, string tableName, string parentMajorId, string majorId, string userId)
        {
            return new ApprovalTask
            {
                Id = Id,
