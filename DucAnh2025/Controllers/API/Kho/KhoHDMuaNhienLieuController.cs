...
namespace DucAnh2025.Controllers.API
{
    ...
    [ApiController]
    public class KhoHDMuaNhienLieuController : ControllerBase
    {
        private readonly IApprovalTaskRepository _approvalTaskService;
        private readonly IKho_HDMuaNhienLieuRepository _khohdmuanhienlieuService;
        ...
        public KhoHDMuaNhienLieuController(IApprovalTaskRepository approvalTaskRepository,
            IKho_HDMuaNhienLieuRepository khohdmuanhienlieuRepository,
            ...
            IPermissionRepository permissionService)
        {
            ...
            _khohdmuanhienlieuService = khohdmuanhienlieuRepository;
        ...
        public string tableName = "Kho_HDMuaNhienLieus";

        [HttpPost("GetByVM")]
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetByVM(string groupId, [FromBody] Kho_HDMuaNhienLieuModel? input = null)
        {
            try
            {
                var model = input ?? new Kho_HDMuaNhienLieuModel();
                var list = await _khohdmuanhienlieuService.GetAllByVM(model, groupId);
        ...
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetAll(string groupId)
        {
            try
            {
                var list = await _khohdmuanhienlieuService.GetAll(groupId);
                return Ok(new ApiResponse<IEnumerable<object>>(true, "Thành công", list));
            }
            catch (Exception ex)
            {
        ...
        [HttpGet("GetAllModel")]
        public async Task<ActionResult<ApiResponse<IEnumerable<object>>>> GetAllModel(string groupId)
        {
            try
            {
                var list = await _khohdmuanhienlieuService.GetAllModel(groupId);
                return Ok(new ApiResponse<IEnumerable<object>>(true, "Thành công", list));
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetById(string id)
        {
            try
            {
                var result = await _khohdmuanhienlieuService.GetById(id);
        ...
        public async Task<ActionResult<ApiResponse<object>>> GetHistory(string id)
        {
            try
            {
                var result = await _khohdmuanhienlieuService.GetHistory(id);
                return Ok(new ApiResponse<object>(true, "Lưu thành công .", result));
            }
            catch (Exception ex)
            {
        ...
        [HttpPost("Create")]
        public async Task<ActionResult<ApiResponse<object>>> Create([FromBody] Kho_HDMuaNhienLieu input, [FromQuery] bool isEdit)
        {
            try
            {
                ...
                if (editCheck)
                {
                    ...
                    if (!await _khohdmuanhienlieuService.CheckExclusive(new[] { input.Id }, now))
                    ...
                    await _khohdmuanhienlieuService.Update(input, "");
                    ...
                    {
                        await _approvalTaskService.Insert(CreateApprovalTask("Hợp đồng mua nhiên liệu- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }
                    else
                    {
                        await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Hợp đồng mua nhiên liệu- sửa !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    }

                    await InsertEmailHistories(content, "Hợp đồng mua nhiên liệu- sửa !", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
                ...
                else
                {
                    ...
                    input.DateApproval = null;
                    input.ApprovalDept = null;
                    await _khohdmuanhienlieuService.Insert(input, "");
                    string content = await BuildEmailContentAdd(input.Id);
                    await _approvalTaskService.Insert(CreateApprovalTask("Hợp đồng mua nhiên liệuThêm mơí !", content, input, tableName, parentMajorId, majorId, userId), userId);
                    await InsertEmailHistories(content, "Hợp đồng mua nhiên liệu - Thêm", input.GroupId, groupId, firstApproval.Id, input.Id, userId);
        ...
        public async Task<IActionResult> Delete(string id)
        {
            ...
            try
            {
                ...
                var query = await _khohdmuanhienlieuService.GetById(id);
                if (query == null)
                ...
                if (!await _khohdmuanhienlieuService.CheckDelete(query))
                ...
                if (!await _khohdmuanhienlieuService.CheckExclusive(new[] { id }, baseTime))
                ...
                await _khohdmuanhienlieuService.Update(query, "");

                var content = await BuildEmailContentDelete(id);
                var dataApproval = await _approvalTaskService.GetByOriginalId(id);
                if (dataApproval == null)
                {
                    await _approvalTaskService.Insert(CreateApprovalTask("Hợp đồng mua nhiên liệu - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }
                else
                {
                    await _approvalTaskService.Update(UpdateApprovalTask(dataApproval.Id, "Hợp đồng mua nhiên liệu - xóa", content, query, tableName, parentMajorId, majorId, userId), userId);
                }

                await InsertEmailHistories(content, "Hợp đồng mua nhiên liệu - xóa", query.GroupId, groupId, firstApproval.Id, query.Id, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> Duyet(string id)
        {
            try
            {
                var user = _applicationUserService.GetCurrentUser();
                var userId = user.Id;
                var entity = await _khohdmuanhienlieuService.GetById(id);
                if (entity == null)
                ...
                if (entity.IsActive != 3)
                {
                    ...
                    await _khohdmuanhienlieuService.Approval(entity, userId);
        ...
        public async Task<ActionResult<ApiResponse<object>>> HuyDuyet(string id)
        {
            try
            {
                ...
                if (entity.IsActive != 3)
                {
                    await _khohdmuanhienlieuService.NoApproval(entity, userId);
                    var dataNoApproval = await _khohdmuanhienlieuService.GetById(entity.Id);
        ...
        {
            var isValidModel = await _khohdmuanhienlieuService.GetHistoryIsValidEdit(id);
            var sb = new StringBuilder();

            sb.AppendLine("Thông tin sửa KhoHDMuaNhienLieu</h3>")
             .AppendLine("<table class=\"table table-hover table-bordered\">")
             .AppendLine("  <thead class=\"bg-info\">")
             .AppendLine("    <tr>")
             .AppendLine("     <th>Tên Trạm</th>")
             .AppendLine("     <th>Ngày ký hợp đồng</th>")
             .AppendLine("     <th>Số HĐ</th>")
             .AppendLine("     <th>Loại nhà cung cấp</th>")
             ...
             .AppendLine("     <th>Có thuế (BG)</th>")
             .AppendLine("     <th>Không thuế (BG)</th>")
             .AppendLine("     <th>Có thuế (KB)</th>")
             .AppendLine("     <th>Không thuế (KB)</th>")
             .AppendLine("     <th>Từ ngày</th>")
             .AppendLine("     <th>Đến ngày</th>")
            ...
            if (isValidModel.Any())
            {
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{oldData?.Id_TenTram}</td>")
                  .AppendLine($"      <td>{oldData?.NgayKyHopDong}</td>")
                  .AppendLine($"      <td>{oldData?.SoHopDong}</td>")
                  .AppendLine($"      <td>{oldData?.Id_LoaiNhaCungCap}</td>")
                  .AppendLine($"      <td>{oldData?.Id_NhaCungcap}</td>")
                  ...
                  .AppendLine($"      <td>{oldData?.DonGiaBGCoThue}</td>")
                  .AppendLine($"      <td>{oldData?.DonGiaBGKhongThue}</td>")
                  .AppendLine($"      <td>{oldData?.DonGiaKBCoThue}</td>")
                  .AppendLine($"      <td>{oldData?.DonGiaKBKhongThue}</td>")
                  .AppendLine($"      <td>{oldData?.TuNgay}</td>")
                  .AppendLine($"      <td>{oldData?.DenNgay}</td>")
                ...
                sb.AppendLine("    <tr>")
                  ...
                  .AppendLine($"      <td>{newData?.Id_TenTram}</td>")
                  .AppendLine($"      <td>{newData?.NgayKyHopDong}</td>")
                  .AppendLine($"      <td>{newData?.SoHopDong}</td>")
                  .AppendLine($"      <td>{newData?.Id_LoaiNhaCungCap}</td>")
                  .AppendLine($"      <td>{newData?.Id_NhaCungcap}</td>")
                  ...
                  .AppendLine($"      <td>{newData?.DonGiaBGCoThue}</td>")
                  .AppendLine($"      <td>{newData?.DonGiaBGKhongThue}</td>")
                  .AppendLine($"      <td>{newData?.DonGiaKBCoThue}</td>")
                  .AppendLine($"      <td>{newData?.DonGiaKBKhongThue}</td>")
                  .AppendLine($"      <td>{newData?.TuNgay}</td>")
                  .AppendLine($"      <td>{newData?.DenNgay}</td>")
        ...
        {
            var detail = await _khohdmuanhienlieuService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Thêm - KhoHDMuaNhienLieu</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Tên Trạm</th>")
              ...
              .AppendLine("  <tbody>");
            sb.AppendLine($"    <tr>")
              .AppendLine($"     <td>{detail.Id_TenTram}</td>")
              .AppendLine($"     <td>{detail.NgayKyHopDong}</td>")
              .AppendLine($"     <td>{detail.SoHopDong}</td>")
              .AppendLine($"     <td>{detail.Id_LoaiNhaCungCap}</td>")
              .AppendLine($"     <td>{detail.Id_NhaCungcap}</td>")
              ...
              .AppendLine($"     <td>{detail.DonGiaBGCoThue}</td>")
              .AppendLine($"     <td>{detail.DonGiaBGKhongThue}</td>")
              .AppendLine($"     <td>{detail.DonGiaKBCoThue}</td>")
              .AppendLine($"     <td>{detail.DonGiaKBKhongThue}</td>")
              .AppendLine($"     <td>{detail.TuNgay}</td>")
              .AppendLine($"     <td>{detail.DenNgay}</td>")
        ...
        private async Task<string> BuildEmailContentDelete(string id)
        {
            var detail = await _khohdmuanhienlieuService.GetDetails(id);
            var sb = new StringBuilder();
            sb.AppendLine("<h3>Xóa - KhoHDMuaNhienLieu</h3>")
              .AppendLine("<table class=\"table table-hover table-bordered\">")
              .AppendLine("  <thead class=\"bg-info\">")
              .AppendLine("    <tr>")
              .AppendLine("     <th>Tên Trạm</th>")
        ...
        private ApprovalTask CreateApprovalTask(string title, string content, Kho_HDMuaNhienLieu input, string tableName, string parentMajorId, string majorId, string userId)
        {
        ...
        private ApprovalTask UpdateApprovalTask(string Id, string title, string content, Kho_HDMuaNhienLieu input, string tableName, string parentMajorId, string majorId, string userId)
        {
            return new ApprovalTask
            {
                Id = Id,
